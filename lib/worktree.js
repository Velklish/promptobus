...
import {
  PROC_INSTALL_TIMEOUT_MS, gitIn, lastLines, procTimedOut, runProc, shellQuote, toPosix,
  writeFileAtomic,
} from './util.js';
...
// it — the journal stays with the service one. git is called directly via the shared
// `gitIn` helper, not through exec.js: the wrapper is needed where a binary on Windows
// has a .cmd, and git is a native .exe. There are no heavy imports: the bus MCP server,
// started for every participant, also reads this module.
 ...
 * Why `branchAdds` stops answering after a squash merge: [reference/03-cli.md#squashedinto--was-the-branchs-work-taken-into-the-base-as-one-commit](../docs/reference/03-cli.md#squashedinto--was-the-branchs-work-taken-into-the-base-as-one-commit) */
...
// Removing the directory and the branch. The caller reaches this function only after the
// content measurements prove the directory removable. A force delete is required because
// ancestry cannot confirm a squash merge.
export function removeWorktree(repoAbs, worktreePath, branch) {
  ...
  // deletion. The `worktree-` prefix is the ownership check, while the caller's
  // disposition is the content proof for the force delete.
