...
/**
 * Why this file is not the record its name claims, or `null` — it is one, or it claims
 * nothing. A document its own schema refuses looks like evidence and is not, and nobody
 * reads the schema by hand in time.
...
export function recordRefusal(filename, file) {
  ...
  let raw;
  // Unreadable is not this check's refusal: the send says «artifact is missing» about the
  // same file, and two refusals naming different causes is worse than one.
