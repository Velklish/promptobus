...
/** The attachment contract, quoted WORD FOR WORD by the worker and the reviewer preambles:
 * one constant is why the two sides cannot drift apart on it. */
...
export function recordRefusal(filename, file) {
  ...
  return counts ? `«${filename}» contradicts itself: ${counts}` : null;
}
