...
export function decideApproval(method, params, record) {
  ...
  if (method === 'currentTime/read' || method === 'item/tool/requestUserInput') {
    return { allow: true };
  }
...
export async function holdMain(argv = process.argv.slice(2), env = process.env) {
  ...
  rpc.onServerRequest((msg) => {
    ...
    if (!decision.allow) {
      log(`approval deny ${msg.method} ${decision.why}${who ? ` ${who}` : ''}`);
  ...
  try {
    ...
    // ReviewStartParams has no effort. ThreadStartParams.config is the open
    // passthrough that already carries mcp_servers; model_reasoning_effort on
    // that object is what a Codex reviewer (and worker) can actually honour.
    // Measured on codex-cli 0.146.0 with no paid turn: the thread echoes the
    // asked level as ThreadStartResponse.reasoningEffort. The worker still
    // also sends effort on the first turn/start — that value persists across
