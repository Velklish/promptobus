...
// общий процесс на рабочее место уносит всех своей смертью, а замка на потоке
// у Codex нет.
//
// Дом реестра — `~/.promptobus/codex`, не `~/.codex`. Второе — дом человека, и писать туда
// сверх того, что кладёт сам Codex, механизм не вправе. `PROMPTOBUS_CODEX_HOME` уводит
...
export function codexStateHome(env = process.env) {
  const named = String(env.PROMPTOBUS_CODEX_HOME ?? '').trim();
  return named || path.join(homedir(), '.promptobus', 'codex');
}
...
// — строка бинаря Codex и замер, поэтому `phrases.tool` обязан звать то же имя.
 ...
 * через транспорт закрывает префикс ключа: наш url поверх личного stdio того же
 * канонического имени больше не склеивается, потому что в оверрайд уезжает `promptobus-<имя>`.
