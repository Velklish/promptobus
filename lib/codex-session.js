...
import {
  appendFileSync, chmodSync, existsSync, mkdirSync, readdirSync, readFileSync, renameSync, rmSync,
  writeFileSync,
} from 'node:fs';
...
// Codex config vocabulary as documented for codex-cli 0.146.0: `sandbox` uses
// `danger-full-access`; `approvalPolicy` uses
// `untrusted|on-failure|on-request|never`; and the `dangerously-bypass*`
// entries are CLI flag spellings. `on-failure` lowers and `never` disables
// the approval gate. The nested `item` shape, string/map forms of
// `permissions`, and whether app-server sends `approvalPolicy` in a
// server-to-client approval at all are assumed from this file's `item`
// convention, unmeasured.
...
function asksForEscalatedPermission(params, record) {
  // Keep this slot list explicit: adding `params` or `params.item` wholesale would reopen blob matching.
  const slots = [
    ...
    { value: params?.item?.sandbox, denied: DANGEROUS_SANDBOX_VALUES, running: record?.sandbox },
    { value: params?.item?.permissions, denied: DANGEROUS_PERMISSION_VALUES },
    { value: params?.item?.approvalPolicy, denied: DANGEROUS_APPROVAL_POLICY_VALUES, running: record?.approvalPolicy },
  ];
...
/** Allowlisted fields of a server-to-client request. Never the prompt, schema, url or params blob. */
...
//
// The COLLISION is the reason; the word is not. Any namespace the personal config
// does not use closes it equally, so the word is the consumer's own name, which
// the host already declares — a participant reading `mcp__<key>__<tool>` sees the
// CLI it is running under and not a package it never heard of.
...
export async function holdMain(argv = process.argv.slice(2), env = process.env) {
  ...
  rpc.onServerRequest((msg) => {
    ...
    const who = [summary.server && `server=${summary.server}`, summary.mode && `mode=${summary.mode}`]
      .filter(Boolean).join(' ');
  ...
  try {
    ...
    const mcp = codexMcpServers(record.mcpServers, record.mcpPrefix);
    if (mcp.skipped.length) {
      log(`MCP did not go — Codex has no transport of that form: ${mcp.skipped.join(', ')}`);
    }
    // ThreadStartParams.config is the open passthrough that already carries
    // mcp_servers; model_reasoning_effort on that object is what both roles
    // honour. Measured on codex-cli 0.146.0 with no paid turn: the thread
    // echoes the asked level as ThreadStartResponse.reasoningEffort. The
    // first turn/start still also sends effort — that value persists across
    // later wakes on the same thread.
    const startParams = {
      cwd: record.cwd,
