...
/**
 * Accept options of one elicitation field — both single-select spellings of the measured
 * `McpElicitationSchema`, minus the decline const.
 */
function acceptOptions(schema) {
  const s = schema && typeof schema === 'object' ? schema : {};
  const consts = Array.isArray(s.oneOf) || Array.isArray(s.anyOf)
    ? (s.oneOf ?? s.anyOf).map((o) => (o && typeof o === 'object' ? o.const : null))
    : (Array.isArray(s.enum) ? s.enum : []);
  return consts.filter((c) => typeof c === 'string' && c && c !== CODEX_DECLINE_CONST);
}

/**
 * `content` of an accepted elicitation: each required field takes its first non-decline
 * option. Derived from the schema, NOT verified live — 03-cli § The Codex holder.
 */
function elicitationAccept(params) {
  const schema = params && typeof params === 'object' ? params.requestedSchema : null;
  ...
  for (const name of required) {
    const [option] = acceptOptions(props[name]);
    if (option) content[name] = option;
  }
  return { action: 'accept', content };
}

export function approvalReply(method, allow, params) {
  if (method === 'mcpServer/elicitation/request' && allow) return elicitationAccept(params);
...
// Strings of codex-cli 0.146.0 that mark its OWN per-call MCP approval and its decline
// option — 03-cli § The Codex holder carries where they were read from (PB-161.4).
const CODEX_APPROVAL_KIND = 'codex_approval_kind';
const CODEX_DECLINE_CONST = '__codex_mcp_decline__';
...
function approvalKind(params) {
  ...
  return kind == null || kind === '' ? null : String(kind);
}
...
export function decideApproval(method, params, record) {
  ...
  if (method === 'mcpServer/elicitation/request') {
    // One method, two questions: codex-cli's own tool approval is accepted, a server's
    // question to a person is declined — 03-cli § The Codex holder (PB-161.4, PB-41).
    if (approvalKind(params)) return { allow: true };
...
export async function holdMain(argv = process.argv.slice(2), env = process.env) {
  ...
  rpc.onServerRequest((msg) => {
    ...
    if (decision.unknown) {
      ...
      markReply();
      return approvalReply(msg.method, false, msg.params);
    }
    if (!decision.allow) {
    ...
    return approvalReply(msg.method, true, msg.params);
  });
