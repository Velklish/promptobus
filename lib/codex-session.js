...
/**
 * A schema with nothing to fill — codex-cli's own `is_message_only_schema`, mirrored
 * field for field from `mcp_server_elicitation.rs` at tag `rust-v0.146.0`. A missing
 * `properties` is NOT message-only there, and a missing `type` is not either.
 */
...
export function decideApproval(method, params, record) {
  ...
  if (method === 'mcpServer/elicitation/request') {
    // Codex-cli's own tool approval is BOTH halves of its own condition — the marker and a
    // message-only schema. Either alone is a question nobody here can answer (PB-161.4).
    const kind = approvalKind(params);
    if (kind === MCP_TOOL_CALL_KIND && messageOnlySchema(params)) return { allow: true };
    if (kind === MCP_TOOL_CALL_KIND) {
      return { allow: false, why: 'an elicitation marked as a tool-call approval but carrying a form to fill is not one — the participant has no person to fill it' };
    }
