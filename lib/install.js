...
function cursorEvents(busCmd, guardCmd, matcher) {
  return {
    postToolUse: [{ command: busCmd, matcher }],
    stop: [{ command: guardCmd }],
  };
}
...
const SPECS = {
  ...
  cursor: {
    ...
    guardEvents: ['stop'],
    matcher: BUS_HOOK_MATCHER,
    outputArgs: ['--output', 'additional_context'],
    events: cursorEvents,
    wrap(doc, hooks) {
      const next = { ...doc };
...
export function planHookInstall(host, root, wanted) {
  ...
  for (const name of INSTALL_HARNESSES) {
    ...
    const busCmd = busHookCommand(hookHost, spec.outputArgs, platform);
