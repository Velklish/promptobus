...
// идентификатор сессии (замечание ревью): по нему гейт владения адресом сверяется
...
export async function liftoffParticipant({
  ...
  if (r.error?.code === 'ENOENT') fail(`${tool.bin}: бинарь пропал между проверкой и запуском — ${who.acc} поднимать нечем.${launchFailNote}`);
...
// [protocol.ts](../src/protocol.ts), и там он значит ПОЛНЫЙ
...
// является, а гейт владения адресом на ней стоит fail-closed. Записи нет — `null`,
...
// против 60 на неразобранном. Такого читателя нет, и заводить его нельзя (,
