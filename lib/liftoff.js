...
export async function liftoffParticipant({
  ...
  if (r.status !== 0) fail(`claude --bg exited with code ${r.status}${output ? `: ${output}` : ''}.${launchFailNote}`);
  ...
  if (seen.state === 'dead') {
    ...
    fail(`claude --bg reported success, but there is no live session "${name}" in claude agents — ${who.nom} was NOT started.`
