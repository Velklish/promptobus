import { createRegistry, driverFor, harnessOf, snapshotSessions } from '../../packages/promptobus/dist/index.js';
...
//
// **Этот файл — единственная дверь механизма к driver'ам** (`BL-466`, ADR-034). Остальные
...
// зовут объявленные контрактом операции. Держит это гейт границы adapter'а
// ([promptobus-adapter.test.mjs](../../test/promptobus-adapter.test.mjs)), и цена его —
// ровно та, ради которой он заведён: второй production driver кладётся в карту ниже, не
 ...
 * про driver'ы не знает (роль и ID разводит `BL-409`). Непустое незнакомое имя fallback не
 ...
 * обхода после закрытия не бросают» (`BL-374`). Запись с чужим harness'ом туда попадает
