...
/**
 * Who the session running this process is. `null` is an answer and not a silence: the
 * caller gets a reason for it, and two candidates are refused rather than picked.
 */
export function resolveSessionIdentity(env = process.env, scope = null) {
  const candidates = identityCandidates(env);
  if (candidates.length === 1) return { id: candidates[0].id, ...candidates[0], candidates, why: null };
  if (!candidates.length) {
    const records = mcpIdentityCandidates(env, scope);
    if (records.length === 1) return { id: records[0].id, ...records[0], candidates: records, why: null };
    if (records.length > 1) {
      return {
        id: null, harness: null, variable: null, candidates: records,
        why: `${records.length} MCP session records prove this address at once (${
          records.map((c) => `${c.harness} via ${c.variable}`).join(', ')
        }) — picking one would hand out an unproven session id`,
      };
    }
    return {
      id: null,
  ...
  return {
    ...
    }) — an ancestor's identity leaked into this session, and picking one would hand out someone else's id`,
  };
}
