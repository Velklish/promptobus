...
/**
 * Who the session running this process is. `null` is an answer and not a silence: the caller
 * gets a NAMED reason for it, and two candidates are refused rather than picked.
 *
 * `reason` is the state, not its prose: `none` — nothing names a session here; `contested`
 * — more than one environment variable does; `contested-records` — more than one MCP session
 * record does. Nothing and too much are opposite troubles and take opposite repairs, and a
 * caller that shows only `why` must still be able to tell them apart — 02-host § Session identity.
 */
