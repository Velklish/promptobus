...
// The task warden is a PROCESS, not a state machine: the only listener and activator.
// Why a process and what its death costs: [guides/hooks-and-trust.md#the-warden-is-a-process-not-a-state-machine](../docs/guides/hooks-and-trust.md#the-warden-is-a-process-not-a-state-machine)
...
// What a round does, and what it leaves to the driver.
// The split: [guides/hooks-and-trust.md#wardenround--one-watch-round](../docs/guides/hooks-and-trust.md#wardenround--one-watch-round)
 ...
 * participant about.
 */
export async function reportStalls(home, task, { sessions = undefined, now = Date.now(), host = null } = {}) {
  const fresh = await stallRound(home, task, {
    now,
    sessions: sessions === undefined ? snapshotOf(readTask(home, task).participants) : sessions,
  });
  return fresh.map((s) => stallLine(s, task, host));
}
