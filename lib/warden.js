...
import { AGENTS_BIN, ok, info, warn } from '../util.js';
import { cliVersion } from '../base.js';
import {
  addressOf, claimWarden, clearWarden, inboxDir, liveWarden, logWarden, readTask, resolveIdentity,
  sessionIdentity,
...
//
// **Решения принимает не этот файл** (`BL-408`). Круги, пороги перестука, health
// непрочитанного, эскалация молчания, разбор стопа и решение «кого активировать» живут в
// package ([supervisor.ts](../../packages/promptobus/src/supervisor.ts)) и про harness не
...
export { beatRound, liveWatched } from '../../packages/promptobus/dist/index.js';
 ...
 *
 * **Снимок круг не запрашивает и запрашивать не вправе** (`BL-420`). Он приходит аргументом
...
function launchWarden(home, task, env) {
  ...
  const child = spawn(process.execPath, [AGENTS_BIN, 'promptobus', 'warden', '--task', task], {
    detached: true,
    stdio: 'ignore',
    env: clean,
  });
...
// Сам процесс. Запускается `ati-agents promptobus warden --task <id>`; корня workspace ему
...
export async function warden(opts = {}, env = process.env, cwd = process.cwd()) {
  ...
  const claimed = claimWarden(home, task, { cli: cliVersion(), session: sessionIdentity() });
  if (claimed.busy) {
    warn(`надзиратель задачи ${task} уже работает: pid ${claimed.busy.pid}, удар сердца ${claimed.busy.beat}`);
    return;
  }
  ok(`надзиратель задачи ${task}: pid ${process.pid} · CLI ${cliVersion()}`);
  logWarden(home, task, `надзиратель поднят · pid ${process.pid} · CLI ${cliVersion()}`);
