...
// Надзиратель задачи — ПРОЦЕСС, а не машина состояний.
//
...
function wardenArgv(host, task) {
  return { node: host.nodePath(), argv: [host.binPath(), 'warden', '--task', task] };
}
...
export function ensureWarden(home, task, { env = process.env, launch = launchWarden, host } = {}) {
  if (host == null) throw new Error('ensureWarden: host обязателен');
  try {
...
export async function warden(opts = {}, env = process.env, cwd = process.cwd()) {
  ...
  if (host == null) throw new Error('warden: host обязателен');
