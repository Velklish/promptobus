...
// Closing a task: the order of close, telemetry and the sweeps.
// Why that order: [reference/03-cli.md#stopmanaged--stop-managed-sessions-of-a-closed-task](../docs/reference/03-cli.md#stopmanaged--stop-managed-sessions-of-a-closed-task)
...
function sweepWorktrees(home, snapshot, host) {
  ...
  for (const meta of listTasks(home)) {
    ...
    for (const p of meta.participants ?? []) {
      // Liveness is read first, and for every participant: harness state can live
      // OUTSIDE the worktree, and the participant that has none is the case that
      // proves it — a Codex reviewer has no worktree and still has an isolated home
      // holding a copy of the owner's credentials. The read is a lookup in the
      // snapshot already built above, so asking for it earlier costs no poll.
...
function sweepParticipantSecrets(home, snapshot) {
  for (const meta of listTasks(home)) {
    ...
    for (const p of meta.participants ?? []) {
      ...
      // Participant directories the driver opened: for Cursor and Codex they hold
      // the reviewer's own working directory, and in it what the lift put there.
      ...
      for (const dir of dirs) {
        ...
        // Neutral about the contents: what sits in the directory is the driver's, and
        // it is not the same for two of them — one leaves an MCP config with tokens, the
        // other a copy of the skills canon. The reason for the removal is the same.
 ...
 * Why it runs after the close and before the sweeps: [reference/03-cli.md#recordtelemetry--one-telemetry-record-per-participant-that-lifted-a-session](../docs/reference/03-cli.md#recordtelemetry--one-telemetry-record-per-participant-that-lifted-a-session) */
...
export async function done(rootOrHost, opts = {}) {
  ...
  if (own.gated) {
    fail(`${foreignTaskLine(readTask(home, id), own)}: the run is finished by its owner. `
      ...
      + `to attach a track, and this one closes the run. ${claimRoute(host.busCommand(['done']))}`);
  }
  // Read BEFORE the close: `closeTask` is idempotent, so afterwards there is
  // nothing left to tell a first close from a repeat, and the telemetry append
  // below must happen exactly once per run.
