...
import { ok, info, warn, fail, shellQuote } from './copy/util.js';
...
/**
 * Погасить managed-сессии закрытой задачи (`BL-424`). Сессию, которую поднял механизм, он
 ...
 * которого нельзя бросать (`BL-374`).
 *
 * `registry` — шов набора: подставной driver считает вызовы, не трогая живого `claude`.
 * `snapshot` — второй шов, функция по участникам (`BL-465`): его подаёт `done`, чтобы вся
...
export async function stopManaged(home, id, { registry = REGISTRY, snapshot = null } = {}) {
  ...
  // закрытия задачи, и отказ отсюда унёс бы с собой остальных участников (`BL-374`).
  const driverAt = (p) => {
    try {
  ...
  let idle = 0;
  let failed = 0;
  // Четвёртый исход, а не оттенок третьего (замечание ревью, `BL-473`): команда гашения
  ...
  for (const p of targets) {
    try {
      ...
      // станет (`BL-473`), и без ожидания уборка ниже пошла бы по состоянию, которого ещё
...
function sweepParticipantSecrets(home, snapshot) {
  for (const meta of listTasks(home)) {
    ...
    // `BL-459` (2026-09-02, дом с 20 закрытыми задачами по 3 участника с сессиями): запусков
    ...
    for (const p of meta.participants ?? []) {
      ...
      // Каталоги участника, заведённые driver'ом (`BL-468`): у Cursor в них лежит рабочее
...
export async function done(rootOrHost, opts = {}) {
  ...
  // Снимок сессий — швом набора, как `sessions` у `status` (`BL-419`) и `wardenRound`: без него
  ...
  // а один адрес живёт в разных задачах разными сессиями (`BL-465`).
  ...
  // run заканчивает. Отказ печатает `fail()`, а не бросок: верхний catch `agents.js`
  ...
  //
  // `--keep-sessions` — выключатель необратимого действия (решение оркестратора, `BL-424`):
  ...
  if (keepSessions) {
    ...
    // Состояние сессий обход берёт ЗАНОВО — уже после гашения (`BL-473`). Реестр driver'а
    ...
    // процесса на каждом `promptobus done` (`BL-465`, счёт запусков там же).
  ...
  // (`BL-432`, решение владельца 2026-09-02). Только что закрытая под неё не попадает ни при
  ...
  try {
    ...
    warn(`журналы давно закрытых задач не убраны (${e.message}) — задача ${id} закрыта, `
      + `это её не отменяет. Убрать руками: ${host.formatNpx(['promptobus', 'prune', '--yes'])}`);
  }
}
