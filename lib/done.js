...
import { appendTelemetry } from './model-routing/telemetry.js';
import {
 ...
 */
function recordTelemetry(host, home, id) {
  try {
    const { file, written } = appendTelemetry(host, home, readTask(home, id));
...
export async function done(rootOrHost, opts = {}) {
  ...
  if (wasActive) recordTelemetry(host, home, id);
