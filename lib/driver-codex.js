...
import { KNOWN_TOOLS, versionLess } from '../tools.js';
...
export const PROVEN_CODEX_VERSION = KNOWN_TOOLS.codex.minVersion;
...
export const PHRASES = {
  sessions: 'потоки участника — ~/.agents/codex/sessions (уводится ATI_AGENTS_CODEX_HOME)',
...
function mcpConfig({ servers }, ref) {
  ...
  if (bus && ref) {
    ...
    if (process.env.ATI_AGENTS_CODEX_HOME) extra.ATI_AGENTS_CODEX_HOME = process.env.ATI_AGENTS_CODEX_HOME;
...
function optionRefusal(_options, tool) {
  if (tool?.version && versionLess(tool.version, PROVEN_CODEX_VERSION)) {
    return `найден codex ${tool.version}, а подъём участника проверен на ${PROVEN_CODEX_VERSION} и новее: `
      ...
      + `Обнови: ${KNOWN_TOOLS.codex.install}.`;
  }
  return null;
}
...
function shadowedUserServers() {
  ...
  // через транспорт закрывает не изоляция, а префикс ключа оверрайда (`BL-509`).
  return [];
}
...
export function stallRoute({ kind, address, repoAbs, task }, id) {
  ...
  const relift = () => (address?.startsWith('reviewer:')
    ? `поднимай reviewer'а заново: ati-agents promptobus review "${repoAbs ?? '<путь клона>'}" --harness codex${task ? ` --task ${task}` : ''}`
