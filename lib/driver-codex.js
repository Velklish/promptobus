...
/**
 * The lift line about workspace skills. A reviewer gets its own sentence rather than a
 * count: a Codex reviewer's working directory is the tree UNDER REVIEW — it has no
 * sandbox of its own, unlike Cursor's — and copying a skills canon into it would dirty
 * a foreign worktree and the branch a worker is still committing to.
 */
export function skillsNoteOf({ src, dest, count, role }) {
  if (role === 'reviewer') {
    return 'not attached — a Codex reviewer works in the tree under review, and the mechanism writes no files there';
  }
  if (!src) {
 ...
 * ALWAYS — trusted or not — so a copy into the worktree is enough. The self-ignoring
 * `.gitignore` beside it keeps the copy out of the worker's diff and lets `done` sweep
 * the worktree; it is written only when there is a copy to hide.
 */
function prepare({
  mcp, prompt, model, effort = null, permissionMode = null, addDirs = [], cwd, denyTools = null,
  ref, role = 'worker', task = null, address = null, root = null,
}) {
  ...
  const src = role === 'reviewer' ? null : workspaceSkillsDir(root);
  const dest = path.join(cwd, CODEX_SKILLS_REL);
  const files = src
    ? [
      { path: path.join(cwd, CODEX_DIR, '.gitignore'), text: '*\n', secret: false },
      { path: dest, copyFrom: src, text: '', secret: false },
    ]
    : [];
  return {
    ...
    cwd,
    files,
    skillsNote: skillsNoteOf({ src, dest, count: src ? skillsCount(src) : 0, role }),
    codexHome: home,
...
async function spawn(plan, {
  ...
  //
  // Trust is the worker's worktree and nothing else. It opens the project's own
  // `.codex/config.toml` and `.codex/agents`, which is what a person working in that
  // directory would get; a reviewer's working directory is the tree UNDER REVIEW, and
  // trusting it would let the repository being reviewed put MCP servers into the
  // session that judges it.
  let built;
  try {
    built = makeParticipantHome({
      ...
      config: codexHomeConfig({
        servers: mcp.servers,
        trusted: role === 'reviewer' ? [] : [trustPath(workdir)],
      }),
    });
  } catch (error) {
