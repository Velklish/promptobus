...
export const PHRASES = {
  ...
  stop: (id, doneCommand = 'promptobus done') => `stop the thread through the mechanism (${doneCommand} / another spawn after stop) — `
...
export function stallRoute({ kind, address, repoAbs, task, reviewCommand = null, doneCommand = null, statusCommand = null }, id) {
  const where = repoAbs ? `cd ${repoAbs} && ` : '';
  const review = reviewCommand ?? `promptobus review "${repoAbs ?? '<clone path>'}" --harness codex${task ? ` --task ${task}` : ''}`;
