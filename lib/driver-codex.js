...
const CODEX_DRY_RUN_COMMAND = `${CODEX} app-server --stdio (prompt via turn/start request)`;
 ...
 * MCP servers into the session judging it ([ADR-008](../docs/adr/adr-008-codex-reviewer-working-directory.md)).
...
// The warden switch and its trace, carried into the composed entry above by name.
//
// The entry's `env` is composed, not inherited: whatever the lifting command held in its
// own environment reaches the MCP server only if it is named here. Both halves of the
// auto-lift rule are such names — `PROMPTOBUS_WARDEN=off` is what prevents a lift,
// `PROMPTOBUS_WARDEN_TRACE` is what records one ([warden.js](warden.js)) — so without the
// forward a warden raised from a participant's MCP server was neither stopped by the
// switch nor visible to the suite's run-level gate (PB-166.2, measured on a live tree:
// the warden's parent was the participant's MCP server and its environment carried
// neither name).
//
// Forwarded only when set, so production behaviour does not move: a real run names
// neither variable, the loop copies nothing, and a live participant still gets the warden
// it wants. The value travels verbatim — this is the operator's switch, not the driver's
// opinion of it.
const WARDEN_ENV_VARS = ['PROMPTOBUS_WARDEN', 'PROMPTOBUS_WARDEN_TRACE'];
 ...
 * `done` sweep the worktree; it is written only when there is a copy to hide.
 *
...
function prepare({
  ...
  // The self-ignoring `.gitignore` is written for a reviewer whether or not there is a
  // canon to hide: a reviewer's working directory is the mechanism's to create, this file
  // is what brings it into existence before the lift, and the task store can itself sit
  // inside a git tree, whose diff the directory has no business entering. A worker's
  // worktree already exists, so a lift with nothing to copy writes nothing into it.
  const files = [];
  if (src || role === 'reviewer') {
    files.push({ path: path.join(workdir, CODEX_DIR, '.gitignore'), text: '*\n', secret: false });
  }
