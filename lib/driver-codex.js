...
import {
  ...
  listSessions, sessionsDir, turnWaitMs, turnIdleMs, turnFailed,
...
export const PHRASES = {
  ...
  mcpBoundary: '**Classified MCP write tools are placed in each mechanism-supplied server\'s `disabled_tools`; this configuration was accepted on codex-cli 0.146.0, but enforcement was not observed on a model turn. The owner\'s personal MCP set remains under the prompt layer, and every external MCP write is forbidden by this prompt regardless.**',
...
// is present. The owner personal MCP set is also lifted by app-server and remains under
// the review prompt layer; this mechanical boundary does not cover those entries.
...
async function spawn(plan, {
  ...
  const holder = startHolder(ref, sessionEnv(env ?? process.env));
