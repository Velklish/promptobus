...
// `config.toml`, because there is no personal set left to merge with. Hooks DO run —
// `PARTICIPANT_ARGV` in [codex-session.js](codex-session.js) says how. The end-of-turn
...
function saidLiftoff({ output, seen }) {
  ...
  info('hooks run for the participant: the lift carries --dangerously-bypass-hook-trust, so the hooks the '
    + "mechanism wrote into this home need no persisted trust; the owner's own Codex sessions still approve theirs");
}
