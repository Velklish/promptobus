...
/**
 * What follows `alarm: self-wake` — the PROGNOSIS, which is the half a reader cannot get
 * from the reason.
 *
 * Three branches of the round end at this label (`selfWake` in supervisor.ts), and what a
 * reader has to decide between them is WAITING or FIXING. Printing one word over all
 * three cost a live misreading — the orchestrator of the 2026-09-10 run read the start-up
 * label on its own address as a broken channel and went looking for the break.
 *
 * The three, and what is actually known about each:
 *
 * - `starting` — the participant has not handed over a contact point. Clears on the
 *   warden's first knock; verified from a run journal (PB-168);
 * - `taken` — a foreign session holds the contact point. Clears when the address's own
 *   session rewrites the record with its own, which the round expects at that session's
 *   next end of turn — and the suite has a standing check that delivery resumes after
 *   exactly that (`promptobus-warden.test.mjs`, "the owner rewrote the contact point").
 *   It does NOT clear if that session is gone for good, so the line says both halves
 *   rather than promising one. The card that filed this read the state as never clearing;
 *   the code and that check say otherwise, and the line follows the code;
 * - `refused` — the driver's channel did not accept the notification. The round retries,
 *   so this too clears if the channel comes back — but nothing here makes it come back,
 *   and this is the one of the three that asks a human to look.
 *
 * A suffix, not a new label: `self-wake` is the term the guides and the skills use, and
 * splitting it would leave them naming a state `status` no longer prints.
 *
 * Two fallbacks, in order. A record written before PB-168 has no `selfWake` — the field
 * is absent, not wrong, and the prognosis is then simply not said rather than guessed.
 * The exception is the one state the wake record itself shows: no contact point handed
 * over at all is the start-up state whoever wrote the health file, and it is also the
 * branch `wakePart` reaches when the warden has not yet run a round for this participant.
 */
// The suffix carries the PROGNOSIS and nothing the reason beside it already carries: the
// reason names WHAT happened, these name whether a reader is waiting or fixing.
...
function selfWakeSuffix(h, wake) {
  ...
  if (!state) return '';
  // Only the refusing branch has a channel to name, and only it is given one: the journal
  // says `socket` for Claude Code, `inject` for Cursor and `rpc` for Codex, and this line
  // said nothing at all.
...
function wakePart(home, id, addr) {
  ...
  if (addr === ORCHESTRATOR && orchestratorSocketGone(wake, h, taskOwner(home, id))) {
    ...
    // notification": the three fallbacks do not share one phrase, and a contact point
    // intercepted by a foreign session never reached a delivery channel at all. The
    // channel is named where it exists, by the state the round recorded, and this line
    // stays the only place the gate refusal is visible to a human without reading the
    // journal.
