...
// The socket ADDRESS may carry a `#<n>` suffix (Codex names a thread on one file); the path
// is what is before it. Checking the raw string calls every live Codex contact point gone.
...
function wakePart(home, id, addr, participant) {
  const parts = [];
  ...
  // The opposite state, and its own word: the mailbox is empty, the turn ENDED, and
  // nothing went out. 03-cli § Guard and warden says how it differs from SILENT and a stall.
  const owed = unansweredSince(home, id, addr, participant);
  if (owed) {
    parts.push(`${UNANSWERED_MARK} since ${owed} — the turn ended with nothing sent on the bus; `
      + 'the session is alive, it needs a nudge and not a respawn');
  }
  return parts;
}
...
export function status(rootOrHost, { task, sessions = undefined } = {}) {
  ...
  for (const meta of tasks) {
    ...
    for (const p of meta.participants ?? []) {
      ...
      try {
        parts.push(...wakePart(home, meta.id, addr, p));
      } catch {
