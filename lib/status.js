...
export function status(rootOrHost, { task, sessions = undefined } = {}) {
  ...
  for (const meta of tasks) {
    ...
    for (const p of meta.participants ?? []) {
      ...
      // The reviewer's diff file is a snapshot: the time and the worktree HEAD it was
      // taken from say how far behind the file is — the same pair the `task` reply prints.
      if (m.diffAt) parts.push(`diff snapshot ${m.diffAt}${m.diffHead ? ` at ${m.diffHead}` : ''}`);
