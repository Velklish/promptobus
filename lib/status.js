...
// занятость, живость, отметка доложенного — переехали в package (,
// [supervisor.ts](../src/supervisor.ts)): печать `promptobus status`,
...
// стопу приходит строкой от driver'а того участника, чьё состояние и разобрано.
...
export function wardenLine(home, id, host = null) {
  const lifter = liftDriver();
  const mark = liveWarden(home, id);
  const lift = host?.busCommand?.(['warden', `--task ${id}`]) ?? `promptobus warden --task ${id}`;
  if (!mark) {
    return {
      alive: false,
      line: `${WARDEN_MARK}: слушателя mailbox'ов задачи нет — участник узнает о сообщении, `
        ...
        + `задачи, терять при перезапуске нечего. Поднять руками: ${lift}`,
    };
  }
  return {
    alive: true,
...
function wakePart(home, id, addr) {
  ...
  if (addr === ORCHESTRATOR && orchestratorSocketGone(wake, h, taskOwner(home, id))) {
    ...
    // откатов теперь два, и второй — перехваченный чужой сессией contact point,
...
export function status(rootOrHost, { task, sessions = undefined } = {}) {
  ...
  const home = promptobusHome(host.workspaceRoot());
  ...
  for (const meta of tasks) {
    ...
    // набор. Поданный снимок — на ВЕСЬ вызов, а не на задачу: `status` без
    ...
    for (const p of meta.participants ?? []) {
      ...
      if (m.name) {
        ...
        // «нет в списке» значило бы звать поднимать её заново.
          ...
          // Штатный конец хода — обычное состояние участника между ходами, и
