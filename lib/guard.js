...
async function decide(args, env, cwd, event) {
  ...
  // SessionStart in the root is only the successor detector: a full guard at start would
  // burn GUARD_BLOCK_LIMIT, since the session id survives resume and clear.
  if (event.hook_event_name === GUARD_START_EVENT) {
  ...
  // A foreign mailbox — nothing to guard: "fetch the mailbox" would be a lie. In the root
  // this is the successor after an id change: a hint, not a return and not an auto-claim.
