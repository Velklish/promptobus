...
import { GUARD_START_EVENT } from '../guardhook.js';
...
async function decide(args, env, cwd, event) {
  ...
  // Идентичность участника — из АРГУМЕНТОВ команды хука, и это ПЕРВЫЙ источник (`BL-469`).
  ...
  const identity = resolveIdentity(env, here, { move: false, declared: args });
  if (storePending(identity.home)) return null;
  ...
  // записи (`BL-466`): где лежат адрес сокета и токен, знает только harness, а имени
  ...
  // Адрес закреплён в журнале за ДРУГОЙ сессией — эта за него не пишет ничего (`BL-469`):
  ...
  ensureWarden(identity.home, task, { env });
  ...
  if (!verdict) {
    ...
    // (`BL-418`). Ставится она там, где ход ЗАКАНЧИВАЕТСЯ: возврат хода (код 2) ниже —
