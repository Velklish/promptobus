...
import {
  ...
  bus, identityLabel, lastTurnAt, markTurn, nameOfArtifact, ORCHESTRATOR, outboundMessages,
  ownership, participantOf, readHealth, readTask,
...
import { isGateRecordName } from './handoff.js';
...
function handoffPendingVerdict(home, task, addr) {
  ...
  let sentGateArtifact = false;
  for (const msg of outboundMessages(home, task, senderId, since)) {
    if (msg.type === 'result') return null;
    if (!sentGateArtifact && msg.type === 'artifact' && msg.artifact) {
      const name = nameOfArtifact(home, task, msg.artifact);
      if (isGateRecordName(name)) sentGateArtifact = true;
    }
  }
  if (!sentGateArtifact) return null;
  return {
    key: `${HANDOFF_KEY}:${since}`,
    reason: 'a gate-record artifact was sent and the turn is ending with no result: send the result '
...
// Two states, and the order is the repair: unread mail first, a debt only after it.
export function guardVerdict(home, task, addr) {
  ...
  return handoffPendingVerdict(home, task, addr);
}
