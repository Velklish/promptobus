...
import {
  ...
  bus, identityLabel, markTurn, ORCHESTRATOR, ownership, participantOf, readHealth, readTask,
...
// Everything unparsed, or still open after the deadline, is an empty payload.
...
// nowhere else: a debt field on disk is out of scope (PB-207).
...
//
// Two states, and the order is the repair: unread mail is read first, and only a
// participant with nothing left to read can be said to owe an answer.
export function guardVerdict(home, task, addr) {
  ...
  // Who owes an answer at all — the table's door, not the guard's: the orchestrator is
  // outside it by decision, and `answerOwedSince` carries that decision and its edge.
...
// A hook parent may write its payload before it closes stdin; give it a second.
...
// Address prefixes of a NON-orchestrator participant. The list is spelled here rather
// than imported: the guard reads a role out of its own arguments, before any address
// grammar is asked, and a closed list missing a role makes that session a successor
// candidate in the root instead of the participant it is.
...
// the successor.
export async function successorHint(identity, cwd, session, event, probe = probeContactPoint) {
  const tasks = activeTasks(identity.home);
  if (sessionOnBus(identity.home, session, identity, tasks)) return null;
  const items = await deadOwnerItems(identity.home, cwd, session, tasks, probe);
