...
const STORE_COMMANDS = new Set(['spawn', 'review', 'status', 'send', 'done', 'dismiss', 'history', 'prune']);
...
Help: ${cmd} help (same as no command, --help, -h) · version: ${cmd} --version (-v)
  ...
  ${cmd} send <address> --body <text> [--type <type>] [--task <id>] [--artifact <file>]
                              write one bus message FROM THIS PROCESS'S OWN ADDRESS —
                              PROMPTOBUS_ROLE if it carries one, orchestrator otherwise.
                              There is no --from: a sender that can be chosen is a sender
                              that can be borrowed, and the barrier for that does not exist
                              yet (ADR-010). Type defaults to status
...
export async function runPromptobus(argv, { host, cwd, env, input, output } = {}) {
  ...
  // import the driver registry (02-host.md § Session identity, ADR-009).
