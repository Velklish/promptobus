...
const SUBCOMMANDS = 'spawn, review, models, status, send, done, dismiss, history, prune, guard, warden, mcp, install, uninstall';
...
const STORE_COMMANDS = new Set(['spawn', 'review', 'status', 'done', 'dismiss', 'history', 'prune']);
...
Help: ${cmd} help (same as no command, --help, -h) · version: ${cmd} --version (-v)
  ...
  ${cmd} done [--task <id>] [--keep-sessions]
                              ...
                              ${cmd} prune)
  ${cmd} send <address> --body <text> [--type <type>] [--task <id>] [--artifact <file>]
                              write one bus message from the address this process can
                              PROVE. A declared PROMPTOBUS_ROLE must be a participant of
                              the task and not held by another session; the orchestrator
                              address is given only to the session that owns the task, and
                              a task with no owner is refused rather than defaulted to.
                              There is no --from: a sender that can be chosen is a sender
                              that can be borrowed (ADR-011). Type defaults to status
...
export async function runPromptobus(argv, { host, cwd, env, input, output } = {}) {
  ...
  try {
    ...
    switch (cmd) {
      ...
      case 'send': {
        const { values, positionals } = parse(at('send'), rest, {
          options: {
            task: { type: 'string' },
            type: { type: 'string' },
            body: { type: 'string' },
        ...
        return (await import('./send.js')).send(host, {
          to: positionals[0],
          task: values.task,
          type: values.type,
          body: values.body,
