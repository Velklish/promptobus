// Loading the catalog: the shipped file, the layer stack above it, and the merge below.
// What each layer may state and where it comes from: [guides/model-routing.md#mergeweights------the-merge-itself---------------------------------------------------------](../../docs/guides/model-routing.md#mergeweights------the-merge-itself---------------------------------------------------------)
...
/**
 * Overlay keys whose VALUES are on the rating scale, and which therefore cannot
 * be read from a version 1 file.
 *
 * ADR-005 refuses a v1 overlay carrying any of them. `ratings` is the obvious
 * one; the two floor keys are the same kind of value and were the easy miss —
 * `reviewerQualityFloor: 5` means "the top band of five" in a file written for
 * ADR-004 and "half way up" under ADR-005, so reading it unchanged would LOWER
 * a person's reviewer floor from the new default of 9 to 5 silently, which is
 * the one direction a floor must never move by itself. A value written on a
 * scale that has been replaced is indistinguishable from one written on the new
 * one, so the version is the only thing that can tell them apart.
 *
 * Every other v1 key — deny, allow, defaults, account, weights, penalties,
 * bonuses, priority, payg — is unchanged in meaning and keeps working, which is
 * what stops this from being a migration everybody has to do.
 */
...
export const DEFAULT_POLICY = Object.freeze({
  ...
  /**
   * How many participants may be live on one harness at once, per harness, and
   * empty by default — an empty block is today's behaviour word for word.
   *
   * A ceiling rather than a steeper penalty because the three subscriptions have
   * different capacities and a penalty only ORDERS candidates: under `balance` the
   * harness is chosen by `pace.effective` inside the band, so a harness whose
   * window is ahead of the others attracts the third and the fourth worker too
   * however expensive the `live-participant` adjustment makes it. Measured
   * 2026-09-06 on the consumer: three workers in a row went to one Codex account,
   * the five-hour window was spent in forty minutes and all three stalled
   * mid-turn.
   */
  caps: Object.freeze({ liveParticipants: Object.freeze({}) }),
  /** ADR-005's defaults on the 1–10 scale. Both remain soft choice rules. */
  qualityFloor: Object.freeze({ worker: 5, reviewer: 9 }),
  /**
   ...
   * `quotaCost` of 10 gives up exactly one band against a `quotaCost` of 1: the
...
// The merge. Four rules — weights replaced, deny unions, allow intersects, the rest per
// field — and provenance as a list of rules: [guides/model-routing.md#mergeweights------the-merge-itself---------------------------------------------------------](../../docs/guides/model-routing.md#mergeweights------the-merge-itself---------------------------------------------------------)
...
/**
 * `caps`, per harness and per cap kind, merged harness by harness: the highest
 * layer that names a harness wins for THAT harness and leaves the others alone.
 * Flat like `penalties` one level down, because each value is a scalar and a
 * half-replaced block of them would say nothing a whole one does not.
 *
 * No layer is recorded: unlike `defaults` and `account`, nothing prints where a
 * cap came from — the decision names the cap that bounded it and the overlay a
 * person opens is the one they wrote.
 */
