...
export const DECISION_WARNINGS = [
  ...
  'flag-not-in-inventory', 'worker-floor-not-met', 'balance-fallback',
];
...
export function resolve({
  ...
  if (scored.length) {
    if (strategy !== 'balance') chosen = bestOf(scored);
      ...
      // The pick, in ADR-004's order. Each harness is represented by the tuple
      // that would actually be picked on it — its best ELIGIBLE candidate by the
      // role's ordering, with the floor applied there rather than after, so the
      // pace compared is the pace of the tuple the comparison is about.
      ...
      // renderer would otherwise have to repeat this rule — best eligible tuple
      // meeting the role's floor — and a second copy of it would show a
      // different representative on a run where the floor moved the first.
