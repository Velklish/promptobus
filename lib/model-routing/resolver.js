  ...
  if (scored.length) {
    if (strategy !== 'balance') chosen = bestOf(scored);
      ...
      if (!representatives.length) {
        ...
        // in that order.
        const leader = Math.max(...balanceRepresentatives.map((r) => r.pace.effective));
        const tied = balanceRepresentatives.filter((r) => r.pace.effective >= leader - band);
      ...
      for (const { harness, row } of short) {
        ...
        fromSnapshot.push({
          ...
          message: `${harness} is running short — its ${pace.window.kind} window "${pace.window.id}" `
            ...
            + `\`${strategyCommands[proposed] ?? `promptobus models strategy --set ${proposed}`}\` once they agree — nothing switches on its own`,
        });
      }
    }
  }
