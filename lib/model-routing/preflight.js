// Preflight: the adapters of the declared harnesses, run together under ONE budget.
// What it is for, the budget rule and the cache seam: [guides/model-routing.md#preflight-running-the-adapters-together-under-one-budget](../../docs/guides/model-routing.md#preflight-running-the-adapters-together-under-one-budget)

import { AVAILABILITY_REASONS, AVAILABILITY_SOURCES, AVAILABILITY_STATES } from '../../dist/index.js';
import { warn } from '../util.js';
import {
  NEVER_CHECKED, SNAPSHOT_VERSION, cacheFileOf, heldOf, isTimestamp, isoStamp, readSnapshot,
  snapshotEntry, staleVersionAt, writeEntries,
} from './cache.js';
 ...
 * of the eight codes to a state, and none of them accompanies `available`. So a
...
// The shared budget: one deadline beside the adapters, not one each.
// What a timeout leaves behind: [guides/model-routing.md#resolvebins--resolve-the-binary-of-every-harness-about-to-be-probed-before-any-adapter](../../docs/guides/model-routing.md#resolvebins--resolve-the-binary-of-every-harness-about-to-be-probed-before-any-adapter)
...
export async function preflight({
  ...
  // Asked only when the cache came back empty, so the ordinary run reads the file
  // once. A version this build does not read is a discard, and the person meets a
  // discard where they would otherwise read "no cache entry" and look for a bug.
  ...
  if (Object.keys(probed).length) {
    try {
      const { dropped = [] } = writeEntries(host, probed, { dryRun }) ?? {};
      if (dropped.length) {
