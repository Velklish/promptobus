...
function resolveBins({ host, harnesses, adapters, deadline }) {
  ...
  for (const harness of harnesses) {
    ...
    // Version is read here, not in resolveToolBin: that member is synchronous on every caller.
    // Only a tool that declares readsVersion. A version already filled is not asked again.
    const declared = adapters.get(harness)?.readsVersion === true;
    const unread = typeof found?.version !== 'string';
    if (found?.ok && found.bin && declared && unread
      && typeof host?.readToolVersion === 'function') {
      try {
        const version = host.readToolVersion(tool, found.bin);
        if (typeof version === 'string' && version.trim()) found = { ...found, version: version.trim() };
      } catch { /* an unreadable version stays absent */ }
    }
