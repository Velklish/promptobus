...
/** The credential record on macOS: one keychain generic password. `/usr/bin/security` by ABSOLUTE
 * path — a `security` off `PATH` is a poor thing to hand a request for a token. `-w` prints it. */
...
function runCapture(bin, argv, ms) {
  ...
  return new Promise((resolve) => {
    ...
    timer = setTimeout(() => {
      timedOut = true;
      // `SIGKILL`, not `SIGTERM`: the budget is already spent, and a child that ignores a polite
      // signal would spend the rest of the run's ceiling on top of it.
      child.kill('SIGKILL');
