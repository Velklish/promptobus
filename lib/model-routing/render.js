// Turning a decision into the lines a person reads on spawn and review.
// What each line must carry and why: [guides/model-routing.md#rendering-a-decision-for-a-person](../../docs/guides/model-routing.md#rendering-a-decision-for-a-person)
...
/**
 * A pace number as the terminal shows it: two decimals and always a sign.
 *
 * Two decimals because the resolver rounds to two and one would print an
 * underspend of −0.02 as "-0.0", which reads as zero and is the one value where
 * the sign is the whole message. Always a sign because these are differences,
 * not levels: "+16.50" is behind pace with room to spend, "-14.08" is ahead of
 * it, and a bare number leaves the reader to work out which.
 */
const points = (n) => `${n >= 0 ? '+' : ''}${n.toFixed(2)}`;

// Which lines a decision must always print, whatever the strategy chose.
// The list and why each is there: [guides/model-routing.md#pacelines--the-pace-table-one-row-per-eligible-harnesspool-representative-under-the](../../docs/guides/model-routing.md#pacelines--the-pace-table-one-row-per-eligible-harnesspool-representative-under-the)
function paceLines(decision) {
  ...
  // A no-pace tuple outside a pool must not add a contradictory note when
  // another pool of the same harness has a binding window. Keep one note only
  // for a harness whose scored tuples have no window at all.
  ...
  for (const { harness, rows } of groups.values()) {
    ...
    lines.push(`${head}${poolText}${rep.tupleId} · ${pace.window.id} ${pace.window.kind} · `
      ...
      // The number is still printed for a harness at its cap: the row says what
      // the account has room for, and the mark says it is out of the comparison
      // anyway. A row silently missing its effective number would read as an
      // unpaceable window, which is a different fact.
