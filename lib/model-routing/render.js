...
/**
 * The pace table: one row per harness, under the candidates (ADR-004).
 *
 * Per HARNESS and not per candidate, because that is the comparison `balance`
 * actually makes — each harness is represented by the tuple that would be picked
 * on it, and the largest `effective` leads.
 *
 ...
 * the tuple that was actually picked. A harness with no eligible candidate has
...
function paceLines(decision) {
  ...
  const harnesses = [...new Set(scored.map((c) => c.harness))];
  const nameWidth = widest(harnesses) + 2;
  for (const harness of harnesses) {
    const rows = scored.filter((c) => c.harness === harness);
    ...
    if (!pace.eligible) {
      ...
      lines.push(`${head}${pace.note}: ${why}`);
      continue;
    }
    lines.push(`${head}${rep.tupleId} · ${pace.window.id} ${pace.window.kind} · `
