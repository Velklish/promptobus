// Participant telemetry: one JSON Lines record per routed participant, appended at `done`.
// Why, the closed shape, and why no lock: [guides/model-routing.md#participant-telemetry-the-collecting-half](../../docs/guides/model-routing.md#participant-telemetry-the-collecting-half)
...
import {
  addrDir, addressOf, dismissedOf, isAddress, routingOf, startedOf, taskDir,
} from '../../dist/index.js';
import {
...
/**
 * A participant's throughput observation in the closed telemetry shape, or `null`.
 *
 * A producer may supply output tokens with model-active generation duration, or a
 * harness-provided tokens/second value. The aliases here are the event spellings
 * accepted at the producer seam; the record itself always carries camel-case names.
 * A partial event is retained as partial evidence, but `throughputRate` declines it
 * until it has a usable rate. Missing evidence is `null`, never zero.
 */
...
/**
 * Append one normalized, per-turn observation without taking the task lock.
 *
 * Cursor's stop fixture (`test/harness-cursor.mjs:1043-1055`) carries
 * `generation_id` and `loop_count` with `output_tokens`; this append is therefore
 * one turn's observation, not a completion-duration reading. A refusal to write
 * is left to the caller, which keeps the guard's turn-end path non-blocking.
 */
...
/**
 * Project one participant's sidecar into run totals and a direct-rate mean.
 *
 * Output tokens are summed across valid per-turn observations. Pair tokens and
 * model-active duration are summed only when the same observation carries both;
 * when that pair yields a rate, it wins. Otherwise direct rates are combined as
 * a token-weighted mean rather than taking the last turn's value; a missing token
 * count has weight one.
 */
 ...
 *
 * Read from the current preflight snapshot (or the cache when called directly),
 * and only while its window reading is within the window TTL. This deliberately
 * does not use `entryLive`: an exhaustion stays live for routing until its reset,
 * or forever when sticky, but that held fact is not a fresh percentage for a
 * delta. A stale entry gives `null` for every window: a delta measured against
 * an hour-old percentage is not a delta, and PB-37 is told so rather than
 * handed a number to divide.
 */
...
/**
 * Which spawn-window readings are missing, and whether the reason is harness-wide.
 *
 * A fresh entry with partial windows reports each missing id separately. An
 * absent, stale, or held entry cannot distinguish its windows, so its reason is
 * reported once for the harness. A fresh entry with a non-null failure reason
 * is likewise harness-wide when it has no usable reading at all.
 */
 ...
 *
 * **Only the envelope is read.** `sender`, `recipients`, `type` and a result's
 * `ts` are counted; `body` is opened by `JSON.parse` and never leaves this
 * function, which is the property the privacy check in the suite exists to hold.
 */
...
/**
 * The moment a participant's own run ended: its last result, its dismissal, or
 * the close of the task.
 *
