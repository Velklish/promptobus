...
/** Whether an entry's window readings are fresh enough to measure a delta. */
function telemetryEntryFresh(entry, at) {
  if (!Array.isArray(entry?.windows) || !entry.windows.length) return false;
 ...
 *
 * Read from the cache as it stands now, and only while its window reading is
 * within the window TTL. This deliberately does not use `entryLive`: an
 * exhaustion stays live for routing until its reset, or forever when sticky,
 * but that held fact is not a fresh percentage for a delta. A stale entry gives
