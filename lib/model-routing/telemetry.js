 ...
 */
export function telemetryRecords(host, home, meta, { at = Date.now(), snapshot = null } = {}) {
 ...
 */
export function appendTelemetry(host, home, meta, { at = Date.now(), snapshot = null } = {}) {
  const file = telemetryFileOf(host);
  const rows = telemetryRecords(host, home, meta, { at, snapshot });
