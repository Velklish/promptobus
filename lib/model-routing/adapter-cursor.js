 ...
 *
 * Two ways an id joins the pool, and this is the "say which" PB-27 asks for: the
 * id **is** a bucket name, or it starts with a bucket name followed by a hyphen.
 * The hyphen is the whole of the second rule — without it `vega` would claim
 * `vegabond-3` — and a bucket name that matches no id contributes nothing rather
 * than being carried as a guess.
 *
 ...
 */
export function autoPoolModels(inventory, bucket) {
  ...
  for (const id of inventory ?? []) {
    ...
    if (exact.has(id) || names.some((name) => id.startsWith(`${name}-`))) ids.push(id);
  }
  return [...new Set(ids)];
}
 ...
 * made.
 */
export function periodWindows(usage, inventory) {
  ...
  const models = autoPoolModels(inventory, usage?.autoBucketModels);
...
async function limitVerdict({ deadline, timeoutMs, deps, carry, inventory }) {
  ...
  const windows = periodWindows(usage.doc, inventory);
  if (!windows.length) {
 ...
 * harness binary: the token read, the backend URL and the two POSTs. It defaults
