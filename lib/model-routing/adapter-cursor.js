...
function noAnswer(which, answer, timeoutMs, carry) {
  ...
  if (answer.timedOut) {
    return verdict('unknown', 'probe_timeout',
      `cursor ${which} did not answer within the ${timeoutMs} ms preflight budget`, carry);
  }
 ...
 *
 * `timeoutMs` is the WHOLE preflight budget and both calls share it: what is left
 * of it is the ceiling of the next one. A binary slower than that is a timeout and
 ...
 * consulted.
 */
async function probe(tool, { host, timeoutMs }) {
  // The deadline is taken BEFORE the binary is resolved, because resolving is
  // part of the probe: a host is free to run `--version` inside `resolveToolBin`
  // — this package's own Cursor driver says it does, with a ceiling of its own —
  // and a deadline started afterwards would let one adapter spend that ceiling
  // plus the whole budget.
  const deadline = Date.now() + Math.max(0, timeoutMs);
  const left = () => deadline - Date.now();
  const found = host.resolveToolBin(tool);
 ...
 * and a second copy of it is a drift waiting to happen.
 */
export function cursorAvailability(tool) {
  return { probe: (request) => probe(tool, request) };
}
