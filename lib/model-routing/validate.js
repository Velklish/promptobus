...
import {
  CATALOG_FILE, CATALOG_SCHEMA_VERSION, DEFAULTS as DEFAULTS_LAYER, OVERLAY_SCHEMA_VERSION,
  RULE_ROLES, SELECTOR_KINDS, WEIGHT_STRATEGIES, mergeRouting, rulePath, rulesFor, rulesForRole, rulesLabel,
} from './catalog.js';
...
const OVERLAY_KEYS = [
  ...
  'reviewerQualityFloor', 'qualityFloor', 'balance', 'allow', 'deny', 'ratings', 'priority', 'payg',
];
...
const CITATION_KEYS = ['rating', 'basis', 'version', 'agentHarness', 'provenance', 'figure', 'fieldSize', 'url', 'date'];
const CITATION_REQUIRED = ['rating', 'basis', 'figure', 'fieldSize', 'url', 'date'];
...
function checkCitation(citation, at, push) {
  ...
  if (citation.fieldSize !== undefined && !(Number.isInteger(citation.fieldSize) && citation.fieldSize >= 1)) {
    push(`${at}.fieldSize`, 'expected an integer of one or more — the size of the field the band was cut from');
  }
  ...
  if (doc.qualityFloor !== undefined) {
    if (!isObject(doc.qualityFloor)) push('qualityFloor', 'expected an object keyed by role');
      ...
      for (const role of RULE_ROLES) {
        ...
        if (!isRating(doc.qualityFloor[role])) push(`qualityFloor.${role}`, 'expected an integer from 1 to 5');
      }
    }
  }
