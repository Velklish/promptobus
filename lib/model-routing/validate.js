    ...
    if (!isObject(doc.caps)) push('caps', 'expected an object');
      ...
      for (const kind of CAP_KINDS) {
        ...
        for (const [harness, value] of Object.entries(block)) {
          ...
          // Zero is lawful and is the point of the minimum being zero rather than
          // one: it is a person saying "never this harness", and a check that
          // demanded a positive number would refuse the clearest thing the key
          // can say.
...
export function validateLayers({ canonical, overlays = [], constraints = null, now = Date.now() } = {}) {
  ...
  for (const layer of sound) {
    ...
    // A ceiling on a harness this CLI does not drive, and an error for the same
    // reason: a misspelt name is a cap that bounds nothing, and a person who
    // wrote one believes their run is held to two participants when it is not.
    for (const kind of CAP_KINDS) {
