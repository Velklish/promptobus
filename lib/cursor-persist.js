...
/** Tmux call on the named server. `-f /dev/null`, the same flag `persist` uses: a person's
 * `~/.tmux.conf` would otherwise change what the mechanism pastes and reads. */
...
/** Server sessions, machine-readable. An empty list for a missing server is not a refusal: a tmux
 * server lives while it has sessions, so "no server running" means "there are no sessions". */
...
export function findSession(name, { server = CURSOR_TMUX_SERVER, env = process.env } = {}) {
  if (!name) return null;
  return tmuxSessions({ server, env }).find((s) => s.name === name) ?? null;
}
...
export async function liftSession({
  ...
  if (seen) {
    // Mark of our session, set BEFORE the pane is stopped: a stopped pane does not touch the
    // session, and an unmarked session on the shared server is indistinguishable from a foreign one.
