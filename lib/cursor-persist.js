...
import { AGENTS_BIN, shellQuote } from '../util.js';
...
//
// **Что такое `agent persist`.** Обёртка над tmux (спайк `BL-478`, REPORT §2): подкоманда
...
//
// **Дом реестра — `~/.agents/cursor`, а не `~/.cursor`.** Второе — дом человека, и писать
// туда сверх того, что кладёт сам Cursor, механизм не вправе; `~/.agents` он уже держит
// (модули, hooks памяти — [install.js](../install.js)). Переменная `ATI_AGENTS_CURSOR_HOME`
// уводит его целиком: ею набор ставит реестр в песочницу, иначе прогон писал бы в дом
// разработчика. Дом самого Cursor уводится своей — `ATI_AGENTS_CURSOR_USER_HOME`, и уводит
// её тоже только набор: настоящие стенограммы лежат в `~/.cursor/projects`.
//
// Все числа и формы ниже сняты живыми прогонами спайка `BL-478` (2026-09-03, `agent`
 ...
 * после старта.
 */
export const LAUNCH_TMUX_SERVER = 'ati-agents-launch';
 ...
 * что было у watchdog'а по потоку (`BL-468`): 180 с — вшестеро больше самого долгого
...
export function cursorStateHome(env = process.env) {
  const named = String(env.ATI_AGENTS_CURSOR_HOME ?? '').trim();
  return named || path.join(homedir(), '.agents', 'cursor');
}
...
export function cursorUserHome(env = process.env) {
  const named = String(env.ATI_AGENTS_CURSOR_USER_HOME ?? '').trim();
 ...
 */
export const BUS_MCP_NEEDLE = `${AGENTS_BIN} promptobus mcp`;
 ...
 * шины — длинная игла (абсолютный `AGENTS_BIN` + ` promptobus mcp`). Замер на живой сессии
 * этой машины (2026-09-03, процессы MCP рабочего места): строка `promptobus mcp` — 159
 * знаков и с `-Ao`, и с `-Awwo` (короткий путь пакета в `node_modules`). У CLI из worktree
 * `AGENTS_BIN` сам 161 знак, `node …/agents.js promptobus mcp` — 218; без `ww` игла
 * обрежется, сервер шины сочтится инструментом, стопа не будет. Тот же файл для окружения
 * уже берёт `ps eww`.
 */
