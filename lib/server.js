...
// Adapter MCP-сервера шины над Promptobus (ADR-032): подкоманда `promptobus mcp`,
...
function joinBus({ home, task, address, gated, host = null }) {
...
export async function serve({ host = null, env = process.env, cwd = process.cwd(), input = process.stdin, output = process.stdout } = {}) {
  const server = createMcpServer({
    ...
    serverInfo: () => ({ name: PROMPTOBUS_SERVER, version: host?.version ?? '0.0.0' }),
