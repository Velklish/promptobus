...
import { createMcpServer } from '../../packages/promptobus/dist/index.js';
...
// Adapter MCP-сервера шины над Promptobus (ADR-032, BL-407): подкоманда `ati-agents promptobus mcp`,
...
// [mcp/server.ts](../../packages/promptobus/src/mcp/server.ts), и о рабочем месте не знает
...
// посторонняя строка в нём ломает клиента-агента так же, как ломала набор (`BL-258`).
//
// Метка машинного адреса приезжает из package вместе с рендером ответов; дверь к ней
// остаётся здесь — её берут фикстуры хука ленты ([bushook.js](../bushook.js)).
export { ADDR_MARK } from '../../packages/promptobus/dist/index.js';
...
function joinBus({ home, task, address, gated }) {
  // Contact point сдаёт driver этого участника (`BL-466`): переменные с адресом сокета и
...
// (ADR-032, §2). Отказ инструмента приходит текстом с `isError`, а не ошибкой протокола:
...
export async function serve({ env = process.env, cwd = process.cwd(), input = process.stdin, output = process.stdout } = {}) {
  const server = createMcpServer({
    ...
    resolveIdentity: () => resolveIdentity(env, cwd),
    // Имя — из своего дома (`contract.js`): им же зовётся запись сервера в конфигах.
    serverInfo: () => ({ name: PROMPTOBUS_SERVER, version: cliVersion() }),
    onJoin: joinBus,
    decorateParticipant,
