 ...
 * written and the worktree HEAD it was taken from. The pair answers one question — how
 * far behind the file is — for `promptobus status` and `promptobus_task`, without
 * opening the file and without asking the reviewer.
 *
 ...
 * this diff was never taken from.
 */
export function stampSnapshot(home, id, address, { at, head = null }) {
  return withTaskLock(home, id, () => {
    const meta = readTask(home, id);
    const p = participantOf(meta, address);
    if (!p) return { found: false };
    const { diffHead, ...rest } = p.metadata;
    gate(() => bus(home).patchParticipant(id, p.id, {
      metadata: { ...rest, diffAt: at, ...(head ? { diffHead: head } : {}) },
    }));
    taskCache?.clear();
    return { found: true };
  });
