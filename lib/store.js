...
export function resolveTaskId(home, declared, session = sessionIdentity(), hints = {}) {
  const spawnRepo = hints.spawnRepo ?? 'promptobus spawn --repo <name> --brief <file>';
  const spawnNewTask = hints.spawnNewTask ?? 'promptobus spawn --new-task';
  if (declared) {
...
export function sendMessage(home, id, { from, to, type, body, artifactPath }, hints = {}) {
  ...
  if (!known.includes(to)) {
    throw new Error(`task ${id} has no participant "${to}" — nobody can fetch the message, `
      + `and a mailbox opened for them will be seen by neither ${hints.status ?? 'promptobus status'} nor task. Task participants: ${known.join(', ')}`);
  }
