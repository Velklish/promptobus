...
export function sessionIdentity(env = process.env, scope = null) {
  ...
  if (answer?.candidates?.length > 1) warnOnce(`session identity: ${answer.why}`);
...
function requireDirectSender(meta, sender, session) {
...
export function sendMessage(home, id, {
  ...
  if (sender && acceptanceRoute(sender, recipient)) {
    requireDirectSender(meta, sender, session);
  }
