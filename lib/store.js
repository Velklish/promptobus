...
import { findRoot, warn, writeFileAtomic } from '../util.js';
import {
  addrDir, addressOf, bindingNames, brokenNote, dropBinding, FOREIGN_MARK,
  FOREIGN_ROUTE,
  foreignSessionOf,
  GateError, isAddress, LEGACY_REL, MECHANISM_VERSION_FIELD, MESSAGE_TYPES, migrate,
...
// Adapter шины над Promptobus (ADR-032, `BL-406`, `BL-430`). Ядро — задачи, участники,
// mailbox'ы, артефакты, восстановимый fan-out, история, файлы каталога задачи и MCP-factory
// — живёт во вложенном package `@ati-agents/promptobus` и о рабочем месте не знает ничего.
//
// Здесь остаётся то, что package'у знать и не положено: поиск корня workspace, путь к store
// внутри него, идентичность сессии Claude Code, человеческая диагностика, routing policy
// ATI и грамматика адресов механизма.
//
// **Слоя совместимости больше нет** (`BL-430`). У package одна поверхность — v1, и
...
//
// **Импорт идёт путём до `dist`, а не именем**: вложенный package не workspace и не
// зависимость CLI, поэтому bare-specifier `@ati-agents/promptobus` резолверу взять неоткуда
// ([reference/10](../../../docs/reference/10-validation.md)). Отсюда предусловие: из
// чекаута CLI работает только после сборки — `npm install` и `npm run build --prefix cli`.
// Пользователя tarball это не касается: `dist` едет в пакете.
//
...
export {
  ...
  // версия прежнего CLI в команде закрытия: её печатает отказ миграции
  LEGACY_DONE,
...
// Корень store шины внутри рабочего места. С `BL-410` это `.promptobus`: переписка,
// участники и артефакты лежат в protocol v1 ([reference/14](../../../docs/reference/14-promptobus-v1.md)).
// Отсюда же имя берут переменные окружения `ATI_AGENTS_PROMPTOBUS_HOME` и `PROMPTOBUS_HOME`.
...
export function promptobusHome(root) {
  ensureStore(root);
 ...
 *
 * Опознаются два хвоста — новый и прежний: `<root>/.promptobus` и `<root>/.agents/a2a`.
 * Опознан — переезд запускается тем же ходом, что и у команды, а устаревший `PROMPTOBUS_HOME`
 * несинхронизированного `.mcp.json` резолвится в НОВЫЙ корень, а не воссоздаёт старый
 * каталог рядом с переехавшим. Не опознан — каталог берётся как есть и переезд по нему не
 * запускается: домом бывает произвольный каталог (так его задаёт набор), и придумывать ему
 * корень нельзя.
 */
function rootOfHome(home) {
  const abs = path.resolve(home);
  if (path.basename(abs) === PROMPTOBUS_REL) return path.dirname(abs);
  const [outer, inner] = LEGACY_REL.split(path.sep);
  const parent = path.dirname(abs);
  if (path.basename(abs) === inner && path.basename(parent) === outer) return path.dirname(parent);
  return null;
}

// Миграция `.agents/a2a` → `.promptobus` при ПЕРВОМ обращении (ADR-032, §7). Дверей в store
// две, и обе ведут сюда: `promptobusHome(root)` — у команд, ищущих корень от cwd, и
...
function ensureStore(root) {
  ...
  const report = migrate(root, { session: sessionIdentity(), harness: FALLBACK_HARNESS });
  migrated.add(root);
...
// задаёт её потребитель: core ролей не знает и берёт их из записей участников (ADR-032, §6).
//
// Правило ATI: переписываются только с оркестратором задачи. Worker'ы и reviewer'ы между
 ...
 * стоять вовсе.
 */
export function bus(home) {
  const hit = engines.get(home);
  if (hit) return hit;
  // Версия механизма называется при открытии — оба пути, команда CLI и MCP-сервер шины,
  // идут через эту дверь, поэтому смесь версий они читают одинаково (`BL-486`).
  const engine = openEngine({ home, policy: atiRouting, cli: cliVersion() });
 ...
 * верхний catch `agents.js` печатает со стеком, и законный отказ читается как поломка
...
export function participantRecord(address, fields = {}) {
  ...
  return {
    ...
    // Версия механизма, сделавшего запись (`BL-486`). Она и есть свидетельство смеси версий:
    ...
    metadata: { ...fields, address, [MECHANISM_VERSION_FIELD]: cliVersion() },
  };
}
...
/**
 * КАТАЛОГИ участника в `workers/` — те, что driver завёл рядом с его файлами (`BL-468`).
...
// Активная задача процесса. Три источника, по убыванию силы (ADR-027): явное объявление
...
// задач разом, — поэтому бросаются `GateError`'ом: голый `Error` верхний catch `agents.js`
...
export function resolveTaskId(home, declared, session = sessionIdentity()) {
  ...
  if (active.length === 0) {
    throw new GateError(`активной задачи нет: ${tasksDir(home)} пуст или все задачи закрыты. `
      + 'Задача заводится при spawn\'е первого worker\'а: ati-agents promptobus spawn --repo <имя> --brief <файл>');
  }
 ...
 * закреплён, или `null` — пишет своя либо сверить нечем (`BL-469`).
 *
...
// участнику идентичность доезжает через env его mcp-config, а не через привязку (`BL-426`).
 ...
 * adapter'а: упаковка каталога — его дело (ADR-032, §6).
 */
...
/**
 * История задач: страница записей от старых к новым, по умолчанию последние 50 (ADR-032, §6).
 *
...
// ([mcp/service.ts](../../packages/promptobus/src/mcp/service.ts)).
 ...
 * проверить mailbox, а не двигать store ([12](../../../docs/reference/12-promptobus.md)).
 * Хвост дома не опознан — переезда по нему не бывает вовсе, значит и ждать нечего.
 */
export function storePending(home) {
 ...
 * аргументы команды Stop-хука, `BL-469`); она сильнее окружения, и порядок здесь не
 ...
 * источником ([reference/13](../../../docs/reference/13-participant.md)).
 */
export function resolveIdentity(env = process.env, cwd = process.cwd(), { move = true, declared: said = null } = {}) {
  ...
  // снесённый каталог.
  const at = (root) => (move ? promptobusHome(root) : path.join(root, PROMPTOBUS_REL));
  if (declared) {
    ...
    const root = rootOfHome(declared);
    home = root ? at(root) : declared;
  } else {
    const root = findRoot(cwd);
    if (!root) {
      throw new GateError('не найден корень workspace (modules.lock + AGENTS.md вверх по дереву) '
        + 'и не задан PROMPTOBUS_HOME — шину Promptobus не с чем связать');
    }
    home = at(root);
  }
  return {
