...
// Engine on a home: the first access restores unfinished fan-outs of every task and
// reports the result, and there is no need to do that on every store call. The key is the
// home itself: a CLI process works with one, and the suite opens one temporary directory
// per file.
...
export function bus(home, { cli } = {}) {
  ...
  const engine = openEngine({ home, policy: atiRouting, cli: cli ?? '0.0.0', recover: false });
  reportRecovery(engine.recover());
