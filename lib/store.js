...
// Adapter шины над Promptobus (ADR-032). Ядро — задачи, участники,
...
// Корень store шины внутри рабочего места. С  это `.promptobus`: переписка,
 ...
 *
 * Опознаются два хвоста — новый и прежний: `<root>/.promptobus` и прежний каталог (`LEGACY_REL`).
...
// Миграция прежнего каталога → `.promptobus` при ПЕРВОМ обращении (ADR-032, §7). Дверей в store
 ...
 */
export function resolveIdentity(env = process.env, cwd = process.cwd(), { move = true, declared: said = null, host = null } = {}) {
  ...
  if (declared) {
    ...
    const root = host?.findRoot?.(cwd) ?? null;
    if (!root) {
