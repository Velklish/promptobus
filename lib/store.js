...
import {
  ...
  FOREIGN_ROUTE,
  foreignSessionOf, foreignTaskLine,
...
import { artifactsDir, blobsDir, filesDir, messagesDir } from '../dist/v1/layout.js';

// Keep these adapter-local path helpers for lib/ callers without widening the package entry.
export { artifactsDir, blobsDir, filesDir, messagesDir };
...
export {
  ...
  sessionOf, sessionIdOf, sameSession, foreignSessionOf, startedOf,
...
// records.
//
// Bus rule: every participant corresponds with the task orchestrator. An approver and
// worker in the same task may correspond directly once a reviewer result is on record; reviewers stay on
// the orchestrator-only route.
function routingPolicy(sender, recipient) {
  if (sender.role === ORCHESTRATOR || recipient.role === ORCHESTRATOR) return { allow: true };
  if ((sender.role === 'approver' && recipient.role === 'worker')
    || (sender.role === 'worker' && recipient.role === 'approver')) return { allow: true };
  return {
    deny: true,
...
function engine(home, version = selectedVersions.get(home) ?? null) {
  ...
  if (hit) return hit;
  const opened = openEngine({ home, policy: routingPolicy, cli: version, recover: false });
...
// Why this call names no session, asked once at a terminal refusal and nowhere else: the
// resolver reads the environment and a record file, and the gate itself stays cheap.
...
/**
 * The name a sent file lands under, at number `n`. Leading dots are dropped: a hidden
 * file is missed by an ordinary listing and by `*.json`, and a reader of the folder then
 * reports evidence that is there as absent.
 ...
 * deduplicated. The numbering runs on the name that LANDS, so two sources differing only
 * by a leading dot take two names instead of one overwriting the other.
 */
...
function outboundMessages(home, id, senderId, sinceMs = 0) {
  return taskMessages(home, id).filter((msg) => {
...
function acceptanceRoute(sender, recipient) {
  return (sender.role === 'approver' && recipient.role === 'worker')
    || (sender.role === 'worker' && recipient.role === 'approver');
}
...
function requireDirectSender(home, meta, sender, session) {
  ...
  if (!session) {
    ...
    if (contestedIdentity(said)) {
      throw new GateError(`task ${meta.id} direct worker↔approver traffic cannot prove participant "${address}": `
        + `${said.why}`);
    }
    throw new GateError(`task ${meta.id} direct worker↔approver traffic cannot prove participant "${address}": `
      + 'the calling harness supplied no session identity');
  }
  const holder = sessionIdOf(sender) ?? sessionOf(sender);
  if (!holder) {
    throw new GateError(`task ${meta.id} direct worker↔approver traffic cannot prove participant "${address}": `
      + 'its task record has no session');
  }
  const foreign = foreignSessionOf(sender, session);
  if (foreign) {
    throw new GateError(`task ${meta.id} participant "${address}" is held by session ${foreign}, and this one is ${session} — `
      + 'direct worker↔approver traffic cannot borrow an address');
  }
}
...
export function sendMessage(home, id, {
  ...
  // and stays under watch. Only mail to the orchestrator gets this exception: direct
  // participant traffic must start from an address already registered in this task.
  //
  ...
  if (!known.includes(from)) {
    ...
    if (to !== ORCHESTRATOR) {
      throw new GateError(`task ${id} has no sender participant "${from}" — direct participant traffic is confined `
        + 'to addresses registered in this task; automatic foreign registration is only for messages to orchestrator');
    }
    const version = writerVersion(home, `automatically registering task ${id} participant ${from}`);
    upsertParticipant(home, id, participantRecord(from, {
      dismissed: new Date().toISOString(),
      ...(version ? { [MECHANISM_VERSION_FIELD]: version } : {}),
    }));
  }
  if (sender && acceptanceRoute(sender, recipient)) {
    requireDirectSender(home, meta, sender, session);
  }
  // Both artifact refusals stand BEFORE the blob is written: a name that cannot land
  // visibly and a record its own schema rejects must not leave payload in the task.
  if (artifactPath) {
    const refusal = recordRefusal(landedName(artifactPath), artifactPath);
    if (refusal) {
      throw new GateError(`task ${id}: ${refusal}. Fix the record and send it again — a document `
        + 'the published schema refuses is read as evidence until someone opens the schema');
    }
  }
 ...
 * these bytes are new here. Content addressing makes a repeat send lawful and free, and
 * the sender is the one who cannot tell its reply from the reply to a new version.
 */
...
function requireSendable(meta, from, to) {
  ...
  const recipient = participantOf(meta, to);
  if (!recipient) return;
  const decision = routingPolicy(participantRecord(from), recipient, meta);
...
export function lastSentAt(home, id, addr) {
  return engine(home).lastSentAt(id, addrDir(addr));
}
...
export function resolveIdentity(env = process.env, cwd = process.cwd(), { move = true, declared: said = null, host } = {}) {
  ...
  if (!isAddress(role)) throw new GateError(`PROMPTOBUS_ROLE="${role}" — expected orchestrator, worker:<slug>, reviewer:<slug> or approver:<slug>`);
  ...
  return {
    ...
    home: canonicalHome,
    declaredTask,
    session: sessionIdentity(env, { home: canonicalHome, task: declaredTask, address: role }),
  };
}
