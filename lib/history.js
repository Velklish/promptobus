...
import {
  addrDir, addressOf, history as historyPage, isAddress, listTasks, nameOfArtifact, readTask,
  taskExists,
} from './store.js';

// Journal of read mail, oldest first; it never looks at the mailbox and marks nothing read.
// Unit and paging: [03-cli.md](../docs/reference/03-cli.md#status-done-sweep-dismiss-history-prune).
...
// Reverse map of a v1 participant id back to a bus address: the forward map is unique, the
// reverse is not (a slug may itself contain a hyphen), so it is built from the task journal.
...
export function history(rootOrHost, { task, participant, limit, all } = {}) {
  ...
  if (participant !== undefined) {
    if (!isAddress(participant)) {
      fail(`${cmd} history: --participant "${participant}" — expected orchestrator, `
        + 'worker:<slug>, reviewer:<slug> or approver:<slug>');
    }
  ...
  for (const e of page.entries) {
    ...
    // An artifact is named by FILE NAME, as on every other surface: the message carries a
    // metadata-record id, and nobody finds the file in the task folder by that.
