...
export function history(root, { task, participant, limit, all } = {}) {
  if (all && limit !== undefined) {
    fail('promptobus history: --all и --limit вместе не принимаются — --all снимает лимит целиком');
  }
  let take;
  if (limit !== undefined) {
    take = Number(limit);
    if (!Number.isInteger(take) || take <= 0) {
      fail(`promptobus history: --limit «${limit}» — ожидается целое число больше нуля`);
    }
  }
  const home = promptobusHome(root);
  if (task && !taskExists(home, task)) {
    const known = listTasks(home).map((t) => t.id);
    fail(`promptobus history: задачи «${task}» в журнале нет`
      + `${known.length ? `. Известные: ${known.join(', ')}` : ''}`);
  }
  let box;
  if (participant !== undefined) {
    if (!isAddress(participant)) {
      fail(`promptobus history: --participant «${participant}» — ожидается orchestrator, `
