...
export function CodexRpc(io, { onLog = null } = {}) {
  ...
  const dispatch = (msg) => {
    onLog?.('in', msg);
    // A JSON-RPC request or notification always carries `method`. Each
    // direction owns its own id space, so a server-to-client request may reuse
    // an integer still sitting in `pending`. Method-first: that message is a
    // request, not the outstanding client reply.
    if (msg.method) {
      if (msg.id !== undefined) {
