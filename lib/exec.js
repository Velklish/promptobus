...
// Process-wide output budget. Node's own default is smaller; a verbose npm or a large
// uncommitted-file list can cross it and lose the process result. A dirty clone is
// then read as "clean tree" or "state unknown", and the latter decides whether to
// clean a worker directory. Kept here because every launch through `run` gets it,
// not only the git callers.
...
// Resolve trace. Off unless `PROMPTOBUS_EXEC_TRACE` names a file, and then every
// command `run` launches is appended to it as `<cmd>\t<resolved path>`. It exists for
// one gate: the suite runs with a SEALED PATH — one directory of symlinks to the
// binaries a run may reach — and the gate at the tail of the run
// ([test/run.mjs](../test/run.mjs)) asserts that nothing was launched from outside
// the run's own directory. Without the trace the seal is a property nobody watches;
// with it, a hole in the seal is a red run instead of a hang in a stranger's binary.
//
// One variable, one file, every process appending: the same shape as the warden
// auto-lift trace (`PROMPTOBUS_WARDEN_TRACE` in [warden.js](warden.js)), and for the
// same reason — the processes are detached and short-lived, and a shared append-only
// file is the only thing all of them can reach.
//
// **The boundary, stated where it can be read:** this covers launches that go
// through `run`, which is every external process inside the bus boundary. A test file
// that calls `spawnSync` itself does not pass here and is not traced.
const TRACE_VAR = 'PROMPTOBUS_EXEC_TRACE';

// Where PATH would find `cmd`. Only for the trace: `run` on POSIX hands the bare name
// to `spawnSync` and lets the kernel resolve it, and changing that to a resolved path
// would change what an ENOENT says and which binary a re-exec picks up.
//
// On Windows the launch plan's PATHEXT search is the answer. On POSIX the execute bit
// is REQUIRED, not just existence: the trace is the escape gate's only evidence, and
// a non-executable file of the right name earlier on PATH is skipped by the kernel —
// so counting it here would name a path nothing ever started, and hide the directory
// the real binary came from behind it. The seal builds its symlinks by the same test
// ([hygiene.mjs](../test/hygiene.mjs)), so the two agree on what "found" means.
