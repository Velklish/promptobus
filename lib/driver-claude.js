...
import { writeEntries } from './model-routing/cache.js';
...
//
// This list is the model INVENTORY, and it exists because the binary has no
// listing to ask: there is no `models` subcommand and no `--list-models` on this
// build, and an unrecognised word after `claude` is taken as a prompt rather than
// refused — so a probe cannot go looking for one. The lift is unchanged by this:
// `--model` is still passed through as the person named it, and a full model name
// (`claude-fable-5`) remains as lawful as an alias.
//
// A name here that the routing catalog does not rate is not a mistake — it shows
// up in `promptobus models` as an unrated runtime row, which is exactly what the
// row is for.
export const MODEL_ALIASES = ['fable', 'opus', 'sonnet'];

// What the availability adapter reports as the inventory: the aliases above plus
// the model this driver lifts participants on. The union is not decoration —
// `DEFAULT_MODEL` is what a participant gets without a flag, and a default that
// moved off the published alias list would otherwise vanish from the inventory
// while still being the thing every spawn asks for.
const PROBE_MODELS = [...new Set([...MODEL_ALIASES, DEFAULT_MODEL])];
 ...
 *
 * **The line exists because the mark is invisible otherwise.** Nothing reads the
 * availability cache yet and no command clears it, so a person whose participant
 * refused to start would be left with a file they never saw holding a state that
 * time does not lift. So the refusal names the file and the way out of it.
 *
 ...
 *
 * The entry is written through `writeEntries` rather than `markExhausted`, which
 * derives its reason from `resetAt` alone and so cannot say "the subscription
 * named a reset this adapter refuses to parse". Both doors are the cache's own
 * and both merge one harness into the stored snapshot.
 *
...
export function markLimitAtStart(host, output) {
  ...
  try {
    ...
    return ` The account limit was spent, so harness "${CLAUDE}" is now marked exhausted in ${cacheFile}:`
      ...
      + ` ${CLAUDE}\`` + ' once that command exists, or delete that harness entry from the file.';
  } catch {
    return '';
  }
}
...
export const claudeDriver = {
  ...
  availability: claudeAvailability(PROBE_MODELS),
