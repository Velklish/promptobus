...
export function markLimitAtStart(host, output) {
  ...
  const clearCommand = host?.busCommand?.(['models', '--clear-exhausted', CLAUDE])
    ?? `promptobus models --clear-exhausted ${CLAUDE}`;
...
// the first edit.
export function stallRoute({ kind, address, repoAbs, task, reviewCommand = null }, id, name) {
  const review = reviewCommand ?? `promptobus review "${repoAbs ?? '<clone path>'}"${task ? ` --task ${task}` : ''}`;
