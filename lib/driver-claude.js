...
import { AGENTS_BIN } from '../util.js';
import { run } from '../exec.js';
import { claudeUserMcp, KNOWN_TOOLS, versionLess } from '../tools.js';
...
// запрещены в `cli/packages/promptobus/src/**` — это сторожит отдельный гейт
// ([promptobus-package.test.mjs](../../test/promptobus-package.test.mjs)).
//
// **Второе правило зеркально первому** (`BL-466`): остальной механизм не импортирует этот
...
//
// Дом этих значений здесь, а не в `contract.js` (`BL-466`): уровни effort, режимы прав и
...
// `claude --help` 2.1.251). Worker'у по умолчанию идёт `auto` (`BL-461`): `acceptEdits` снимает
...
// Модель участника без флага `--model`. Дом здесь, а не у команд подъёма (`BL-468`): имя
...
export const PHRASES = {
  ...
  // перепиской. Второй harness пишет иначе, и потому это слово driver'а (`BL-468`).
...
//
// **Вход в разбор — КОНЕЦ ХОДА, а не состояние `blocked`** (`BL-428`). До этой задачи вход
...
// и `idle/done`. То есть молчаливый участник был невидим целиком, и доклад `BL-416` не уходил
...
// живым замером и потому уцелели при укорочении (`BL-418`): опора на протокол шины, а не
...
/**
 * Канал пробуждения ЭТОЙ сессии — вход диагностики (`BL-466`). Где лежит адрес сокета и как
...
//
// **За чужой адрес запись не идёт** (`BL-469`). Адрес закреплён за сессией в журнале, и
...
//
// Наружу отсюда не торчит ничего (`BL-466`): сборка конфига и argv — половины ОДНОЙ
...
function optionRefusal({ effort = null } = {}, tool) {
  ...
  return `--effort ultracode: найден claude ${tool.version}, а это ключевое слово доезжает до сессии `
    ...
    + `Обнови бинарь (${KNOWN_TOOLS.claude.install}) либо поднимай участника с --effort xhigh.`;
}
...
// Потолок ожидания смерти сессии и шаг опроса (`BL-473`).
//
...
//
// **Идемпотентно, и это не послабление** (`BL-424`): гасит его `promptobus done` по всему списку
...
// (`BL-473`). Живой `claude stop` отвечает раньше, чем сессии не станет в реестре, а следом
