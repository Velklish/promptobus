...
// from `claude --help`, 2.1.251 and unchanged on 2.1.263). A worker defaults to `auto`: `acceptEdits` skips
...
/**
 * The modes a session bypasses permission prompts in, in the sense the harness's
 * own held-message dialog means by it — "this session bypasses prompts".
 *
 * **Measured, not read off the names** (2026-09-11, claude 2.1.263): a background
 * session was lifted in each mode, the warden's postcard was knocked into its
 * messaging socket, and its record and log were read. `bypassPermissions` held
 * the message as a dialog and the record went to `waitingFor: "permission
 * prompt"`; `dontAsk`, `acceptEdits` and `auto` each took it — the body reached
 * the session and the record stayed `busy`. So the list is one mode long, and
 * `dontAsk` is not in it however its name reads.
 */
...
// Model aliases the harness publishes under `--model`. The help aliases remain in
// their printed order (`claude --help` 2.1.263, measured 2026-09-06: "an alias for
// the latest model (e.g. 'fable', 'opus', or 'sonnet')" — the same sentence 2.1.251
// printed, so that set did not move when the `fable` alias re-pointed); the proven
// baked-catalog `haiku` exception follows them.
//
// The binary's baked catalog carries a fourth family, `haiku`, and a `mythos`
// family beside it. The help sentence omits `haiku`, while its baked aliases block
// maps it to `claude-haiku-4-5`; a one-turn probe on 2.1.263 accepted that full id,
// so this proven, rated alias joins the dictionary. `mythos` has no baked alias and
// its one-turn check refused the full id, so it remains out. The help sentence is
// still the source for the aliases it advertises; the one explicit baked-catalog
// addition is kept only because liftoff and a rated row establish that it is usable.
//
...
// routing catalog rates. Measured 2026-09-06 on `claude` 2.1.263: `--model`'s own
...
// — `{id:"claude-opus-5",family:"opus",…}` and the same for the others, found by
// grepping the installed binary, which is the only listing this harness publishes
// at all (no `models` subcommand, no `--list-models`).
//
// **`claude-fable-5-1` joined the list on 2.1.263, and `claude-fable-5` did not
// leave it.** Both are full first-party rows of that same baked table —
// `{id:"claude-fable-5",family:"fable",display_name:"Fable 5",knowledge_cutoff:
// "January 2026",…}` and `{id:"claude-fable-5-1",family:"fable",display_name:
// "Fable 5.1",knowledge_cutoff:"June 2026",…}` — and both are in the binary's own
// array of ids it takes. What moved is which of them the `fable` ALIAS resolves
// to: `fable:{default:"claude-fable-5-1",per_provider:{gateway:"claude-fable-5"}}`
// and `latest_per_family.fable:"claude-fable-5-1"`, against 2.1.251's
// `fable:{default:"claude-fable-5"}`. `alias_migration` is still `{}`, so the
// binary rewrites nothing: the predecessor is a name a person may still type, not
// a name that now means the successor. Keeping it is a READ, not a guess.
//
...
//
// A scope names a FAMILY, not one model: the harness prints "Fable" on that row,
// and the weekly window it bounds is spent by every model of the family. So the
// `fable` entry carries both ids the binary serves under `family:"fable"` — the
// successor first, in the order `MODEL_IDS` lists them. Naming only the id the
// alias currently points at would leave a tuple on the predecessor bound by no
// window at all, which reads as "no limit known" rather than as the shared one it
// actually spends.
...
// The id each alias resolves to TODAY, read out of the proven binary's own baked
// catalog (`aliases.<name>.default` on 2.1.263). This is a second table and not a
// second reading of the one above, because the two answer different questions and
// gave the same answer only while every family served one model.
//
// A SCOPE names a family and may name several ids: one "Fable" weekly window is
// spent by every model of the family. An ALIAS names exactly one id, because
// `--model fable` starts exactly one model. `models calibrate` reads this one to
// put a run recorded as `--model fable` on the row that rates the model it
// actually ran, and it refuses an alias standing in front of more than one id —
// two ids is not a resolution, and taking the first would attribute a person's
// runs to a row at random. Handing it the scope table would therefore have made
// every aliased Fable run unrateable the moment the family gained a second model.
//
// **An alias resolves to where the proven binary points it today, and a telemetry
// record carries no binary version.** So runs recorded before a re-point are
// attributed to the successor: a `--model fable` run from before 2.1.257 is read
// here as `claude-fable-5-1`. That is the honest limit of resolving a name whose
// meaning moved, not a defect — the alternative is to resolve nothing and rate no
// aliased run at all. It is one more reason a ROW is never keyed on an alias.
...
// Re-proven live on 2026-09-06 against binary 2.1.263 (PB-34), because the number
// is a measurement and not a floor: a participant lifted in a standalone workspace
// sat on `idle`/`done`, a bus message reached it 6.2 s later — sent 07:56:26.508Z,
// the session back on `busy`/`working` and its reply in the mailbox at
// 07:56:32.744Z — with nothing else touching that session. The record form
// `claude agents --json` prints is unchanged from 2.1.251.
// Source: spike 2026-08-29 on 2.1.251; re-measured 2026-09-06 on 2.1.263.
...
// a limit.
//
// The late-start mark uses the limit pattern only as a gate, then checks LIMIT_RESETS
// independently: a refusal can contain both phrases, and the reset phrase still
// makes the exhaustion the subscription's. `sessionStall` below asks `.test` and is
// unaffected by that classification.
 ...
 *
 * LIMIT_DETAIL remains the limit gate, while LIMIT_RESETS chooses the code. A limit
 * refusal reads the same whether the session wrote it
 * after a turn or the binary wrote it instead of starting. When the refusal says
 * the limit RESETS, the exhaustion is the subscription's limit,
 * `subscription_exhausted`; otherwise nothing explains it and it is
 * `manual_exhaustion`. The reset phrase is checked even when an earlier
 * `hit your ... limit` phrase also appears anywhere in the refusal.
 *
...
//
// **`waitingFor` carries TWO dialogs and the record does not say which** (PB-165).
// One is a permission prompt of the session's own work; the other is a peer
// message the session HELD because it was lifted in a mode that bypasses prompts
// and the sender did not attest its own — which is what the warden's postcard
// becomes there. Measured 2026-09-11 on 2.1.263: the moment the postcard was
// knocked into the socket of a session lifted with `--permission-mode
// bypassPermissions`, its record read `status: waiting`, `state: done`,
// `waitingFor: "permission prompt"` — while the session finished its shell
// command, answered, and reached its Stop hook. So the classification here stays
// `permission` and is honest about it: one field, one answer. Which of the two it
// was is decided a layer up, on the bus's OWN marks, by `stallStands`; the lift
// closes the case at the source with `crossSessionInbound` in the settings file
// (`settingsFile` below).
...
export function stallRoute({ kind, address, repoAbs, task, reviewCommand }, id, name) {
  ...
  // A dialog, and the harness has ONE field for two of them: a permission prompt
  // of the session's own work, and a peer message the session held because it was
  // lifted in a prompt-bypassing mode and the sender did not attest its permission
  // mode (PB-165). The state machine has already dropped the second where it could
  // prove it — a turn that ended with the dialog standing — so what reaches here is
  // the one a person is genuinely wanted at; the second possibility is still named,
  // because a participant lifted by an older mechanism gets no such file and the
  // person opening the session should know which of the two they are looking at.
  if (kind === 'permission') {
    return `only a person can answer: ${PHRASES.enter(id)} (or drop the session: ${PHRASES.stop(id)}). `
      + 'If what stands there is a held message from another session rather than a prompt of its own work, '
      + 'the participant was lifted without this mechanism\'s participant settings file — deliver it there, and '
      + `the next lift with \`--permission-mode ${PROMPT_BYPASS_MODES[0]}\` will carry `
      + '"crossSessionInbound": "accept" and hold nothing';
  }
  if (kind === 'limit') {
...
// Host MCP identities are canonical pairs; Claude Code's permissions file takes
// its namespaced spelling. This is deliberately separate from PHRASES.tool: the
// latter is the short name the participant types in its prompt.
...
// declared in `src/hooks.ts` for both generated hook settings and this participant
// settings file. A READY command is wrapped here: the adapter builds it (it knows the
// workspace binary path), and the harness knows the form it lands in.
 ...
 *
 * `crossSessionInbound: "accept"` is added for a prompt-bypassing mode and for no
 * other, and it is the KEY THE HARNESS ITSELF NAMES (PB-165). A session lifted
 * that way does not deliver a peer message from a sender that did not attest its
 * permission mode — it HOLDS it as a dialog and says so in those words. The bus
 * is the only sender that reaches a participant, the notification it sends is the
 * one thing the participant must never miss, and it is the participant's own
 * mechanism that wrote this file: accepting it here is the bus saying "my own
 * postcards are delivered", not a general opening of the session to strangers.
 * Left out under every other mode on purpose — the dialog is a real review step
 * where a person can answer it, and a run that is not bypassing prompts has one.
 */
function settingsFile({ denyTools, guardCommand, permissionMode = null, extraSettings }) {
  return {
...
function prepare({
  ...
  const settings = settingsFile({
    denyTools, guardCommand, permissionMode, extraSettings,
  });
  return {
    argv: spawnArgv({
 ...
 * implemented.
 *
 * Five capabilities beyond the previous five declare binary PROPERTIES, not
 * operations: `denyTools` — `permissions.deny` in the settings file (the
 * read-only reviewer stands on it), `mcpDenyTools` — canonical MCP write tools
 * become `mcp__<server>__<tool>` deny ids, `systemPrompt` — the `--settings`
 * file itself, `sessionList` — the `claude agents --json` registry, `enter` — a
 * person entering the session (`claude attach`). Claude Code has all five.
 */
export const claudeDriver = {
  ...
  options: {
    ...
    // Alias → the one id it resolves to today, handed over the same way
    // `defaultModel` is: `models calibrate` groups a person's telemetry by model
    // and a run lifted on `--model opus` must land on the row that rates
    // `claude-opus-5`. It is the driver's dictionary, so it leaves through the
    // registry rather than being imported — the boundary gate refuses the other
    // direction (promptobus-adapter.test.mjs).
    //
    // This is `MODEL_ALIAS_IDS` and NOT the scope table the availability adapter
    // is handed below. The two were one object while every family served one
    // model, and that made them look like one dictionary; they are not. A scope
    // may name several ids because a family shares one window, an alias names one
    // because `--model fable` starts one model, and `calibrate` resolves an alias
    // in front of two ids to nothing at all.
    ...
    skillsDir: true,
    // Nothing of this driver's lands in the participant's working directory: its
    // mcp-config and settings file go to the task store, and the flags point at them.
    // So there is no directory in the repository for a lift to overwrite, and Git is
    // asked nothing before the write.
    launchDirs: [],
  },
  phrases: PHRASES,
