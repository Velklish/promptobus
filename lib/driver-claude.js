...
// Stall of a session: null, or { kind, reason }, kind permission | limit | unknown. Where the
// reason's two halves come from and the three dialogs one field carries:
// [reference/05-drivers.md#sessionstall--sessionstall-answers-null-for-no-stall-otherwise--kind-reason--kind-is-permission](../docs/reference/05-drivers.md#sessionstall--sessionstall-answers-null-for-no-stall-otherwise--kind-reason--kind-is-permission).
...
export function stallRoute({ kind, address, repoAbs, task, reviewCommand }, id, name) {
  ...
  // A reviewer has a package command; an approver is lifted only by the consumer.
  const relift = () => (address?.startsWith('reviewer:')
    ? `lift the reviewer again: ${review}`
    : (address?.startsWith('approver:')
      ? `ask the task orchestrator to lift the approver again after the piece's green review`
...
function optionRefusal({ effort = null, statusCommand } = {}, tool) {
  ...
  if (!versionLess(tool.version, ULTRACODE_MIN_VERSION)) return null;
