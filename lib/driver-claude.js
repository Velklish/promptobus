...
// Здесь собрано ВСЁ, что знает про
...
// Это НЕ минимальная версия и не гейт: поверхность инъекции
...
// Число нужно другому: `promptobus status` и `doctor` называют, на чём канал
...
/**
 * Слова harness'а, которые adapter вставляет в свои строки. Общий текст остаётся
 ...
 * CLI нет вовсе (вне первого этапа). Остальное объявлено и реализовано.
 *
 * Четыре capability сверх прежних пяти объявляют СВОЙСТВА бинаря, а не операции:
...
export const claudeDriver = {
  ...
  phrases: PHRASES,
  // Перевод harness-neutral контекста в свой план подъёма — дело driver'а,
