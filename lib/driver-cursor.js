...
import { fail, info, skillDirs } from '../util.js';
import { run } from '../exec.js';
import {
  CURSOR_SKILLS_REL, cursorMcpDuplicates, findToolBin, KNOWN_TOOLS, resolveUtilBin, versionLess,
} from '../tools.js';
...
// Driver harness'а Cursor — второй production driver шины (ADR-032 §3, ADR-034, ADR-035,
// ADR-036). Здесь собрано ВСЁ, что механизм знает про Cursor: форма его конфигов, флаги
...
// интерактивный TUI в панели tmux, поднятый подкомандой `agent persist` (ADR-036): она
...
//
// Все числа и формы ниже сняты живыми прогонами спайков `BL-467` (headless) и `BL-478`
// (`persist`) на `agent` 2026.09.02-c22c1a3 и tmux 3.6b, а не выведены из документации:
// документации ни на `-p --output-format stream-json`, ни на `persist` у Cursor нет вовсе.
// Ссылки вида «REPORT §4.4» ведут в отчёты спайков, приложенные к задачам
// [BL-468](../../../docs/archive/BL-468-cursor-driver/task.md) и
// [BL-484](../../../docs/backlog/BL-484-cursor-driver-persist-sessions.md).
//
...
// проверена живьём и держится под `persist` (REPORT §4.7 спайка `BL-478`: ход «выполни
...
// deny-конфига. Это НЕ минимум (минимум объявлен в `KNOWN_TOOLS.cursor`) — число называет,
 ...
 * исполняется отдельным ходом сразу после текущего (REPORT §4.3 спайка `BL-478`): в идущий
 ...
 *
 * Все четыре команды у Cursor теперь есть, и все — подкоманды `persist` (ADR-036): сессия
...
// `prepare` (ADR-034 §2), и вызывающий команду по частям не собирает.
 ...
 * (тот же приём, что у headless-ревью ADR-019).
 *
...
// `allow` обязателен схемой: без него `agent` падает с `Invalid project config` (ADR-019).
 ...
 * §4.4 спайка `BL-478`: девять имён, из них два выдуманных, дали ноль хуков; после
 ...
 * `transcript_path` (REPORT §4.4 спайка `BL-478`). Хуки читаются только из ПРОЕКТНОГО
 ...
 * (ADR-036). Своих флагов у подкоманды нет вовсе — всё после неё уезжает интерактивному
 * `agent` как есть (REPORT §4.1 спайка `BL-478`), поэтому набор флагов тот же, что был у
...
/**
 * План подъёма из harness-neutral контекста (ADR-034 §2). Ничего не пишет и не запускает.
 *
 ...
 *
 * **Версия.** Отказ свой, не общий `minVersion`: общий минимум (`KNOWN_TOOLS.cursor`)
 * выведен из read-only-гарантии ревью и держит headless-путь ADR-019, который на старом
 ...
 * вовсе (ADR-036): подъём упёрся бы в «панель-поставщик pty не поднялась» уже после того,
 ...
 */
function optionRefusal(options, tool, { util = resolveUtilBin } = {}) {
  if (tool?.version && versionLess(tool.version, PROVEN_CURSOR_VERSION)) {
    return `найден agent ${tool.version}, а подъём участника Cursor проверен на ${PROVEN_CURSOR_VERSION} и новее: `
      ...
      + `Обнови: ${KNOWN_TOOLS.cursor.install} (бинарь обновляет себя и сам — довольно одного запуска).`;
  }
  const tmux = util(TMUX_UTIL, { fresh: true });
  if (!tmux.ok) return `${tmux.reason} Без него участник Cursor не поднимается вовсе: сессия — панель tmux (ADR-036).`;
  return null;
}

/** Имя утилиты рабочего места, на которой стоит подъём. Дом объявления — `KNOWN_UTILS`. */
 ...
 * хода.
 *
 * **Сдаёт его ХУК, и это изменение против ADR-035 §7** (там сдавал бегун). Причина, по
 * которой сдача переезжала в бегуна, под persist исчезла: тогда отпечаток, пойманный
 ...
 * contact point ПЕРЕПИСАН (`moved` в [supervisor.ts](../../packages/promptobus/src/supervisor.ts)),
...
// --- канал пробуждения ------------------------------------------------------------
//
// **Как участник Cursor узнаёт о сообщении** (`BL-324`, переписано в `BL-484`). Механизм
...
//      `moved` в [supervisor.ts](../../packages/promptobus/src/supervisor.ts) сравнивает
 ...
 *
 * **Отказа «ход участника идёт» здесь больше нет** (ADR-036, против ADR-035 §7): инъекция в
 ...
 *
 * **Состояние `stale` у Cursor появилось** (ADR-036, против ADR-035 §2). Под headless его не
...
export function stallRoute({ kind, address, repoAbs, task }, id) {
  ...
  const relift = () => (address?.startsWith('reviewer:')
    ? `поднимай reviewer'а заново: ati-agents promptobus review "${repoAbs ?? '<путь клона>'}"${task ? ` --task ${task}` : ''}`
...
function liveBin(recorded) {
  ...
  const found = findToolBin(CURSOR);
 ...
 *
 * **Возвращается операция после того, как сессии у harness'а НЕТ** (`BL-473`): следом за
 ...
 * (ADR-032 §1), как и у Claude. Не путать с `enter`: то — вход ЧЕЛОВЕКА в сессию из
 ...
 * (ADR-036): инъекция в TUI, а не новый процесс. Доставка проходит и в идущий ход — текст
...
export const cursorDriver = {
  ...
  options: {
    ...
    // Утилиты рабочего места, без которых driver не поднимет сессию. Дом объявления —
    // `KNOWN_UTILS` ([tools.js](../tools.js)); driver называет имя, резолв и проверку версии
    // зовёт adapter — тем же порядком, что у бинаря harness'а (ADR-034 §6).
    utils: [TMUX_UTIL],
