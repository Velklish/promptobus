...
export function stallRoute({ kind, address, repoAbs, task, reviewCommand, doneCommand }, id) {
  ...
  if (kind === 'watchdog') {
    ...
    // worktree could be read at all (a reviewer's cwd is a sandbox with no commits, and
    // "could not be read" is its permanent state). So it names the signals and leaves
    // the measurement to the verdict, which is the line that actually holds them.
  ...
  if (kind === 'question') {
    return 'the turn is HELD on a question, not spent on one: AskQuestion is neither answered nor skipped without a person, '
      + `and the turn stands until a key is pressed (measured 278.497 s, ended by Escape; no upper bound found). `
      + `A message will NOT run until then. Look in and press Escape — ${PHRASES.enter(id)} `
      + '(leave the view with ctrl+b d, the turn is cut with ctrl+c)';
  }
  if (kind === 'failed') {
