...
import {
  ...
  silentIsStall, stopSession, toolKidsOf, turnIdleMs, turnState, worktreeTouchedMs, writeSession,
...
/**
 * Name of the BINARY, and it is not the harness name. The two were the same string
 * until 2026-09-05, and the difference cost a hung test: `options.tool` said `cursor`,
 * `spawn` handed that to `host.resolveToolBin`, the standalone host hands a name back
 * without searching, and `run('cursor')` then found the operator's own
 * `~/.local/bin/cursor` and waited on `cursor mcp enable` — while the stand had
 * stubbed `agent`, the name this driver documents (PB-2).
 *
 * `cursor-agent` rather than `agent`: it is the name Cursor itself ships and installs,
 * it is first in this file's own `CURSOR_BINS` search order, and unlike a bare
 * `agent` it cannot collide with a stranger's binary of that name on a shared PATH.
 *
 * One constant, two readers — `options.tool` and the availability adapter — so the
 * name a host is asked for and the name the adapter probes cannot drift apart.
 */
export const CURSOR_TOOL = 'cursor-agent';

function versionLess(a, b) {
  const parts = (value) => String(value).match(/^\s*(\d+(?:\.\d+)*)/)?.[1]
    .split('.').map(Number) ?? [];
  const pa = parts(a);
  const pb = parts(b);
...
function searchPath({ env = process.env, home = homedir() } = {}) {
...
function resolveTmux(_name, { env = process.env } = {}) {
  const { sep, dirs } = searchPath({ env });
  const r = run('tmux', ['-V'], {
    encoding: 'utf8',
    env: { ...env, PATH: dirs.join(sep) },
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  if (r.error || (r.status !== 0 && !String(r.stdout ?? r.stderr ?? '').trim())) {
    return {
      ok: false,
      reason: `tmux: not found in PATH or in the known install locations (${CURSOR_INSTALL_DIRS.join(', ')}). `
        + 'Install: brew install tmux (macOS) / your distro package (Linux).',
    };
  }
  const raw = `${r.stdout ?? ''}\n${r.stderr ?? ''}`;
...
function mcpConfig({ servers }) {
  ...
  // Cursor REPLACES this child's environment, measured: a sentinel on the lift command
  // reaches the harness and not its MCP child, so both names travel or neither does.
  const extra = {};
...
function sessionEnv(base = process.env, extra = {}) {
  // Drop from the INHERITED environment, then take `extra`: a caller that names one of
  // these wins, an ancestor that happens to have one never does. Codex's order (PB-182).
  const env = { ...base };
...
function inspect(ref) {
  ...
  const kids = turn.busy && turn.silent
    ...
    : [];
  // The worktree is asked only where the first two signals already point at a stall: it
  // costs two git calls, and on an idle session or a live tool nothing would read the
  // answer. It is the signal that covers a turn editing files and spawning nothing —
  // the shape twice reported as a false stall (PB-7).
  ...
  if (turn.busy) {
    ...
    return {
      state: 'alive',
      busy: true,
      // The verdict names every measurement and its span: an orchestrator has to tell a
      // silent transcript from a dead session, and "silent longer than the threshold"
      // said neither what else was looked at nor for how long.
...
export function stallRoute({ kind, address, repoAbs, task, reviewCommand, doneCommand }, id) {
  ...
  if (kind === 'watchdog') {
    // The route has the stall KIND and nothing else — not the numbers, not whether the
    // worktree could be read at all. It names the signals and leaves the measurement to
    // the verdict beside it, which is the line that holds them (PB-183).
 ...
 *
 * The binary reaches this function as a normalized concrete path. normalizeTool
 * resolves an updater symlink and probes --version on that same path before launch.
 *
...
export const cursorDriver = {
  ...
  options: {
    ...
    skillsDir: false,
    // Everything this driver writes into the participant's working directory sits under
    // `.cursor/`, so that is where a launch path stops being the working directory. Git
    // is asked about the files this driver writes and not about the directory: a
    // project's own `.cursor/rules` stays its own, and no lift overwrites it.
    launchDirs: [CURSOR_DIR],
  },
  phrases: PHRASES,
  // Availability of the ACCOUNT, asked before any session exists — a different
  // question with a different lifetime from everything else in this file, so it
  // lives in its own module ([adapter-cursor.js](model-routing/adapter-cursor.js)).
  // The tool name is handed to it rather than repeated there: it is the same
  // string `options.tool` above declares.
