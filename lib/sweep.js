...
import { driverFor, harnessOf, snapshotSessions, validate } from '../dist/index.js';
...
function artifactPlan(home, id, participantId) {
  ...
  if (broken.length) return { broken, going: [], held: new Set() };
  const mine = sentArtifacts(home, id, participantId);
  // A blob is deduplicated inside the task: it leaves only once no surviving record names it.
  return {
    broken,
    going: records.filter((a) => mine.has(a.id)),
    held: new Set(records.filter((a) => !mine.has(a.id)).map((a) => a.sha256)),
  };
}
...
// Hard links the payload still has. `placeFile` links a files/ entry BEFORE the record
// lands, so a count above one also covers a sender whose record is still in flight.
function linkCount(file) {
  try {
    return statSync(file).nlink;
  } catch {
    return 0;
  }
}

function sweepArtifacts(plan, kept) {
  ...
  for (const rec of plan.going) {
    ...
    swept.records += 1;
    if (plan.held.has(rec.sha256) || !existsSync(rec.blob)) continue;
    if (linkCount(rec.blob) > 1) { swept.busy.push(rec.filename); continue; }
...
function takePiece(host, home, id, address, mark, registry) {
  ...
  if (tree.state === 'none' || tree.state === 'removed') sayArtifacts(sweepArtifacts(plan, kept), address);
...
function sayArtifacts(swept, address) {
  ...
  for (const filename of swept.busy) {
    warn(`blob of artifact «${filename}» stays: another hard link to it is still there, so a neighbour holds `
      + 'the payload and its record may not be on disk yet');
  }
}
