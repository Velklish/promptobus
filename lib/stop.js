...
import {
  addressesOf, approverHere, ORCHESTRATOR, ownerRoute, ownership, participantOf, readTask,
...
export async function stop(rootOrHost, { task, address } = {}, { registry = REGISTRY } = {}) {
  ...
  // The proof `sweep` and `dismiss` share: this closes a session the mechanism started for THIS
  // run, and a foreign hand would take a worker off work somebody else is watching.
  ...
  if (!own.allowed && !approverHere(meta, session)) {
    ...
    fail(`${unprovenOwnerLine(home, meta, own)}: a participant session is stopped by the task mailbox `
      + 'owner or by an approver of this task holding its own recorded session — the session belongs to '
      + `that run. ${approver ? `${approver} ` : ''}${ownerRoute(home, meta, own, host.busCommand(['stop']))}`);
  }
  ...
  if (address === ORCHESTRATOR) {
    fail(`${ORCHESTRATOR} has no session this mechanism started — it is the one that started `
      + `the others. Stop a task participant. ${route}`);
  }
  ...
  if (state === 'unknown') {
    fail(`participant ${address}: its session state could not be read — ${unknownWhy(p, sessions, driver)}. `
      + `Nothing was stopped, and the session may still be running — check ${driver.phrases.sessions}`);
  }
  if (state !== 'alive') {
