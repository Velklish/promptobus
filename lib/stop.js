...
import {
  addressesOf, ORCHESTRATOR, ownerRoute, ownership, participantOf, readTask,
...
export async function stop(rootOrHost, { task, address } = {}, { registry = REGISTRY } = {}) {
  ...
  if (!own.allowed) {
    fail(`${unprovenOwnerLine(home, readTask(home, id), own)}: a participant session is stopped by the `
      + `task mailbox owner — the session belongs to their run. ${ownerRoute(home, readTask(home, id), own, host.busCommand(['stop']))}`);
  }
