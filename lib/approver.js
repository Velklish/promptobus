...
import {
  dryRunToolNote, guardHookNote, keepBrief, liftHarness, memoryRule, mcpNote, mcpServerLines,
  normalizeLaunchTool, optionRefusal, participantMcp, participantPluginDir, PROMPTOBUS_SERVER,
  readBrief, withToolVersion,
...
import { ATTACHMENT_CONTRACT, GATE_RECORD_SCHEMA, RESULT_BODY_MAX } from './handoff.js';
...
function reviewerSubjectLines(taskMeta) {
  const rows = (taskMeta.participants ?? []).filter((p) => p.role === 'reviewer');
  if (!rows.length) return 'this task records no reviewer';
  return rows.map((p) => {
    const at = p.metadata?.repoAbs;
    const address = addressOf(p);
    return at ? `${address} lifted from ${at}` : `${address} with no recorded subject`;
  }).join('; ');
}

function subjectRule(repoDir) {
  return '--approver lifts an approver for a piece a reviewer already read, so the subject must be '
    + `the path that reviewer lifted from. This call passed ${repoDir}.`;
}
...
function validateApproverPreconditions(host, {
  ...
  const recordedReviewers = reviewerSubjectLines(taskMeta);
  if (!reviewer) {
    throw new GateError(`${subjectRule(repoDir)} That path resolves to ${reviewAddr}, and task ${task} has no such reviewer. `
      + `Reviewers in this task: ${recordedReviewers}.`);
  }
  const recordedRepo = reviewer.metadata?.repoAbs;
  if (!recordedRepo) {
    throw new GateError(`${subjectRule(repoDir)} ${reviewAddr} is recorded in task ${task} with no repository path, `
      + `so there is no subject to match. Reviewers in this task: ${recordedReviewers}.`);
  }
  if (canonical(recordedRepo) !== canonical(repoDir)) {
    throw new GateError(`${subjectRule(repoDir)} ${reviewAddr} is recorded at ${recordedRepo}, not ${repoDir}.`);
  }
  if (!reviewerResultSent(home, task, reviewAddr, reviewer)) {
    throw new GateError(`--approver requires a type=result message from ${reviewAddr} for the subject ${repoDir} in task ${task}'s journal — `
...
export function planApprover(rootOrHost, {
  ...
  strategy = undefined, routing = null, dryRun, brief: briefFile,
} = {}) {
  ...
  const prompt = buildApproverPrompt({
    ...
    reviewAddr, branch, rules, driver, host, mcpBoundary, brief,
  });
  ...
  return {
    home, taskId: task, slug, address, reviewAddr, name, nsPath, repoDir, cloneRoot, worktreeDir, brief,
    briefStem: `brief-approver-${slug}`,
