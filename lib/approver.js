...
function validateApproverPreconditions(host, {
  ...
  if (participant && harness && harness !== participant.harness) {
    throw new GateError(`${address} in task ${task} was started by harness ${participant.harness}, and --harness asks for `
      + `${harness}: a repeat call would restart the approver in another tool — open a separate task instead.`);
  }
...
Your gate run travels as JSON to \`${GATE_RECORD_SCHEMA}\` — one entry per gate command — and is sent BEFORE the result as type=artifact. Request the file stem \`gates-<your approver slug>.json\`.`;
}
