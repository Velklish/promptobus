...
import { clearExhausted } from './model-routing/cache.js';
import { preflight } from './model-routing/preflight.js';
import { render } from './model-routing/render.js';
import { ROLES, resolve } from './model-routing/resolver.js';
 ...
 */
export function routingMetadata(decision) {
...
function validateCommand(host, { catalogFile, now }) {
  ...
  info(`layers: ${verdict.layers.map((l) => `${l.id} ${l.present ? l.path : '(absent)'}`).join('\n          ')}`);
  ...
  for (const e of verdict.errors) bad(`${e.code} · ${e.layer}${e.at ? ` · ${e.at}` : ''}: ${e.message}`);
...
export function availabilityOf(snapshot) {
  return Object.entries(snapshot?.harnesses ?? {}).map(([harness, e]) => {
    const row = {
      harness,
      state: e.state,
    ...
    if (e.tier !== undefined) row.tier = e.tier;
    if (e.credits !== undefined) row.credits = e.credits;
    if (e.resetCredits !== undefined) row.resetCredits = e.resetCredits;
    if (Array.isArray(e.windows) && e.windows.length) row.windows = e.windows;
    return row;
  });
}
