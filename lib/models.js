  ...
  try {
    ...
    if (e instanceof GateError) {
      ...
      warn(`${layer.subject} could not be used while reading the strategy default: ${e.message}. `
        + 'Run `promptobus models validate` to inspect the routing stack');
      return null;
    }
    throw e;
  }
...
async function calibrateCommand(host, {
  ...
  const report = calibrate(records, {
    tuples: shipped, overlayTuples: merged.tuples ?? [], aliases: aliasTables(host),
  });
  const moved = Object.keys(report.ratings).length;

  // Every refusal happens BEFORE anything is printed. A refusal thrown after a
  // JSON document has gone to stdout leaves a caller holding a complete report
  // and a non-zero exit, with nothing saying which of the two to believe.
  ...
  if (json) {
    emit(output, JSON.stringify({
      ...
      skipped,
      write: write
        ? { layer: layer.id, path: layer.path, tuples: moved, applied }
        : null,
    }, null, 2));
    return 0;
  }
  ...
  if (!write) {
    if (moved) info(`to apply these lines: ${host.formatNpx(['models', 'calibrate', '--write'])}`);
    return 0;
  }
  if (!moved) {
