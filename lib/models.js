// The `models` command surface: preflight, decision, calibrate and the exhaustion mark.
// What each subcommand reads, writes and refuses: [guides/model-routing.md#the-models-command-what-each-subcommand-reads-and-writes](../docs/guides/model-routing.md#the-models-command-what-each-subcommand-reads-and-writes)
import { existsSync } from 'node:fs';

import { GateError, PromptobusError } from '../dist/index.js';
import { adapterOf, driverByHarness } from './drivers.js';
import { hostOf } from './host.js';
import {
  bad, info, ok, warn, writeFileAtomic,
} from './util.js';
import {
  CATALOG_FILE, OVERLAY_SCHEMA_VERSION, STRATEGIES, loadCatalog, readLayerFile, readLayers,
...
/**
 * Ask one yes/no question on the terminal and give back what was typed.
 *
 * `node:readline` is imported where it is used rather than at the top: this is
 * the only interactive prompt in the package, and a module that opened stdin at
 * load would attach a reader to every `models` call, including the ones a
 * warden makes with no terminal at all.
 */
...
// The `models` command surface: which subcommand reads what and which writes.
// The table: [guides/model-routing.md#agedsnapshot--where-the-decision-takes-the-moment-its-snapshot-was-assembled](../docs/guides/model-routing.md#agedsnapshot--where-the-decision-takes-the-moment-its-snapshot-was-assembled)
 ...
 * does not know. The `GateError` carries the layer that caused it, so the
 * routed-call code does not need to inspect the files a second time. An
 * untagged error names no layer rather than blaming the shipped catalog and
 * stays `overlay-invalid`: overlays are the only files a person edits, while
 * the catalog ships with the package. The route out is named: the whole
 * picture is one command away.
 */
 ...
 * What is kept and what is deliberately not: [guides/model-routing.md#routingmetadata--the-decision-as-it-is-kept-on-the-participant](../docs/guides/model-routing.md#routingmetadata--the-decision-as-it-is-kept-on-the-participant) */
 ...
 *
 * A missing default and an unreadable stack are different facts: the latter is
 * warned about here, while the layer-named refusal stays with `models` and
 * `models validate`.
 */
...
/**
 * The alias dictionaries of the harnesses this workspace declared, keyed by
 * harness.
 *
 * Through the registry (`driverByHarness`), never by importing a driver: the
 * boundary gate refuses a mechanism module that reaches past the map
 * (promptobus-adapter.test.mjs), and this is the same handover
 * `options.defaultModel` already makes. A driver that publishes no aliases
 * contributes nothing, which is the honest answer for a harness whose model
 * names are all full names.
 */
function aliasTables(host) {
  ...
  for (const harness of host.declaredTools()) {
    ...
    try {
      ...
      // A declared harness with no driver in the map is not this command's
      // refusal: the workspace names it, nothing can run it, and its records
      // simply resolve no alias.
      continue;
    }
...
/**
 * The layer ADR-005 lets `--write` touch: the one whose id is `user`.
 *
 * Named by id and not taken from `writable`, and the difference matters. The
 * writable layer is the WORKSPACE one (`models strategy` writes there), and a
 * rating is not a property of one workspace — the same account runs the same
 * models everywhere. ADR-005 supersedes exactly one sentence of ADR-004's host
 * contract for exactly this command, and naming the id here is what keeps that
 * exception as narrow as the ADR says it is.
 */
...
/**
 * `models calibrate [--json] [--write] [--yes]`.
 *
 * Three steps, and they are separate on purpose: read the file, ask the pure
 * function ([calibrate.js](model-routing/calibrate.js)), and only then — with an
 * explicit agreement in hand — merge. The proposal a person reads and the block
 * `--write` merges are the same object, so what was agreed to is what lands.
 *
 * The confirmation is a real one. With a terminal the command asks and takes
 * `y`; without one it refuses unless `--yes` was passed, which is ADR-005's
 * option 5B — an automation that already has a person's agreement can record it,
 * and a wrapper that merely forwarded `--write` cannot.
 */
async function calibrateCommand(host, {
  json, write, yes, catalogFile, now, output, ask, stdin,
}) {
  // `--yes` is the record of an agreement to a WRITE. On its own it agrees to
  // nothing, and silently ignoring it would let `calibrate --yes` read as
  // "applied" in a script that lost its `--write` somewhere.
  if (yes && !write) {
  ...
  // The SHIPPED catalog and the merged stack, separately and on purpose. Every
  // band is compared against the shipped one; the merged one is read to show a
  // person the override they already have. Comparing against the merged stack
  // would make their own override the base of the next comparison, and a second
  // run on the same records would walk the rating another step (ADR-005).
  ...
  // Text mode shows the complete proposal before it asks for agreement. Keep
  // `--json` as one document: its non-TTY refusal is still raised before that
  // document is emitted.
  if (!json) {
  ...
  // Read, merge, write — and only `ratings`. Every other key of the file is the
  // person's, and a tuple they already overrode by hand keeps whatever they did
  // NOT calibrate: the merge is per tuple and per rating, not a replacement of
  // the block.
  let applied = false;
  if (write && agreed && moved) {
    ...
    // `schemaVersion` 2 is not a courtesy here: an overlay that carries a value
    // on the rating scale at version 1 is refused by load and by validate
    // (ADR-005), so writing the block without raising the version would leave a
    // person with a routing stack that no longer loads.
  ...
  // One document on stdout under `--json`, and nothing else: the write outcome
  // is a FIELD of it rather than prose printed after it, because `ok` and `info`
  // go to the console and not to the command's output stream, so a machine
  // reading stdout would get the document with a sentence glued to its end.
  if (json) {
 ...
 * Why the block is assembled here and not in the resolver: [guides/model-routing.md#availabilityof--promptobus-models--what-the-resolver-would-pick-right-now](../docs/guides/model-routing.md#availabilityof--promptobus-models--what-the-resolver-would-pick-right-now) */
...
export async function models(rootOrHost, {
  ...
  write = false,
  yes = false,
  // The confirmation seam of `calibrate --write`, and the stream its TTY is read
  // from. Both are arguments so the suite can drive the agreed path and the
  // refused one without a terminal — the same reason `adapterFor` is one.
  ask = defaultAsk,
  stdin = process.stdin,
  catalogFile = CATALOG_FILE,
  adapterFor = adapterOf,
