...
import {
  ...
  sessionIdentity, slugify, stampSnapshot, taskExists, unreadNote, upsertParticipant, watchParticipant,
  ORCHESTRATOR,
} from './store.js';
import {
...
// driver tool stripping holds the file/command boundary, and a driver-specific MCP
// deny can hold the classified external write boundary.
//
...
// The host names MCP writes in canonical pairs; only a driver that declares a
// mechanical MCP deny translates and adds them. The reserved bus is deliberately
// left available for mailbox reads and result delivery. `complete` distinguishes an
// empty classification from a host that has not inspected its external servers.
...
export function planReview(rootOrHost, { target, base, task, title, model, effort: effortOpt, permissionMode: permissionOpt, harness, strategy = undefined, routing = null, dryRun } = {}) {
  ...
  // Git reads are checked separately: the next one depends on the previous. The
  // patch, stat and raw records come from one diff queue; the comparison after it
  // reads only the captured HEAD commit, never the moving tracked tree again.
  ...
  // The diff file is a ONE-TIME snapshot: it is written once, and the worker goes
  // on committing while the reviewer reads — the PB-31 reviewer reported findings
  // that were already closed. There is nothing to fix in the file itself; what the
  // reviewer needs is its age and whether the tracked tree matched HEAD. The moment
  // and worktree HEAD (`head`, read just above for the base) sit beside the tree state
  // returned by the diff pass. The reviewer gets all three in the prompt, a person
  // gets them in command output, and the journal keeps them on the reviewer's record
  // so `status` and `promptobus_task` answer without opening the file.
  const snapshot = {
  ...
  const prompt = buildPrompt({ taskId, nsPath, repoDir, address, diffPath, stat, untracked, baseLine, snapshot, gateRecords, rules, skill, driver, host });
  const reReview = buildReReview({ diffPath, repoDir, stat, untracked, baseLine, snapshot, gateRecords });
  ...
  return {
    home, taskId, createNew, otherActive, slug, address, name, nsPath, repoDir, baseRef, baseLine, snapshot, gateRecords, owner, warnings,
    diff, untracked, stat,
...
/**
 * Where the reviewer actually sits. Printed on the real lift as well as in `--dry-run`,
 * and for the same reason: the `reviewer … started in <repo>` line names the repository
 * under review, which for these harnesses is NOT the session's working directory.
 *
 * The reviewer has a directory of its own only where the driver chose one: a harness
 * without settings for a single lift reads them from the working directory, and seating
 * it in the reviewed clone would write configs into a foreign tree. One function for both
...
export async function review(rootOrHost, opts) {
  ...
  // The strategy is resolved before the gate, because ADR-004 gives it a second
  // source: `--strategy` first, then the merged `defaults.strategy`, then
  // nothing — and nothing is the legacy path, unchanged. Reading the default
  // costs the overlay files and no probe.
  ...
  // reviewer works with the OLD config, and the lines would describe a set that
  // is not there.
  if (!plan.reuse) {
    sayMcp(plan);
    sayReviewerHome(plan);
    // Named on the real lift and not only in `--dry-run`: the canon travels as FILES into
    // the participant's working directory, so whether it arrived is a fact about this
    // lift, and the operator has no other way to see it.
    info(`workspace skills: ${skillsNote(plan)}`);
    info(`reviewer rules:`);
    for (const f of plan.rules) console.log(`  ${f}`);
  }
  ...
  if (plan.reuse) {
    ...
    // The record now holds THIS snapshot: a re-review does not rewrite the participant
    // (a live reviewer only gets a message), and without the patch `status` would keep
    // showing the first snapshot for the whole review — the very lag this answers.
...
// directory or as a reviewer's literal repository directory. Belonging is asked
// of the journal, not of the directory name or the branch: names are built from
// a template and match by chance. We do not check `repo` — the field is also on
// the reviewer record, and the target would be listed in every task where the
// repository was ever reviewed; `repoAbs` is the reviewer's exact directory.
...
// Questions to git about the diff base use the shared low-level helper, not through
// `git()`/`gitRaw()`: those take the whole plan with a refusal, and here a refusal is a
// legal answer.
...
// Gate records a worker attached, resolved rather than described. The files folder holds
// every worker's files and every review round's — a reviewer told "a JSON file" finds many.
function gateRecordPaths(home, taskId) {
  const dir = filesDir(home, taskId);
  const re = new RegExp(`^${GATE_RECORD_STEM}-[^.]*\\.json$`);
  try {
...
function retargetDiff(plan, diffPath) {
  const { taskId, nsPath, repoDir, address, stat, untracked, baseLine, snapshot, gateRecords, rules, skill, driver, host } = plan;
  plan.diffPath = diffPath;
  plan.prompt = buildPrompt({ taskId, nsPath, repoDir, address, diffPath, stat, untracked, baseLine, snapshot, gateRecords, rules, skill, driver, host });
  plan.reReview = buildReReview({ diffPath, repoDir, stat, untracked, baseLine, snapshot, gateRecords });
}
...
function subject({ diffPath, repoDir, stat, untracked, baseLine, snapshot, gateRecords }) {
...
function buildPrompt({ taskId, nsPath, repoDir, address, diffPath, stat, untracked, baseLine, snapshot, gateRecords, rules, skill, driver, host }) {
...
${subject({ diffPath, repoDir, stat, untracked, baseLine, snapshot, gateRecords })}
...
- **Gate** — always "not run, because a reviewer runs nothing", plus your verdict on the author's gate record. The line is never omitted: an omitted line reads as a green one.
...
The header is at most ${RESULT_BODY_MAX} characters; the findings follow it in the same body, and they are not bounded. This reviewer result has no artifact filename to quote: **You have no artifact to move them into** — file writes are disabled for you by the mechanism, and the report leaves by one channel. That is the one place where your hand-off differs from a worker's, whose overflow does go into an attached file.
...
function buildReReview({ diffPath, repoDir, stat, untracked, baseLine, snapshot, gateRecords }) {
...
// The ordinary output ceiling is shared with the zone walk (`PROC_MAX_OUTPUT`, util.js),
// and both wrappers also have the shared git wait ceiling. A timeout or output overflow
// leaves no status, so the `error` branch names it. Outcome: `{ out }` — what was read,
// `{ refusal }` — the refusal text the command prints.
...
// One git diff invocation produces the review patch, stat and raw records for the
// same tracked-tree reads. Comparing those records with the immutable base-to-HEAD
// diff identifies tracked paths whose working state differs from HEAD without
// opening the tracked tree a second time — the gap between two reads is the race
// this fact exists to expose. Untracked paths are listed separately by `ls-files`.
function snapshotDiff(repoDir, diffFrom, head) {
  // A stat-only change can leave the worktree side of a raw record at 0000000 even
  // when the file still has the same content as HEAD. Refresh the index stat data
  // before the snapshot read so that comparison is about content, not mtime.
