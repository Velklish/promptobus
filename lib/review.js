...
export function planReview(rootOrHost, { target, base, task, title, model, effort: effortOpt, permissionMode: permissionOpt, harness, dryRun } = {}) {
  ...
  // Reviewer slug comes from the review subject, not from the clone root: with
  // several workers of one repository `basename` would give one address, and
  // review of the second would go to the live reviewer of the first as a
  // re-review. There is no flag on purpose.
...
function missingTarget(host, { task, title, base, model, effort, harness, dryRun }) {
  ...
  const cwdPair = here.nsPath.split('/').length < 2
    ...
    : null;
  if (cwdPair) {
    return `${head}\n    The repository of the current directory is ${here.dir}, but it ${cwdPair}.${ask}`;
  }
...
// Root of the clone the target sits in, and its namespace path under repos/. We
// descend from repos/ by path parts to the first directory with `.git`. Taking
// the second path segment is wrong: a namespace can be deeper than two
// segments, and the target can be a worktree.
...
// Diff file name by slug and number — one home for the convention on predict and on write.
...
// and the write a second review of the same slug would slip in. The `wx` flag
// refuses on an existing file — a signal to take the next number. Returns the
// path that LANDED on disk. Exported for the test: a race cannot be reproduced
