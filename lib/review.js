...
import { ok, info, warn, fail, GIT_MAX_OUTPUT } from './copy/util.js';
import { normalize } from './copy/fuzzy.js';
...
import { hasFeature, openParticipant } from '../../packages/promptobus/dist/index.js';
import { driverOf, REGISTRY } from './drivers.js';
import { ensureWarden } from './warden.js';

// Reviewer — участник шины Promptobus (ADR-021): фоновая сессия harness'а в директории
...
//
// **Read-only — не пожелание, а capability** (`BL-466`, ADR-034). Harness, не умеющий
...
export function planReview(rootOrHost, { target, base, task, title, model, effort: effortOpt, permissionMode: permissionOpt, harness, dryRun } = {}) {
  ...
  // Driver ПОДЪЁМА — тот, которым человек просит поднять сессию (`--harness`, `BL-468`).
  ...
  // harness каталог скиллов, — его словарь (`BL-468`).
  ...
  // в файле настроек; флаг (`BL-461`) — на один подъём, как `--effort`.
  ...
  // Каталог скиллов берёт не всякий harness — тем же правилом, что у worker'а (`BL-468`).
  ...
  // хук рабочего места не доезжает (`BL-426`). Permission-mode дефолтный: чтение не
  // спрашивают, и флага здесь нет вовсе.
  // Модель без флага — умолчание driver'а (`BL-468`), как у worker'а.
  ...
  return {
    ...
    // аргументами команды Stop-хука в его файле настроек (`BL-469`).
...
export async function review(rootOrHost, opts) {
  ...
  if (opts.dryRun) {
    ...
    // Своё рабочее место reviewer'а — только там, где driver его выбрал (`BL-468`): harness
    ...
    // Та же строка и по той же причине, что у spawn'а (`BL-484`): имя сессии, которое
  ...
  const { record } = openParticipant(plan.home, plan.taskId, participantRecord(plan.address, {
    // Harness записи — тот, которым её поднимают, тем же правилом, что у worker'а (`BL-468`).
  ...
  info(`новый дифф этому же reviewer'у: ati-agents promptobus review "${plan.repoDir}" --task ${plan.taskId}`);
  warnUnread(plan);
  return plan;
}
...
function deadSessionNote(plan) {
  return `${keptNote(plan)} Поднимай reviewer'а заново: ati-agents promptobus review "${plan.repoDir}" --task ${plan.taskId}.`
    + ' Сообщений от этого адреса не будет — ждать их бессмысленно.'
    + (plan.createNew ? ` От ревью отказался — закрой задачу: ati-agents promptobus done --task ${plan.taskId}` : '');
}
...
function launchFailureNote(plan) {
  ...
  return plan.createNew
    ? `${kept} Это активная задача-сирота: reviewer не поднят, и следующий вызов без --task её не подхватит. `
      + `Закрой её: ati-agents promptobus done --task ${plan.taskId}`
...
function warnSecondTask({ createNew, otherActive, taskId }) {
  ...
  info(`закончишь ревью — закрой его задачу: ati-agents promptobus done --task ${taskId}`);
}
...
function missingTarget(host, { task, title, base, model, effort, harness, dryRun }) {
  ...
  const flags = [
    ...
    // Инструмент подъёма едет в подсказку наравне с остальными флагами (`BL-468`): повтор
