...
export function planReview(rootOrHost, { target, base, task, title, model, effort: effortOpt, permissionMode: permissionOpt, harness, strategy = undefined, routing = null, dryRun } = {}) {
  ...
  // reviewer needs is to know its age. Both numbers are taken here, next to the
  // read: the moment, and the worktree HEAD the diff was taken from (`head`, read
  // just above for the base — one read, and a second one would only move the race
  // window to the other side of the diff). The reviewer gets them in the prompt,
  // a person gets them in the command output, and the journal keeps them on the
  // reviewer's record so `status` and `promptobus_task` answer "how far behind"
...
export async function review(rootOrHost, opts) {
  ...
  const { record } = openParticipant(plan.home, plan.taskId, participantRecord(plan.address, {
    ...
    // Which snapshot this reviewer holds — the same pair `stampSnapshot` patches on a
    // re-review.
...
The file is a SNAPSHOT taken at ${snapshot.at} against that base${snapshot.head ? `, from worktree HEAD ${snapshot.head}` : ''} — it is written once and never updated, while the author goes on committing. Everything committed after that moment is missing from it. The current state is the working copy ${repoDir} itself: before reporting a finding, open the file there and check that the line is still as the snapshot shows it — a review of a stale file reports closed findings as open. A fresh diff file comes as a re-review; ask the orchestrator for one, the same command re-snapshots.
Change summary:
${stat || '(new files only)'}
