...
// left available for mailbox reads and result delivery.
function reviewerDenyTools(host, driver) {
  const builtIn = driver.options.denyTools ?? [];
  if (!hasFeature(driver, 'mcpDenyTools')) return builtIn;
  const classified = typeof host.participantDenyTools === 'function'
    ? host.participantDenyTools('reviewer').filter(({ server }) => server !== PROMPTOBUS_SERVER)
    : [];
  if (!classified.length) return builtIn;
  return [...builtIn, ...driver.mcpDenyTools(classified)];
}
...
export function planReview(rootOrHost, { target, base, task, title, model, effort: effortOpt, permissionMode: permissionOpt, harness, strategy = undefined, routing = null, dryRun } = {}) {
  ...
  const denyTools = reviewerDenyTools(host, driver);
...
function buildPrompt({ taskId, nsPath, repoDir, address, diffPath, stat, untracked, baseLine, snapshot, rules, skill, driver, host }) {
  ...
  const mcpBoundary = hasFeature(driver, 'mcpDenyTools')
    ? '**Host-classified MCP write tools are mechanically denied by the harness; the remaining external MCP surface is read-only under this prompt, the second layer.**'
    : '**MCP tools of external systems are read-only under a prompt-only guard; this harness does not mechanically deny MCP writes.**';
