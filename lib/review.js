...
export function planReview(rootOrHost, { target, base, task, title, model, effort: effortOpt, permissionMode: permissionOpt, harness, strategy = undefined, routing = null, dryRun } = {}) {
  ...
  const prompt = buildPrompt({ taskId, nsPath, repoDir, address, diffPath, stat, untracked, baseLine, snapshot, rules, skill, driver, host });
  const reReview = buildReReview({ diffPath, repoDir, stat, untracked, baseLine, snapshot });
  ...
  return {
    home, taskId, createNew, otherActive, slug, address, name, nsPath, repoDir, baseRef, baseLine, snapshot, owner, warnings,
    diff, untracked, stat,
...
function retargetDiff(plan, diffPath) {
  const { taskId, nsPath, repoDir, address, stat, untracked, baseLine, snapshot, rules, skill, driver, host } = plan;
  plan.diffPath = diffPath;
  plan.prompt = buildPrompt({ taskId, nsPath, repoDir, address, diffPath, stat, untracked, baseLine, snapshot, rules, skill, driver, host });
  plan.reReview = buildReReview({ diffPath, repoDir, stat, untracked, baseLine, snapshot });
}

function subject({ diffPath, repoDir, stat, untracked, baseLine, snapshot }) {
...
function buildPrompt({ taskId, nsPath, repoDir, address, diffPath, stat, untracked, baseLine, snapshot, rules, skill, driver, host }) {
...
${subject({ diffPath, repoDir, stat, untracked, baseLine, snapshot })}
...
function buildReReview({ diffPath, repoDir, stat, untracked, baseLine, snapshot }) {
