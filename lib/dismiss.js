...
import {
  addressesOf, dismissParticipant, ORCHESTRATOR, ownerRoute, ownership, readTask,
  resolveTaskId, sessionIdentity, unprovenOwnerLine,
} from './store.js';
...
export function dismiss(rootOrHost, { task, address } = {}) {
  ...
  // Same owner gate as `promptobus done`: the reports this stops go to the mailbox owner,
  // and the right is proven rather than assumed — 03-cli § The owner gate.
  ...
  if (!own.allowed) {
    fail(`${unprovenOwnerLine(home, readTask(home, id), own)}: a participant is dismissed by the task mailbox owner. `
      + 'Reports about them go to that owner, and a foreign hand would stop them for the wrong recipient. '
      + `${ownerRoute(home, readTask(home, id), own, host.busCommand(['dismiss']))}`);
  }
