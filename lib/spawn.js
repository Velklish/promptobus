...
The whole body is at most ${RESULT_BODY_MAX} characters. Everything that does not fit — evidence, measurements, the reasoning behind decisions you took yourself — goes into a file attached with artifactPath, and the header's last line names that file. The bound is measured, not chosen: headers of this shape run 1000 to 2100 characters, a result body on this bus runs to a median of 5254, and the orchestrator reads every one of them in sequence to find those four facts.

Your gate run travels as a record, not as a sentence, and the reviewer cannot run anything to check it. Write the record as JSON to the shape of \`${GATE_RECORD_SCHEMA}\` in the promptobus package — one entry per gate command, carrying the command as executed, its exit code, the counts the runner printed, the sha of \`git rev-parse HEAD\` it ran on, whether tracked files were dirty, the timestamp and your address — and send it BEFORE the result as its own type=artifact message. **Name the file \`${GATE_RECORD_STEM}-<your worker slug>.json\`**: that is the name the reviewer's prompt resolves, and a file named anything else is never found. The Gate line of the header names it too. The reviewer compares that sha with the tree it is reviewing, so a record made on another sha is not evidence about this one. What the record does is carry your claim into machine form and bind it to one tree; it cannot prove the run happened, and only a re-run on the same sha by someone allowed to run can.${driver.phrases.promptRules}`;
}
