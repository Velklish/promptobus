...
import { guardHookCommand } from '../../packages/promptobus/dist/hooks.js';
import {
...
import { openParticipant } from '../../packages/promptobus/dist/index.js';
...
// через собственный конфиг MCP (ADR-016). Конфиг шины лежит в сторе задачи и уезжает
...
//
// **Ни argv, ни имени harness'а здесь нет** (`BL-466`, ADR-034). Этот файл называет ПРЕДМЕТ
...
// Имя записи шины в конфиге участника — из своего дома: его читает и хук шины (bushook.js).
...
// (ADR-032, §3): descriptor уезжает к нему, и он же переводит его в свой конфиг.
export function participantMcp(rootOrHost, { address, taskId, home }, driver) {
  ...
  const mcpServers = {
    ...
    [PROMPTOBUS_SERVER]: {
      ...
      args: [host.binPath(), 'promptobus', 'mcp'],
...
/**
 * Файлы, которые driver просит положить рядом с сессией до подъёма (ADR-034). Порядок —
...
// Окружение фоновых сессий шины. `CONTEXT_STORE_STOP_GATE=0` — штатный рычаг хука памяти
// (fail-open): его протокол требует показать кандидата факта ЧЕЛОВЕКУ до save_fact, а за
// фоновой сессией человека нет — без рычага она висла бы в blocked.
...
// свойство harness'а, и у второго они свои (`BL-466`). Рычаг памяти — `host.extraEnv()`
// (у ATI `CONTEXT_STORE_STOP_GATE=0` в ati-host).
//
// **Идентичности шины здесь нет** (`BL-469`). До неё тройку `PROMPTOBUS_*` клали в окружение
// поднимаемой сессии ради Stop-хука (`BL-426`), и предпосылка «окружение сессии кладёт
...
// ([guardhook.js](../guardhook.js)), а окружение сессии её не несёт вовсе — класть туда
...
// Имя сервера памяти в промптах участников: правило памяти называет его инструменты, а
// написание имени инструмента — свойство harness'а (`BL-468`). Дом — ati-host: вторая
// копия в spawn.js разошлась бы с текстом memorySection.
export { MEMORY_SERVER } from './ati-host.js';
...
// слова отказа принадлежат harness'у (`BL-466`). Гейт общий у worker'а и reviewer'а:
...
/**
 * Driver ПОДЪЁМА по флагу `--harness` (`BL-468`). Два гейта, и оба до всякой записи на диск.
 *
 ...
 *
 * Второй — декларация рабочего места: harness, которого нет в `.agents/tools.json`, не
...
// Режим прав сессии участника (`BL-461`): значение флага сверяется с перечнем бинаря до
...
function buildPrompt({ taskId, nsPath, brief, rules, branch, driver, host }) {
  // Имена инструментов шины — написанием ЭТОГО harness'а (`BL-468`): участник ищет их по
...
export async function planSpawn(rootOrHost, opts) {
  ...
  // harness называет человек флагом `--harness` (`BL-468`), без флага берётся прежний; у
  ...
  // Заголовок куска работы — из НОВОГО брифа, и для существующего адреса тоже (`BL-453`):
  ...
  if (existing) {
    if (explicitTaskTitle) {
      ...
      // или перезапуск с другим брифом (`BL-453`). Два случая он этим и обходит. Штатный
      ...
      // беда, от которой уходила `BL-197`.
      //
      ...
      // новая, и предсказание с обеими строками разошлось бы с живым прогоном (`BL-453`).
  ...
  // Имя сессии пересчитывается и для существующего адреса (`BL-453`): живого участника
  ...
  // Модель без флага — умолчание driver'а (`BL-468`): имя модели принадлежит harness'у
  ...
  // только команда сторожа цикла (`BL-426`, `BL-463`) и ключи рабочего места, адресованные
  ...
  // настроек — его дело (ADR-032, §3; ADR-034), и вариадические опции бинаря, из-за которых
  ...
  // его на один spawn (`BL-461`): track'у со стендом вне worktree auto иначе спрашивал бы
...
// показывать то, что уедет на диск, а идентичность участника уезжает именно сюда (`BL-469`).
...
// Имя бинаря называет driver: своего у команды нет (`BL-466`).
...
export async function spawn(rootOrHost, opts) {
  ...
  if (opts.dryRun) {
    ...
    // Чего в этом выводе НЕТ и почему (`BL-484`): у harness'а, который придумывает имя
  ...
  // Привязка «сессия → задача» (ADR-027): `promptobus spawn` запускается Bash'ем из сессии
  ...
  const { record } = openParticipant(plan.home, plan.taskId, participantRecord(plan.address, {
    // Harness записи — тот, которым её и поднимают (`BL-468`). Без него registry приписал бы
  ...
  const { output, session, seen } = await plan.driver.spawn(plan.launch, {
    tool,
    // Адрес, задача и дом шины уезжают driver'у ВМЕСТЕ с планом (`BL-324`): у harness'а,
  ...
  ensureWarden(plan.home, plan.taskId);
