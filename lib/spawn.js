...
import { openParticipant, ROUTING_FIELD } from '../dist/index.js';
...
export function liftHarness(rootOrHost, harness = null) {
  ...
  if (!tools.includes(lifter.id)) {
    throw new GateError(`--harness ${lifter.id}: tool is not declared in ${host.toolsManifestRel()} of the workspace `
      ...
      + `would have no workspace rules or skills. Declare it: ${host.formatCommand(['tools', 'add', lifter.id])}, `
      + `then ${host.syncHint()}`);
  }
  return lifter;
}
...
export async function spawn(rootOrHost, opts) {
  ...
  // The participant goes into the journal BEFORE dependency install and the `claude`
  // start: refusal branches leave through `fail()`, and Ctrl+C on a long `npm ci` would
  // otherwise leave a directory with no record — a repeat hits "directory is taken, and
  // the journal has none". A repeat recognizes its own directory by `worktreeName`.
