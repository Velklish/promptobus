...
import {
  ok, info, warn, fail, runLogged, runProc, shellQuote,
  PROC_MAX_OUTPUT, GIT_NET_TIMEOUT_MS, PROC_INSTALL_TIMEOUT_MS, toPosix,
} from './util.js';
...
import { clearThroughputSidecar } from './model-routing/telemetry.js';
...
// `skillOverrides` is the one workspace setting copied to the participant: it suppresses
// personal file copies of same-named skills, which themselves travel as the plugin directory.
...
export function participantPluginDir(rootOrHost) {
  ...
  // Look at the host-declared manifest, not the directory: the manifest is what
  // makes a directory a plugin.
  const manifest = path.join(host.workspaceRoot(), host.pluginManifestRel());
  if (existsSync(manifest)) return dir;
  warn(`${manifest}: plugin directory is missing or has no manifest`
...
/**
 * Paths the lift is about to overwrite and Git already tracks.
 *
 * The rule is the hazard, not one harness: launch files land in a directory the driver
 * claims inside the participant's working directory, and a `copyFrom` entry WIPES its
 * destination before copying. Whatever the repository kept there is gone, the branch is
 * dirty from its first second, and `done` will not sweep the directory.
 *
 * Git is asked about **the paths the lift will write**, not about the claimed directory
 * whole. The difference is a false warning that would fire on a lawful repository: a
 * project may keep its own `.codex/config.toml` or `.cursor/rules` under a claimed
 * directory — the trust record of [ADR-007](../docs/adr/adr-007-codex-participant-isolated-home.md)
 * exists so a worker READS exactly that — and no lift touches those files. Asking about
 * the directory would have named them on every lift and called them about to be
 * overwritten, which is false twice over.
 *
 * The claimed directory (`options.launchDirs`) is still what says where a launch file's
 * path stops being the working directory and starts being the relative path Git is asked
 * about. A driver whose launch files land outside the repository — Claude Code writes
 * them to the task store — claims none, and nothing is asked. A directory handed to
 * `git ls-files` lists everything tracked under it, which is right: a copy destination is
 * the one path where the whole subtree goes.
 */
function warnTrackedLaunchDirs(files, launchDirs) {
  // Working directory → the paths, relative to it, that this lift writes into a claimed
  // directory. The copy destination is one of them, so the wipe is noticed on that path
  // alone and does not depend on a sibling file being written beside it.
  const claims = new Map();
  for (const claimed of launchDirs ?? []) {
    const inside = `${path.sep}${claimed}${path.sep}`;
    for (const file of files) {
      const at = file.path.indexOf(inside);
      if (at < 0) continue;
      const dir = file.path.slice(0, at);
      const written = claims.get(dir) ?? new Set();
      // Git pathspecs are POSIX whatever the platform separator is.
      written.add(toPosix(file.path.slice(at + 1)));
      claims.set(dir, written);
    }
  }
  for (const [dir, written] of claims) {
    // Go through runProc so the launch is bounded and the sealed-PATH execution trace
    // sees the same git read as every other process at this boundary.
    const listed = runProc('git', ['-c', 'core.quotePath=false', '-C', dir, 'ls-files', '--', ...written], {
      encoding: 'utf8', timeout: GIT_NET_TIMEOUT_MS, maxBuffer: PROC_MAX_OUTPUT,
    });
 ...
 *
 * `launchDirs` is the driver's own `options.launchDirs` — the caller hands it over rather
 * than the whole driver, because this function writes files and asks nothing else of the
 * harness.
 */
export function writeLaunchFiles(files, launchDirs = []) {
  warnTrackedLaunchDirs(files, launchDirs);
  for (const file of files) {
 ...
 */
export function toolName(driver, server, name, host) {
  return driver.phrases.tool(server, name, host);
}
...
export function memoryRule(driver, host) {
  return host.memorySection((server, name) => toolName(driver, server, name, host)) ?? '';
}
...
export function optionRefusal(driver, effort, tool, host = null) {
  const statusCommand = host?.busCommand?.(['status']) ?? null;
  return driver.optionRefusal({ effort, statusCommand }, tool);
}

export function normalizeLaunchTool(driver, tool, env) {
  return typeof driver?.normalizeTool === 'function'
    ? driver.normalizeTool(tool, { env })
    : tool;
}

// One read per lift, and only when this driver declared it. A filled version is kept.
export function withToolVersion(driver, host, tool) {
  if (typeof tool?.version === 'string' && tool.version.trim()) return tool;
  if (driver?.options?.readsVersion !== true) return tool;
  if (!tool?.bin || typeof host?.readToolVersion !== 'function') return tool;
  let version = null;
  try {
    version = host.readToolVersion(driver.options.tool, tool.bin);
  } catch {
    return tool;
  }
  if (typeof version !== 'string' || !version.trim()) return tool;
  return { ...tool, version: version.trim() };
}
...
export function liftHarness(rootOrHost, harness = null) {
  ...
  if (!tools.includes(lifter.id)) {
    throw new GateError(`--harness ${lifter.id}: tool is not declared in ${host.toolsManifestRel()} of the workspace `
      ...
      + `would have no workspace rules or skills. A successful install does not satisfy this: it writes `
      + `"harnesses", which records where hooks went, and never "tools", which is this allow-list. Declaring `
      + `a harness is a hand edit of that file: add "${lifter.id}" to its "tools" array, then ${host.syncHint()}`);
  }
  return lifter;
}
...
// Role-prefixed sidecars share the worker namespace; reserve those prefixes so two
// addresses cannot overwrite one participant config.
const RESERVED_WORKER_PREFIXES = ['reviewer', 'approver'];

function refuseParticipantPrefix(slug, route) {
  const role = RESERVED_WORKER_PREFIXES.find((candidate) => slug.startsWith(`${candidate}-`));
  if (!role) return slug;
  throw new GateError(`the worker name "${slug}" starts with "${role}-", and that prefix is taken by the ${role}: `
    + `its files in workers/ are named the same (${role}-<name>.mcp.json), and one of the two `
...
function workerSlug(home, taskId, nsPath, explicit) {
  ...
  if (explicit) {
    ...
    return refuseParticipantPrefix(named, 'Pick another --worker.');
  }
  ...
  const base = refuseParticipantPrefix(slugify(path.basename(nsPath), Number.MAX_SAFE_INTEGER) || 'repo',
...
export function sessionName(task, { slug, reviewer = false, approver = false, taken = [], title } = {}) {
  const head = approver ? 'Accept:' : reviewer ? 'Review:' : 'Worker:';
...
function buildPrompt({ taskId, nsPath, brief, rules, branch, worktreePath, ruleDirs, driver, host, repoSkills, worktreeDeps, address }) {
  ...
  const roots = [worktreePath, ...ruleDirs];
  return `${host.workerPreamble({ taskId, nsPath, branch, worktreePath })}${host.liveRunNote(nsPath)}

Allowed roots (session cwd and addDirs): ${roots.join(', ')}. The first root is your worktree; make repository edits there.
...
8. Do not end a turn having sent nothing since the last thing that asked you for an answer. Types task, question, review and result ask for one; status, answer and artifact do not. Nothing to report yet — that is itself a status: what you are doing and how long it will take.
9. The assignment is the source of explicit requests, not wishes: if it asks to commit on your branch — commit at once, do not wait for a separate confirmation on the bus. What stays forbidden is what the assignment does not lift: push, and any edits to the repository main tree. Do not move the branch of your own will: the orchestrator takes the result from this worktree's branch. If the assignment explicitly asks for another branch — create it, but in the same turn tell the orchestrator with a status which branch now carries the result. A silently changed branch sends its publication into a void: it will publish the one spawn created, and that one will still sit on the original commit.

## Hand-off form

A result opens with four lines, in this order, and nothing above them:

- **Done** — what was done, and the commits that carry it.
- **Gate** — the gate command as executed, its exit code, and the counts it printed. A gate you did NOT run gets this line too, as "not run, because …": an omitted line reads as a green one.
- **Open** — what is left unclosed and where you worked around a problem. This line matters most.
- **Decide** — what needs a decision from the orchestrator, or "nothing".

The whole body is at most ${RESULT_BODY_MAX} characters. Everything that does not fit — evidence, measurements, the reasoning behind decisions you took yourself — goes into a file sent first with artifactPath as its own type=artifact message. Read the landed filename from the immediate bus reply to your own \`${bus('promptobus_send')}\` call for that artifact — not from the path you passed and not from the orchestrator — and, without ending the turn, put that returned name in the result header's last line before sending the result. The bound is measured, not chosen: headers of this shape run 1000 to 2100 characters, a result body on this bus runs to a median of 5254, and the orchestrator reads every one of them in sequence to find those four facts.

${ATTACHMENT_CONTRACT} So everything you attached as an artifact before \`review\` ran, the reviewer sees in its subject as a list — evidence you attached is evidence it has, and re-sending a file because a report said it saw none costs a round for nothing. What stays outside that list is what you never attached: a probe you only retell is not on record.

Your gate run travels as a record, not as a sentence, and the reviewer cannot run anything to check it. Write the record as JSON to the shape of \`${GATE_RECORD_SCHEMA}\` in the promptobus package — one entry per gate command, carrying the command as executed, its exit code, the counts the runner printed, the sha of \`git rev-parse HEAD\` it ran on, whether tracked files were dirty, the timestamp and your address — and send it BEFORE the result as its own type=artifact message. Read the landed filename from the immediate bus reply to your own \`${bus('promptobus_send')}\` call for that record — not from the path passed to that call and not from the orchestrator — and, without ending the turn, use that returned name in the Gate line. **Request the file stem \`${GATE_RECORD_STEM}-<your worker slug>.json\`**: the reviewer's prompt resolves landed names with that stem, while the bus may append a number to avoid a collision. The reviewer compares that sha with the tree it is reviewing, so a record made on another sha is not evidence about this one. What the record does is carry your claim into machine form and bind it to one tree; it cannot prove the run happened, and only a re-run on the same sha by someone allowed to run can. **The bus checks the record against that schema when you send it** and refuses an invalid one with the faults listed by field, so a document nobody could read as evidence does not reach the reviewer looking like one. A card's own verification run is an entry of this same record with \`kind\` \`verification\`. ${GATE_LINE_COUNTS} ${GATE_LINE_VERIFICATION} The command stays the command as executed: \`kind\` is the whole marker, and the handover record does not gain a check for it. The mutation probe is not an entry here — a probe is not a gate command; its place is the \`mutationProbe\` check of the record below.

The gate record says WHAT you ran; it does not say why that run means anything. That is the second record, and you send it the same way: JSON to the shape of \`${HANDOVER_RECORD_SCHEMA}\`, as its own type=artifact message before the result, requesting the file stem \`${HANDOVER_RECORD_STEM}-<your worker slug>.json\`, with the landed name read from the bus reply to your own \`${bus('promptobus_send')}\` call and quoted in the **Gate** line after the gate record's. This record is checked against its schema at send too, and an invalid one is refused the same way. It carries the branch sha — what \`git rev-parse HEAD\` prints, at full length — the base sha you compared against, read the same way, and five checks, each of which the reviewer can read without running anything:

1. **The difference of verdict NAMES between the base commit and your branch** — every name that was on base and is not on the branch, each with why it went, or the explicit empty list. Counts are not this check: a suite that goes 244 to 242 to 246 reads as growth while a guarantee is gone, and a removed test never turns red.
2. **The mutation probe**: the sha it ran on, the \`file:line\` you broke, the exit code **of the run made with the mutation in place** — not of the probe tool around it — how many verdicts passed against the base total, how many the run never reached, and the names that turned red. A run that stopped early, a mutation that never applied, and a probe that reddened nothing are each not a probe — they are declared in the check's \`notRun\` branch or run again. **Name in \`expected\` what you broke the code to redden, written down before the run**, and the bus refuses the record when \`reddened\` does not carry that name: a verdict that turned red for a foreign reason reads exactly like one that turned red for yours, and only you know which you were aiming at. More names in \`reddened\` than in \`expected\` is a wider net, not a fault. From the release that ships \`treeLag\`, a probe kept on an earlier sha than the record's \`tree\` needs \`checks.mutationProbe.treeLag\`: why the commits since then reached neither the mutated file nor the tests whose verdicts are in \`reddened\`. Before that release the running bus refuses the field.
3. **\`git status --porcelain\` at both ends of the probe** — empty before it and empty after the restore. A probe on a dirty tree measured something other than the commit; a dirty tree after it left the mutation where the reviewer reads.
4. **Every red you call environmental, flaky or pre-existing**, each with the assertion in full, the same verdict run on the base commit, and the symmetric difference of the failure sets. Red on your branch and green on base is your own change.
5. **Every gate command you did NOT run**, each with its reason — the machine half of the \`not run, because …\` line.

**A check you leave out is not a check you passed.** Every one of the five is required, and the only way past one is its own \`notRun\` branch naming why it was impossible here. A check honestly declared impossible passes; the form has that branch precisely so it never rewards an invented number. The bus checks the SHAPE of what you send against the schema and refuses a record that does not match it. It does not run these checks for you and does not grade them — running them is yours, judging them is the reviewer's, and the reviewer reads the record against the tree.${driver.phrases.promptRules}`;
}
...
export async function planSpawn(rootOrHost, opts) {
  ...
  if (resolved.group) {
    throw new Error(`${repo} is a group address (${nsPath}), not a repository: a worker needs one. `
      ...
      + `Name one repository of the group (a \`${nsPath}/<name>\` address): a whole group is not a spawn target, and this command does not clone one.`);
  }
  ...
  if (task && readTask(home, task).status === 'done') {
    throw new GateError(`task ${task} is closed — nobody sees a worker lifted in it: `
      + `\`${host.busCommand(['status'])}\` lists active ones, and \`${host.busCommand(['done'])}\` cleanup sweeps worktree directories of every closed task, `
      ...
      + `there is no way to continue work in a closed task: ${host.busCommand(['done'])} has no reverse.`);
  }
  ...
  if (!task && existing) {
    ...
    if (own.gated) {
      throw new GateError(`${foreignTaskLine(readTask(home, existing), own)}: `
        ...
        + `${claimRoute(host.busCommand(['spawn']))} `
        + 'Need a separate run — repeat spawn with --new-task. '
        + `No route fits — the run is finished by its owner: a foreign hand running ${host.busCommand(['done'])} would cut live work.`);
    }
  }
  ...
  if (participant) {
    ...
    if (participantSession(participant, opts.sessions) === 'alive') {
      // The bus stop takes this ONE participant through its driver; `done` would close the task.
      const restart = driver.capabilities.stop
        ? host.busCommand(['stop', address, `--task ${taskId}`])
        : driver.phrases.stop('<id>');
      throw new Error(`task ${taskId} already has ${address} running: session "${was.name}" is alive. `
        ...
        + ` If you need a restart — close the session first: ${restart}.`);
    }
  }
  ...
  // argv, config, and a settings file is its job, and it also knows the binary's
  // prompt representation. Model is opus by default: the user's session default
  ...
  // Assembled through a closure rather than once, because two lines of the preamble are not
  // knowable here: whether the repository's generator laid its process skills out in the
  // worktree, and whether a fresh worktree's dependencies installed. The plan writes nothing
  // to disk and the worktree does not exist yet, so it carries both forecasts and `spawn`
  // rebuilds once with the outcomes. Nothing but the prompt differs between the two, and
  // rebuilding before the launch files are written keeps them written exactly once (PB-8).
  const plannedWorktreeDeps = existsSync(worktreePath)
    ? { ran: false, repeat: true }
    : worktreeHasLock(repoAbs)
      ? { planned: true, command: npmCiCommand() }
      : { ran: false };
  const buildLaunch = (repoSkills, worktreeDeps = plannedWorktreeDeps) => {
    const prompt = buildPrompt({ taskId, nsPath, brief, rules, branch, worktreePath, ruleDirs, driver, host, repoSkills, worktreeDeps, address });
  ...
  const plannedSkills = declaredGenerator
    ...
    : { kind: 'none', argv: null };
  const { prompt, launch } = buildLaunch(plannedSkills, plannedWorktreeDeps);
  // A missing worktree is read as the main checkout. The refusal stands here,
  // before the first write of this lift, and `--dry-run` takes the same plan.
  if (typeof driver.refuseForeignProjectHooks === 'function') {
    driver.refuseForeignProjectHooks(existsSync(worktreePath) ? worktreePath : repoAbs, worktreePath);
  }
  ...
  return {
    ...
    // The repository's generator and dependency install: what was declared, their
    // forecasts, and the door for `spawn` to put the real outcomes into the preamble once
    // the worktree exists.
    repoSkills: plannedSkills, worktreeDeps: plannedWorktreeDeps, rebuild: buildLaunch,
...
export function skillsNote(plan) {
  ...
  return 'not attached — this host ships no workspace-skills plugin';
}
...
export function runRepoGenerator({ worktreePath }, { fresh = true, exec = runProc } = {}) {
  ...
  // The log goes next to the directory, not inside the worker's tree.
  const logPath = `${worktreePath}.generate.log`;
  const result = runLogged(declared.argv, {
    cwd: worktreePath, timeout: PROC_INSTALL_TIMEOUT_MS, logPath, exec,
  });
  // What the generator leaves is checked only after a successful run.
  if (result.status === 0) {
    return { kind: 'ok', argv: declared.argv, ms: result.ms, logPath, dirt: worktreeDirt(worktreePath) };
  }
  return {
    kind: 'failed', argv: declared.argv, why: result.why, code: result.status, ms: result.ms, logPath,
  };
}
...
/**
 * The participant's dependency-install line. A fresh plan carries the lock forecast,
 * while a real spawn replaces it with the install outcome. A repeat carries `repeat`:
 * it deliberately does not rerun or check dependencies in a worktree that may contain
 * the previous session's changes.
 */
export function worktreeDepsLine(state) {
  if (state?.repeat || state?.kind === 'kept') {
    return 'Worktree dependencies were not re-run or checked: this is a repeat spawn into an existing worktree. '
      + `Look before relying on node_modules; run ${npmCiCommand()} yourself if needed.`;
  }
  if (state?.planned || state?.kind === 'planned') {
    return `Worktree dependencies: package-lock.json is present; a fresh spawn will run ${state.command ?? npmCiCommand()} `
      + 'before writing launch files.';
  }
  if (!state?.ran) {
    return 'Worktree dependencies: no package-lock.json was found, so there is nothing to install.';
  }
  const command = state.command ?? npmCiCommand();
  if (state.ok) {
    return `Worktree dependencies were installed in this worktree: ${command} already ran; `
      + 'you do not need to run it again before you work.';
  }
  return `Worktree dependencies were NOT installed: ${state.why ?? 'the install was refused'}; run ${command} by hand `
    + 'before you work, and say so in your first status.';
}
...
// Outcome of install in a just-created worktree. A refusal is a warning with a command,
// not `fail`: spawn of the participant is not rolled back for this.
 ...
 */
export function keepBrief(plan) {
  const stem = plan.briefStem ?? `brief-${plan.slug}`;
  try {
    ok(`brief kept in the task files: ${occupyTaskFile(filesDir(plan.home, plan.taskId), stem, '.md', `${plan.brief}\n`)}`);
  } catch (e) {
...
export async function spawn(rootOrHost, opts) {
  ...
  if (opts.dryRun) {
    ...
    // A driver whose session environment is not merely stripped but REDIRECTED says
    // where to: the line above reports a dropped variable, and on its own that reads
    // as "the harness default will be used". Absent for drivers that only strip.
    ...
    sayModule(plan);
    // Binary version is not asked in `--dry-run`: nothing is started.
    info(dryRunToolNote(plan.driver));
    info('command:');
    // A driver may describe a non-argv prompt channel; ordinary drivers keep the
    // existing runnable argv template.
    const command = plan.launch.dryRunCommand
      ?? `${plan.driver.options.tool} ${plan.argv.slice(0, -1).map(shellQuote).join(' ')} <prompt>`;
    console.log(`  cd ${shellQuote(plan.cwd)} && ${command}`);
    info('prompt:');
  ...
  // The binary is resolved HERE, not in the plan: a declared driver reads `--version`,
  // and the plan promises it does nothing. `opts.tool` is a test seam.
  const rawTool = opts.tool ?? plan.host.resolveToolBin(plan.driver.options.tool);
  const tool = withToolVersion(
    plan.driver, plan.host, normalizeLaunchTool(plan.driver, rawTool, plan.env),
  );
  ...
  const ultra = optionRefusal(plan.driver, plan.effort, tool, plan.host);
  if (ultra) fail(ultra);
  const provenance = launchProvenance(plan.host, tool);
  info(provenanceLine(provenance));

  // The set the participant is lifted with is announced by the same rules block as in
  // `--dry-run`.
  sayModule(plan);
  sayMcp(plan);
  // The same line the dry run prints, and for the reason `review` prints it too: the
  // canon travels as files, so whether it arrived is a fact about this lift.
  info(`workspace skills: ${skillsNote(plan)}`);
  info(`worker rules:`);
  for (const f of plan.rules) console.log(`  ${f}`);
  ...
  // The participant record is written before either repository repair step: otherwise
  // Ctrl+C on a long generator or `npm ci` leaves a directory with no journal line, and a
  // repeat hits "directory is taken".
  ...
  // One binding stays, so a task this session owns beside the bound one gets its contact point
  // only here and at the end of the turn — 03-cli § Guard and warden.
  ...
  clearThroughputSidecar(plan.home, plan.taskId, plan.address);
  ...
  // The repository's own generator runs after the journal record and before dependencies:
  // a Ctrl+C inside either long step cannot leave a directory the next spawn refuses to
  // reuse, and a generator that uses `npm run …` still sees the pre-install worktree. Both
  // outcomes are known before the launch is prepared. Launch files are intentionally
  // written only after dependency install, so a generator or install cannot be overwritten
  // by the driver; if install is interrupted, a repeat rewrites the launch files but does
  // not rerun or check dependencies (PB-8).
  ...
  if (freshWorktree) {
    ...
    worktreeDeps = installWorktreeDeps(plan.worktreePath);
    sayWorktreeDeps(worktreeDeps);
  } else {
    worktreeDeps = { ran: false, repeat: true };
  }
  ...
  const { output, session, seen } = await plan.driver.spawn(plan.launch, {
    ...
    persist: (id, state, sessionId) => upsertParticipant(plan.home, plan.taskId,
      { ...record, metadata: { ...record.metadata, session: id ?? null, ...(sessionId ? { sessionId } : {}) } }),
