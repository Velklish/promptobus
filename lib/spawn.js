...
**A check you leave out is not a check you passed.** Every one of the five is required, and the only way past one is its own \`notRun\` branch naming why it was impossible here. A check honestly declared impossible passes; the form has that branch precisely so it never rewards an invented number. The package does not run these checks for you and does not validate what you send — it publishes the shape and delivers the requirement, and the reviewer reads the record against the tree.${driver.phrases.promptRules}`;
}
