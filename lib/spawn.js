...
function buildPrompt({ taskId, nsPath, brief, rules, branch, driver, host, repoSkills, worktreeDeps, address }) {
...
export async function planSpawn(rootOrHost, opts) {
  ...
  const buildLaunch = (repoSkills, worktreeDeps = plannedWorktreeDeps) => {
    const prompt = buildPrompt({ taskId, nsPath, brief, rules, branch, driver, host, repoSkills, worktreeDeps, address });
