 ...
 * package в потоки процесса не пишет, а окружение и вывод — дело adapter'а (ADR-032, §2).
...
// Store шины `v0.61.0` — maildir-хранилище задачи (ADR-016). **Production store'ом он
// быть перестал**: cutover `BL-410` перевёл механизм на protocol v1 ([v1/](v1/)), и здесь
// остаётся тот единственный читатель, ради которого этот код живёт дальше, — миграция
// `.agents/a2a` → `.promptobus` ([migrate.ts](migrate.ts)). Второй вызывающий — набор:
...
// --- объявленная привязка «сессия → задача» (ADR-027) ------------------------
//
...
// --- вставка при слиянии track'а stalls (BL-416/418) в store.ts ------------------------
