// Восстановимый fan-out, mailbox и history protocol v1 (ADR-032, §6).
//
...
//
// Требование к ФС наследуется целиком (ADR-032, §4): жёсткие ссылки внутри одного тома. Их
...
/** Событие «кого будить». Форма — та, которую принимает `activate` driver'а (`BL-408`). */
 ...
 * блок.
 *
 * Экспортирован ради цитаты контракта (`BL-454`): справочник называет порог секундами, и
 ...
 * сообщении, то есть отказ на успехе (`BL-447`).
 *
 ...
 * ДОСТАВЛЕННОМ сообщении (`BL-447`). А вот если канона при этом нет — это настоящая потеря,
...
export function completeFanout(home: string, task: string, message: MessageV1, fault: FaultHook = NO_FAULT): string[] {
  ...
  for (const [index, recipient] of message.recipients.entries()) {
    ...
    // на одно сообщение ушли бы два события активации (`BL-455`). Доставка при этом одна —
