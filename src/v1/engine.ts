// Engine protocol v1: единственная дверь в store v1 (ADR-032, §6).
//
...
//
// К CLI engine подключён дверью механизма (`cli/lib/promptobus/store.js`): она открывает его
// с корнем рабочего места и правилом ATI, а модели отдаёт потребителям как есть (`BL-430`).
...
export interface EngineOptions {
   ...
   * `.promptobus` в конце может не стоять вовсе: `ATI_AGENTS_PROMPTOBUS_HOME` называет каталог, а
   * не рабочее место, и склеивать его с именем root'а второй раз означало бы уводить store
   * мимо того каталога, который назвал человек.
   */
  home?: string;
  ...
  recover?: boolean;
  /**
   * Версия механизма, который читает журналы этим engine (`BL-486`). Своей у package нет
   * вовсе — он едет внутри CLI, — поэтому её называет открывающий, как `home` и `policy`.
 ...
 * человеческие имена рядом (ADR-032, §6 — «упаковка каталога остаётся делом adapter'а»), а
...
export interface Engine {
   ...
   * тот же inode лишнего байта не стоит. Путь blob'а наружу при этом не выходит (ADR-032, §1).
   */
