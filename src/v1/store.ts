// Журнал задачи v1: задача, участники, owner и его явный захват (ADR-032, §6).
//
...
//    запись — иначе поля первого вызова исчезают молча (находка `BL-408`). Здесь такого
...
/**
 * Версия механизма, читающего журнал (`BL-486`). Приходит АРГУМЕНТОМ, как home и policy:
 * собственной версии у package нет вовсе (он приватный и едет внутри CLI), а копилка на
 * уровне модуля была бы мостом для чужого значения — ровно тем, что здесь запрещено
 * (ADR-032, §2). Задаётся при открытии engine и оттуда доходит до каждого чтения.
 *
...
// CLI: package не импортирует `cli/lib` вовсе, на этом стоит standalone-сборка. `null` —
...
export function readTask(home: string, task: string, cli: ReaderVersion = null): TaskV1 {
  ...
  if (!verdict.ok) {
    ...
    // журнала, а смесь версий после `sync` (`BL-486`): MCP-сервер шины живой сессии поднят
...
export function createTask(home: string, { id, title, owner, adapter = {} }: NewTask, now: Clock): TaskV1 {
  ...
  try {
    ...
    // тем же окном, только шире — между ней и записью помещается сосед (`BL-249`).
 ...
 * исчезают молча (`BL-408`). Схема проверяется ПОСЛЕ слияния — патч, ломающий запись,
