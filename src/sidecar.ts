...
// переезжают с версией протокола (ADR-032, §6). Эти файлы к протоколу не относятся вовсе:
// их формат задаёт adapter, читает и пишет их он же, а миграция переносит их байт в байт.
// Отдельный модуль и делает эту границу видимой: cutover `BL-410` сменил store, а не их.
//
...
// Слушание шины держится не на модели, а на процессе (ADR-028). Своего состояния у него
...
//
// Имя файла осталось от прежнего имени надзирателя (`BL-351`) и переименованию не
...
// записи сессии у harness'а нет вовсе (`BL-418`). Счётчик сторожа для этого не годится:
...
// --- привязки «сессия → задача» (ADR-027) ------------------------------------
//
 ...
 * `BL-426`: идентичность участника (роль и задача) сегодня доезжает до сессии только через
...
// процесс держит журнал, знает окружение, а окружение читает adapter (ADR-032, §2). Не
