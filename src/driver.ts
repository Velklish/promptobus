// Driver contract и driver registry, entry point "./driver".
//
...
// может — его сторожит гейт набора package.
 ...
 * есть одноимённый метод, и спрашивает их `requireCapability`. Четыре последних
...
export interface DriverCapabilities {
   ...
   * Спросит его первым `--harness`: там выбор driver'а делает человек.
   */
   ...
   * Спросит его первым `--harness`: у harness'а без входа человека совет
 ...
 * серверов. В собственный конфиг и arguments его переводит сам driver.
 */
...
/**
 * Контекст подъёма: всё, что driver'у нужно, и ничего про его harness. Ни одного
...
/**
 * План подъёма: перевод harness-neutral контекста в argv, конфиги и файлы.
...
export interface LaunchPlan {
  ...
  files: LaunchFile[];
  /**
   * Рабочий каталог сессии, если driver выбрал НЕ тот, что назвал вызывающий.
 ...
 * adapter'а, harness-specific команда приходит строкой отсюда: иначе команду входа
 * harness'а пришлось бы знать каждому, кто печатает строку про сессию.
 */
export interface DriverPhrases {
   ...
   * сессия. Имя собирает клиент, а не сервер, и у разных клиентов оно разное:
  ...
  /**
   * Правила ЭТОГО harness'а, которые дописываются к промпту участника: то, что
  ...
  promptRules: string;
  /**
   * Как зовётся сессия у этого harness'а, когда имя выбирает не механизм.
...
export interface DriverOptions {
  ...
  /**
   * Модель, которой участник поднимается без флага `--model`. Дом — driver:
   ...
   * ней, `inject` — вставка текста в живую сессию помимо сокета.
  ...
  envDrop: string[];
  /**
   * Утилиты рабочего места, без которых driver не поднимет сессию. Имена, а не
...
export interface StalledParticipant {
  ...
  reason: string;
  /**
   * Harness записи, как она его назвала. Нужен потребителю, чтобы спросить
...
export interface StopResult {
  ...
  note: string;
  /**
   * Команду гашения driver ОТДАЛ, а подтвердить исчезновение сессии не смог:
...
export interface Driver {
   ...
   * CLI берёт из него значения на верхнем уровне модуля, то есть упал бы на любой
   ...
   *
   * **Операция возвращается, когда сессии у harness'а уже НЕТ** — либо с честным
...
/**
 * Объявил ли driver свойство harness'а. Операции у таких флагов нет, поэтому
...
export function openParticipant(home: string, task: string, participant: ParticipantV1, registry: Registry, { mode = 'managed' as ParticipantMode } = {}): {
  ...
  // run'а бьются за журнал по построению, и проигравший обязан прочитать
 ...
 *
 * Исход `await`'ится: driver вправе дождаться, пока сессии у harness'а не станет,
