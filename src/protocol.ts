...
// остального механизма — `cli/lib/promptobus/store.js`: оттуда типы берёт `lint`, там же он берёт
// обе шапки гейта чужого mailbox'а. В `cli/lib/promptobus/contract.js` их нет вовсе — тот листовой,
// его читает `bushook.js`, а того каждый layout рабочего места. Второго списка в коде не
// бывает: гейт литеральных копий `lint` держит один дом на ключ, и он же назван картой
// `VALUE_HOMES` в `cli/lib/lint.js`.
...
// остаётся броском, а признак ожидаемой несёт класс: верхний catch `agents.js` опознаёт
...
// нет — `worker-x` сам по себе не адрес. С cutover'а `BL-410` это имя ещё и **id участника
...
export function participantFileStem(address: string): string {
  ...
  // никто не искал и никто не убирал (`BL-421`). Вход сюда сегодня недостижим: единственный
...
// куда core не заглядывает (ADR-032, §6). Заглянуть, однако, приходится в шести местах:
...
/**
 * Имя поля, в котором механизм оставляет свою версию при подъёме участника (`BL-486`).
 ...
 * сессию у harness'а (`claude attach/logs/stop`), и его же разбирает подъём из вывода `--bg`.
 */
 ...
 * (`CLAUDE_CODE_SESSION_ID`) и с которым приходит писать (`BL-469`, замечание ревью). Кладёт
 ...
 * там префиксная: harness называет одну сессию двумя написаниями — `CLAUDE_CODE_SESSION_ID`
 * даёт полный uuid, а короткий `id`, который подъём разобрал из вывода `--bg`, это первые
 * восемь hex того же uuid (замер 2026-09-03 на `claude` 2.1.251: `id: "e8c5be23"` при
 ...
 * или `null`: пишет своя либо сверить нечем (`BL-469`).
 *
