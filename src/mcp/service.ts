...
//
// Service передаётся factory явно, и дефолтной реализации у него нет: половина перечня —
// дело adapter'а, а не store. Владение mailbox'ом, привязка «сессия → задача», резолв активной
// задачи и шапка `PROMPTOBUS_HOME=… · задача=… · адрес=…` стоят на идентичности сессии, а
// окружение читает только adapter. Собирает service adapter потребителя.
//
