...
// эскалация молчания и решение «кого активировать» (ADR-032, §2 — строка «harness-neutral
// машина состояний надзирателя»).
//
...
/**
 * Стоп ли это на самом деле (`BL-416`). Пока на шине было ожидание, участник между
 * сообщениями сидел внутри вызова инструмента и для harness'а был занят; с
 * [ADR-031](../../../../docs/adr/adr-031-drop-a2a-wait.md) он, отправив сообщение, ход
...
export function stallStands(home: string, task: string, participant: ParticipantV1 | null | undefined, stall: SessionStall | null | undefined): boolean {
  ...
  // незаконченный старт, а не стоп (`BL-428`). Окно открылось вместе с новым входом в разбор:
  ...
  // участников** ([status.js](../../../lib/promptobus/status.js)) — положенное туда, оно оставило бы
  // её без защиты и развело каналы, ровно против того, ради чего `BL-416` предикат сводил.
  //
 ...
 * с которым её поднимали. Замер 2026-09-03 на `claude` 2.1.251: фоновые сессии — заранее
...
/**
 * Занята ли сессия участника ходом (`BL-418`). Ветки две, потому что участники двух родов,
...
export function blockedParticipants(home: string, task: string, participants: ParticipantV1[] | null | undefined, sessions: SessionSnapshot): StalledParticipant[] | null {
  ...
  // Harness записи едет в доклад вместе с ней (ADR-034): МАРШРУТ по стопу спрашивают у того
  ...
  for (const p of participants ?? []) {
    ...
    // нужны ровно для него (`stallStands`, `BL-416`).
    ...
    // (`wakeTakenBy`, `BL-469`). Для owner'а это тот же класс, что стоп, — сообщений от
 ...
 * секунду, и своего опроса ему не положено (`BL-418`). `null` — состояния сессий нет, и это
 * неизвестность.
 */
export async function supervisorRound(home: string, task: string, { now = Date.now(), registry, sessions = null as SessionSnapshot }: {
  ...
  for (const p of meta.participants ?? []) {
    ...
    // Чей это contact point на самом деле (`BL-469`): спрашивается до порогов перестука —
