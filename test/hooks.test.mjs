...
test('renderBusHook emits systemMessage and has no --output switch', () => {
  const text = renderBusHook({ syncHint: () => 'promptobus install' });
  assert.match(text, /Generated by promptobus install/);
  assert.match(text, /systemMessage/);
  assert.doesNotMatch(text, /--output/);
  assert.doesNotMatch(text, /additional_context/);
});

test('renderBusHook names the host regeneration command', () => {
  const text = renderBusHook({ commandName: 'alpha', syncHint: () => 'alpha install' });
  assert.match(text, /Generated by alpha install/);
});
...
test('generated runner itself does not create user-level hook directories', () => {
  ...
  writeFileSync(script, renderBusHook({ syncHint: () => 'promptobus install' }));
