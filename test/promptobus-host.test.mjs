...
const { planPromptobusHooks, GUARD_HOOK_EVENT } = await import(path.join(here, '..', 'dist', 'hooks.js'));
...
const plannedB = planPromptobusHooks(hostB);
// PB-173: the plan is the guard alone, and its command still follows the host —
// that was the property the removed bus-hook checks carried for this pair.
check('hook plan carries the guard, and its command follows the host bin',
  Object.keys(planned).join(',') === 'settings'
  && planned.settings[GUARD_HOOK_EVENT][0].hooks[0].command.includes(hostA.nodePath())
  && planned.settings[GUARD_HOOK_EVENT][0].hooks[0].command
     !== plannedB.settings[GUARD_HOOK_EVENT][0].hooks[0].command,
