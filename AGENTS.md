...
The task tracker and decision log live in `docs/` and are managed with `npx github:Velklish/backslop#v0.8.0` (configuration: `backslop.json`, task prefix: `PB`). There is no task list in files: `npx github:Velklish/backslop#v0.8.0 status` prints the queue, active work, deferred work, and triage. The operating rules are in `docs/backlog/README.md`; terms are in `docs/GLOSSARY.md`. The layout version is the `version` field in `backslop.json`; update backslop with `npx github:Velklish/backslop#v0.8.0 upgrade` when you decide to, not because of someone else's commit.
...
1. **Task.** Take the first queued task (`npx github:Velklish/backslop#v0.8.0 status`) or create one: `npx github:Velklish/backslop#v0.8.0 new <slug> --title "…"` puts it in triage; `--queue` puts it directly in the queue. A small change without a tracker record is allowed if one pass fully implements it and it does not change a contract. A finding this pass will not close becomes a file immediately: `npx github:Velklish/backslop#v0.8.0 new <slug> --parent N[.M]`, with evidence.
...
3. **Documentation in the same pass.** An undocumented change is incomplete: update the relevant `docs/reference/` section, the affected subsystem README, and CHANGELOG. An architectural decision needs `npx github:Velklish/backslop#v0.8.0 adr <slug>` and a row in `docs/README.md`.
4. **Gates before reporting.** `npx github:Velklish/backslop#v0.8.0 gates` is green: the commands in `gates` in `backslop.json`, the exit code of each, and the “gates N, green M” count. Verify a test change with a mutation probe: commit first, then run the probe.
5. **Acceptance and archive** are one approver pass: review the diff, run `npx github:Velklish/backslop#v0.8.0 archive N`, complete `result.md` (outcome, what was done, verification), and ensure `npx github:Velklish/backslop#v0.8.0 lint` is green. A worker does not declare their work accepted or move task files between directories.
6. **Triage review** follows closure immediately: every entry gets a next step — merge, clarify, `npx github:Velklish/backslop#v0.8.0 mv N queue`, or `npx github:Velklish/backslop#v0.8.0 mv N deferred` with a return condition. Ask the owner only before rejecting an entry.
...
**A fresh clone or worktree carries no generated adapter output** — `.claude/skills/backslop-*/SKILL.md` and `CLAUDE.md` are written by `npx github:Velklish/backslop#v0.8.0 init` and are not tracked. Run `init` first in a fresh checkout: without it `backslop lint` is red on "no generated output" and the skill files the lift prompt names do not exist; `git status` does not change from it.
