...
| `promptobus_mailbox` | `{ claim?, task? }` | Забрать непрочитанное и пометить прочитанным. На адресе orchestrator вызов без сессии получает копию, оставляет оригиналы, и ответ об этом говорит. `claim: true` перехватывает ящик у прежней сессии |
...
CI гоняет те же шаги на Node 20 и 22, на Ubuntu и macOS ([ci.yml](.github/workflows/ci.yml)). Гейты, которые обязана пройти правка, перечислены под ключом `gates` в [backslop.json](backslop.json).
