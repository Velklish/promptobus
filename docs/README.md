...
The canonical project documentation. For current work, use `npx github:Velklish/backslop#v0.8.0 status`; for project direction, see [ROADMAP.md](ROADMAP.md); for why the system is arranged this way, see the ADRs in the table below.
...
| [backlog/](backlog/README.md) | Task tracker: one file per task, status is the directory, summary is `npx github:Velklish/backslop#v0.8.0 status` | Living |
...
Create a new ADR with `npx github:Velklish/backslop#v0.8.0 adr <slug>` **and add a row to the table above**: without the row, `npx github:Velklish/backslop#v0.8.0 lint` fails.
