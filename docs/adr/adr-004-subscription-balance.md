...
1. Among **eligible** candidates, each `(harness, pool)` group is represented by its best tuple by the role's ordering that meets the role's quality floor, or by its best remaining tuple when that group has none; a pool-less binding window is the one account-wide group, and the group's `effective` is that tuple's.
2. If one or more representatives meet the role's quality floor, only those representatives enter the balance comparison. The largest `effective` leads. Every `(harness, pool)` group within `balance.band` of the leader is tied with it, and inside the tied set the **`balanced` score** of each group's representative decides, then ADR-003's own tie-break — confirmed availability, canonical priority, tuple id. Measuring the band from the single leader makes the tied set well defined whatever order the candidates arrived in, which is the determinism ADR-003 calls load-bearing.
...
**Amendment, 2026-09-09 (PB-59).** The quality floor sits above the balance comparison: when any representative meets it, only floor-meeting representatives enter the leader band and tie-break; when none does, all representatives remain eligible for the soft fallback. The existing `*-floor-not-met` wording is used only when no scored candidate meets the floor. If a scored floor-meeting candidate cannot be paced and a below-floor representative is taken, the same warning code names the taken tuple and harness, the floor-meeting tuple and harness, and says that its binding window was not paced. Below-floor candidates remain scored rows rather than being filtered.
