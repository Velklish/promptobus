...
| `workerPreamble` | `workerPreamble(ctx: { taskId: string; nsPath: string; branch: string; worktreePath: string }): string` | Never absent; it supplies the worker's host-specific preamble. `worktreePath` is the absolute path used as the worker session's cwd. A host may omit it from its own text: the package's allowed-roots line still names it, and no preamble check refuses that omission. |
...
**A Codex shell command is that command path, and it does carry the variable.** On codex-cli 0.156.1 a shell command inside the session has `CODEX_THREAD_ID` set. With the Claude and Cursor identity variables absent, the resolver returns that id and `status` prints it as `owner`. What a process whose environment carries the variable does at the owner gate is [03-cli § ownership](03-cli.md#ownership--the-owner-gate-of-done-stop-and-dismiss). The orchestrator's own MCP server under Cursor or Codex is not measured, and by the 0.146.0 record above that child has no identity. A `promptobus_mailbox` call from a process that names no session, on the `orchestrator` address, receives a copy, that section's no-identity line, the sentence that the originals stayed, and that gate's route for running the call again; the originals stay. A participant address is not that gate and still fetches.
