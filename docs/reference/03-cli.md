...
**The proven version is a measurement, not a floor.** `PROVEN_CLAUDE_VERSION` in `lib/driver-claude.js` is **2.1.263** (PB-34; 2.1.251 before it), and it is the version on which the warden's delivery by injection into a live session's messaging socket was last *observed to work*. It gates nothing: the driver does not refuse an older or a newer binary, and an injection a build silently stopped accepting costs a participant no message — it simply learns of one when it next calls `mailbox` itself. The number is there so that `promptobus status` and `doctor` can print `injection proven on claude <version>` and a "why does the participant not wake" hunt starts by comparing that against `claude --version`. The Cursor and Codex drivers spell the same constant differently on purpose: `PROVEN_CURSOR_VERSION` and `PROVEN_CODEX_VERSION` *are* floors and refuse a lift below them, because those two harnesses changed their lift surface under us and this one has not.
