...
JSON-RPC over that stdio stream uses one integer id space per direction. A server-to-client request (an approval, an elicitation) may therefore carry the same id as an outstanding `turn/start`. The holder classifies a message with `method` as a request or notification first; only an id without `method` is the client reply. An unmatched response is logged as `orphan` rather than dropped in silence.
