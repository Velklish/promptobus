...
| `role`, `harness`, `model`, `effort` | what actually ran. For Codex that is the level sent on `thread/start` as `config.model_reasoning_effort` (both roles) and, for a worker, also as `effort` on the first `turn/start` — not only the value printed on the status line |
...
**Everything measured above was measured for `turn/start`.** A reviewer is lifted with `review/start`, and it is held to the same criterion — the request acknowledged within 15 seconds, then `turn/started`. Whether the real binary answers `review/start` that quickly and emits `turn/started` for a review is assumed here, not measured: the test double treats the two methods alike, so the suite cannot tell them apart either. One live reviewer lift settles it, and if the assumption is wrong it is a finding rather than a silent gap.

**A Codex reviewer's effort is honoured on `thread/start`.** `ReviewStartParams` has no `effort` field, so the routed level cannot ride on `review/start`. Measured on codex-cli 0.146.0 with no paid turn: `thread/start` `config.model_reasoning_effort` is echoed as `ThreadStartResponse.reasoningEffort` (`low` and `xhigh` both held). The holder therefore puts the record's effort on that config key for worker and reviewer alike. The worker still also sends `effort` on its first `turn/start` — that value persists across later wakes on the same thread. Catalog tuples that name a Codex reviewer keep the role. Without `--effort` the key is omitted and the thread takes the account default.
...
An `mcpServer/elicitation/request` is declined with `{ action: "decline" }` — the shape app-server 0.146.0 documents for a declined elicitation, and the one that lets the turn continue. The participant has no person to fill a form. The holder log names the method plus allowlisted `server` / `mode` only; it does not write the prompt, schema, URL, or params blob. `item/tool/requestUserInput` and `currentTime/read` stay allowed.
