...
the participant has: a Codex reviewer lifts in an isolated `CODEX_HOME`, and the
owner's personal MCP set is not in it. Cursor remains
...
The holder is measured on macOS, and CI exercises it on Linux with stubs; Windows is outside the participant contract ([§ Spawn](#spawn)).
...
For approvals, the holder follows the generated codex-cli 0.146.0 schemas measured on 2026-09-10 ([fixtures](../../test/fixtures/codex-app-server/0.146.0/README.md)): there is no `item` wrapper around approval params. A live participant measurement on 2026-09-12 captured its own `apply_patch` call as:
...
about every binary or approval shape (PB-191, PB-194).
