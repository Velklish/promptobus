...
**The proven version is a measurement, not a floor.** `PROVEN_CLAUDE_VERSION` in `lib/driver-claude.js` is **2.1.263** (PB-34; 2.1.251 before it), and it is the version on which the warden's delivery by injection into a live session's messaging socket was last *observed to work*. It gates nothing: the driver does not refuse an older or a newer binary, and an injection a build silently stopped accepting costs a participant no message — it simply learns of one when it next calls `mailbox` itself. It was first proven by a spike on 2026-08-29 against 2.1.251 and re-proven on 2026-09-06 against 2.1.263. The number is there so that `promptobus status` and `doctor` can print `injection proven on claude <version>` and a "why does the participant not wake" hunt starts by comparing that against `claude --version`. The Cursor and Codex drivers spell the same constant differently on purpose: `PROVEN_CURSOR_VERSION` and `PROVEN_CODEX_VERSION` *are* floors and refuse a lift below them, because those two harnesses changed their lift surface under us and this one has not.

**Re-proving it costs one lift and one message.** The check is a participant in a throwaway standalone workspace (`promptobus.json` with `{ "tools": ["claude"] }` and `PROMPTOBUS_HOME` inside it), a brief short enough to end its first turn, and then a bus message sent to it while `claude agents --json` shows it on `idle`/`done`. The wake landed when the same record goes back to `busy`/`working` without anything else touching the session, and the participant's own reply arrives in the orchestrator mailbox. On 2.1.263, measured 2026-09-06: session `b1e3fc2c`, message out at 07:56:26.508Z, reply in at 07:56:32.744Z — 6.2 s — and the `agents --json` record form was unchanged (`pid, cwd, kind, startedAt, sessionId, name, id, status, state`).
...
**Opus 5.5 is in the list ahead of the proven binary.** The installed `claude` reached 2.1.280 on 2026-09-23, and its baked table — read offline, no paid turn — holds `claude-opus-5-5` as a first-party row of `family:"opus"` (`display_name` "Opus 5.5", knowledge cutoff June 2026, `fallback_3p:"claude-opus-5"`, `pricing:"tier_4_20_cache_read_0_20"`), lists it beside `claude-opus-5` in its array of accepted ids, and points the alias at it: `opus:{default:"claude-opus-5-5",per_provider:{bedrock:"claude-opus-5-5",vertex:"claude-opus-5-5",foundry:"claude-opus-4-6",mantle:"claude-opus-5-5",anthropic_aws:"claude-opus-5-5",gateway:"claude-opus-4-7"}}` and `latest_per_family.opus:"claude-opus-5-5"`, with `alias_migration` still `{}`. So the id enters `MODEL_IDS` and the catalog, the `Opus` scope names both ids, and the predecessor keeps its rows because the binary still takes it. What does not move yet is `PROVEN_CLAUDE_VERSION`, and with it `MODEL_ALIAS_IDS.opus`: an alias resolves where the *proven* binary points it, and until the one-lift re-proof above is made on 2.1.280, a run recorded as `--model opus` is read as `claude-opus-5` although the installed binary starts Opus 5.5 — the lag this section names, now on the alias rather than on the inventory. A routed pick names the full id, and there the lag runs the other way: the inventory is static, so on a build whose baked table lacks the id — 2.1.263 carries no `claude-opus-5-5` at all (`grep -a -c` over the binary: 0) — the resolver still picks an Opus 5.5 tuple, which outscores Opus 5 on every rung, and what such a build does with the id is not measured. A person on an older build denies it with an overlay, `deny: { models: ["claude-opus-5-5"] }`, until PB-245.1 re-proves the driver.
