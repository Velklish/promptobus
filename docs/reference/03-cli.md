...
`validate` covers the shape of every layer, duplicate tuple ids, a harness no driver of this CLI drives, an effort outside that driver's `EFFORT_LEVELS`, weights that do not sum to 100, references to a tuple, model, harness or effort that does not exist, a name that is both allowed and denied, and the host's own declaration — exactly one layer is writable whenever any layer is declared, reported here as a finding on the layer `host` rather than thrown, because a person whose host declares none would otherwise be told by `models validate` that their stack holds and by `models` that it does not. The layer list it prints marks the writable one. Its warnings are `stale-rating` — a rating older than `STALE_RATING_DAYS` (90), never an exclusion — and two of its own, `priority-duplicate` and `priority-not-canonical`, which check the canonical-priority convention the guide documents and never reach a decision.
