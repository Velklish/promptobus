...
**Fable 5.1's `quality` is a stated hypothesis, and its other two ratings are cited.** No SWE-bench Verified figure is published for that exact model: the leaderboard this catalog reads carries no Fable 5.1 row, and Anthropic's launch page publishes no SWE-bench figure for it at all (both seen 2026-09-06). What the vendor did publish is a head-to-head against Fable 5 on six benchmarks — Terminal-Bench 4.0, Terminal-Bench-Science 0.1, Humanity's Last Exam, GDPval-AA v2, OSWorld 2.0 and CursorBench 3.2.0 — with 5.1 ahead on every one, and **none of those has an anchor pair** in ADR-005's table, so none of them can be banded without a catalog-wide re-band. So `quality` is 10 in `evidence.hypothesis` with that reasoning written out: Fable 5's 95.0 % bands 10, the scale stops at 10, and a successor ahead on every published comparison cannot sit below its predecessor. `speed` cites Artificial Analysis at 70 tokens/s → band 2, and `quotaCost` cites Anthropic's own launch page, $10 in / $50 out → blended $30 → band 10; the cache-read cut to $0.25 is real and the blended `(input + output) / 2` rule does not read it. The same Artificial Analysis read also turned Fable 5's `speed` from a hypothesis into a citation at 72 tokens/s, in the same band 2 it already held.
