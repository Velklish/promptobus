...
**The text form ends with one line about the telemetry file** — how many records it holds and how large it is, and nothing read out of the records; reading them back is `models calibrate` below. The line is printed beside the decision rather than inside it, and `--json` does not print it at all: that form is one document a machine parses, and the count is not a field of it. See [Participant telemetry](#participant-telemetry).
...
`models` prints the pace table under the candidates, one row per `(harness, pool)` representative the document names, the pool when one is named, the binding window, both shares, the underspend, the penalty and the effective number. A harness with separate pool windows therefore has one pace row per pool; an account-wide window has one pool-less row. `--json` carries the same numbers on every scored candidate. Neither appears under the other four strategies: a decision carrying a number no rule of its own strategy used would invite a reader to believe one did. Under `balance`, a floor-meeting representative is considered before a below-floor representative's pace; the below-floor representative remains in the document, and can enter only when no paced representative meets the floor. If the floor-meeting candidate is scored but its binding window is not paced, the below-floor pick remains possible and the `*-floor-not-met` warning names both tuples and harnesses. **Nothing in this package pins the reviewer to a harness** — a reviewer is routed by pace like a worker, with the floor of 5 and the diversity bonus above it.
...
| `role`, `harness`, `model`, `effort` | what actually ran. For Codex that is the level sent on `thread/start` as `config.model_reasoning_effort` (both roles) and as `effort` on the first `turn/start` — not only the value printed on the status line |
