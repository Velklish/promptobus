...
**The other four keys of that file, and what each is for.** `worktree.bgIsolation: "none"` is written for the approver alone: the harness refuses a background session's `Write` and `Edit` in its main checkout, and the approver is seated in the clone root to write there ([05-drivers](05-drivers.md#the-approver-writes-to-the-shared-clone-the-harness-guard-and-the-key-that-lifts-it)). `enableAllProjectMcpServers` is there because without it a background session in a repository that has an `.mcp.json` stalls on the interactive server-choice dialog, and there is nobody at the keyboard to answer it — the same class of loss as the held postcard, arriving one step earlier. `viewMode: "focus"` is for the person who looks in: a summary rather than a sheet of tool calls. `permissions.deny` is written FIRST and only where tools are actually denied; key order in the file is not behaviour, so this is a convention rather than a requirement, and the reason to state it is that there is no reason to reshuffle it without cause.
