...
Per key it prints the run count, the medians of `durationSec`, `turns` and `reviewRounds` **per accepted piece** — a run that was not dismissed before `done` and sent at least one result — and the median window delta, which is one number per run: the largest movement among the windows that applied to it. A dismissed run still counts as a run and as spend and never as an accepted piece; a window that went backwards mid-run is a reset and contributes nothing.

The evidence threshold is **5 runs per key** ([ADR-005](../adr/adr-005-ten-point-scale-absolute-bands-calibrate.md), a constant and not an overlay key); below it the line reads `insufficient data: N of 5` and proposes nothing, and a key no catalog row rates gets its medians and no proposal. Among the eligible keys the most-observed one is the **pivot** and keeps its catalog bands; every other key moves from **its own** catalog band, by one band at a 1.5× surprise against the ratio its bands imply and by two at 3×, capped at ±2 and clamped to 1…10. Every band compared against is the **shipped catalog's**, not the merged stack's, so running the command twice on one telemetry file proposes the same thing twice instead of walking the rating a step per run; an override the person already has is printed beside the catalog band (`catalog 5, your overlay 6`). `speed` falls when a run takes unexpectedly long, `quotaCost` rises when its windows move unexpectedly far. A missing measurement omits that one rating and says why, and so does a measured zero — `usedPercent` is whole percent and `durationSec` whole seconds, so a median of 0 is below the resolution of the measurement rather than a value near zero. `quality` is never proposed from telemetry — the accepted-piece and review-round counts are printed as a note.

`--json` prints the whole report as one document. `--write` merges **only** the proposed `ratings` into the layer whose id is `user`, per tuple and per rating, keeping every other key of that file and every rating the person set by hand; a `user` overlay still on `schemaVersion` 1 is raised to 2 with the block, since a version 1 overlay carrying `ratings` no longer loads. Only ratings that moved are written: an override equal to the catalog band would go on overriding after the next catalog update moves the row underneath it. The command asks on the terminal first and takes `y` or `yes`, in any case; **without a terminal it refuses unless `--yes`** is also given, which is the non-interactive record that the person already agreed to exactly those lines. `--yes` on its own, with no `--write`, is refused rather than ignored. Under `--json` the outcome of the write is a field of the single document on stdout, and every refusal is raised before that document is printed. This is the one exception to the `user` layer being read-only for the tool ([ADR-004](../adr/adr-004-subscription-balance.md) § Host contract, superseded here for this command alone), and it is that narrow: only `calibrate --write`, only `ratings`, only after agreement.
...
2. when one or more representatives meet the role's quality floor, only those representatives enter the comparison. The largest `effective` leads, and every `(harness, pool)` group within `balance.band` of it is tied with it. The band is measured from the leader's **number**, which is what makes the tied set well defined whatever order the candidates arrived in;
...
A bus `result` from a live Codex reviewer is a later event on that turn, not a lift condition. Fixture waits that run across host sleep are not acceptance evidence. The personal MCP set is still not isolated; the holder does not write `~/.codex/config.toml`.

For approvals, the holder denies `config/read` by method, dangerous modes only in the request's own `sandbox`, `permissions`, or `approvalPolicy` fields (at the top level or under `item`), and mutation targets outside `cwd`/`addDirs`; a value equal to the session's recorded mode is treated as an echo except `danger-full-access`, which is always denied.
...
An `mcpServer/elicitation/request` is declined with `{ action: "decline" }`. The participant has no person to fill a form. The holder log names the method plus allowlisted `server` / `mode` and the JSON-RPC `id` only; it does not write the prompt, schema, URL, or params blob. `item/tool/requestUserInput` stays allowed, and `currentTime/read` is answered with the current time.
