...
That number is written out by hand and no gate compares it to `package.json`: it moves in the commit that cuts a release, and in no other. A reader who needs the version of the tree in front of them asks the tree — `promptobus --version` prints it without a host file.
