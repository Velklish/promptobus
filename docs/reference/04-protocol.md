...
**The engine does not validate it,** for the same reason it does not validate the gate record: `validate` knows four models and an artifact's bytes are opaque to it. `test/handover-record.test.mjs` asserts the schema by ajv and asserts that `validate` goes on not knowing the name. **`send` checks its SHAPE** by the same door and the same reader the gate record goes through — one mechanism covers both stems, and a record the published schema rejects is refused rather than landed. What `send` does not do, and must not, is run or grade the checks themselves: **running them is the participant's, and judging them is the reviewer's.** A mechanism that ran a worker's tests to grade its handover would be a second runner with a second opinion about a tree it does not own.
