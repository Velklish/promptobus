...
- **Priority:** P1.
- **Evidence level:** source/definition review at `1e0401a`, including `lib/install.js:442`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** Owns all manifest --check drift work, including the overlapping evidence in PB-154. Use one content delta for actual writes, --check and --dry-run; retain foreign manifest keys. PB-154 owns only validation of merged Cursor events.
