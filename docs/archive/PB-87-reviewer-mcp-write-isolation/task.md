...
- Extend the host contract with a per-server write-tool classification the host answers (e.g. `participantDenyTools(role)` on `src/host.ts`, alongside the existing `participantServers()`); consumer-cli' `cli/lib/promptobus/ati-host.js` answers it for the canonical set.
...
- **Priority:** P1.
- **Evidence level:** source/definition review at `1e0401a`, including `lib/review.js:1069`, `lib/driver-claude.js:216`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** High-priority design and capability verification, not an implementation-ready allow/deny list. Do not infer write capability from names alone. Specify a fail-closed reviewer MCP surface per harness while preserving bus result delivery; verify actual enforcement before claiming read-only. Changes in a consumer repository require a separately authorized task. No external write probe is authorized by this triage.
