...
Downstream, consumer-cli does depend on exactly the specifier form this package never tests: cli/lib/promptobus/host.js:4 and :9 `import ... from 'promptobus/host'`, cli/lib/sync.js:18 `import { planPromptobusHooks } from 'promptobus/hooks'` (both confirmed present in the consumer-cli checkout). So a broken or mis-pointed exports entry — e.g. a future rename of dist/host-index.js without updating package.json, or accidentally aliasing ./host to the wrong dist file — would pass npm test and npm pack --dry-run (the two commands .github/workflows/ci.yml runs) and only fail downstream, inside the consumer's own build.
...
- **Priority:** P2.
- **Evidence level:** source/definition review at `1e0401a`, including `test/promptobus-package.test.mjs:371`, `lib/host.js:4`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
