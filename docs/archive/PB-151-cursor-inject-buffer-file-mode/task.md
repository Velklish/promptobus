...
- **Priority:** P1.
- **Evidence level:** source/definition review at `1e0401a`, including `lib/cursor-persist.js:555`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** Without an explicit mode, visibility depends on umask and pre-existing file mode; do not claim every system creates a world-readable file. Require private mode on a restrictive- and permissive-umask fixture and cleanup on every post-creation exit.
