...
**The sender was then established free of charge**, without a model: the bus MCP server was started exactly as the participant's `config.toml` starts it — same command, same four environment variables read off the live stand — and driven by a client that **declares the elicitation capability**, so a server wanting one was free to ask. Through `initialize`, `tools/list` and a real `tools/call` of `promptobus_send`: zero server→client requests, the call succeeded, the message landed. So this package neither elicits nor needs to, and the elicitation originates in codex-cli, which attributes it to that server while asking something of its own. **What** it asks is not established — the fields are deliberately unlogged — and "per-call MCP approval" remains the reading the evidence fits, not a measurement. Filed as [PB-161.4](../../backlog/queue/PB-161.4-codex-participant-cannot-use-the-bus-elicitation-denied.md).
