...
- Read the access token from the keychain first, `CURSOR_API_KEY` second (the env path is unmeasured: a 401/403 on it is `quota_unknown`, on a keychain token `not_authenticated`); a token the adapter cannot read is `unknown` / `quota_unknown` — `cursor-agent status` has already answered the auth question and `models` listed the inventory, so the account is fine and only the second channel failed (ratified 2026-09-06 after the worker's deviation); the adapter never reads the refresh token and never writes the token anywhere.
