...
- Hidden models: the resolver's inventory is the snapshot rows with `hidden !== true` (ADR-004; the field is filled by PB-28), so a tuple on a hidden model is excluded by the existing `model-not-in-inventory` and never becomes a `runtime` row; a fixture with a hidden row.
...
- `routingMetadata()` in `lib/models.js` gains `windows: [{ id, kind, scope, usedPercent }]` — the chosen tuple's applicable windows at the lift (the set the pace block uses), `[]` when the harness has none; PB-36 reads it at `done` for the window delta of a run. Additive, on the participant's open `metadata`; named in [04-protocol](../../reference/04-protocol.md) § Participant metadata.
