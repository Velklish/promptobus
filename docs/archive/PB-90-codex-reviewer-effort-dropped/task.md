...
- **Priority:** P1.
- **Evidence level:** source/definition review at `1e0401a`, including `lib/codex-session.js:992`, `lib/driver-codex.js:284`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** The missing transmitted effort is code-supported. The proposed config key and readback mechanism remain protocol hypotheses until verified against the installed supported binary; confirm with protocol/config inspection before any paid turn. Do not remove a working turn-level setting without a worker regression case.
