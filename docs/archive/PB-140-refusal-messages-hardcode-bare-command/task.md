...
- **Created:** 2026-09-06
- **Dependencies:** PB-55, PB-54, PB-80
...
Under the real consumer host (`consumer-cli`, `node_modules/@consumer-workspace/consumer-cli/lib/promptobus/ati-host.js:23,260`): `const COMMAND = 'consumer-cli'; ... busCommand: (args) => [COMMAND, 'promptobus', ...args].join(' ')` — so the runnable command is `consumer-cli promptobus <verb>`, not `promptobus <verb>`. `which promptobus` in this workspace returns nothing (exit 1) — `promptobus` is not on `PATH`, so every one of the 27 literals is not just wrong prefix, it is unrunnable for a real user under the real host.
...
- **Priority:** P2.
- **Evidence level:** source/definition review at `1e0401a`, including `test/cli.test.mjs:99`, `lib/review.js:169`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
