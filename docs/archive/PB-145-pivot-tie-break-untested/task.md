...
- **Priority:** P2.
- **Evidence level:** source/definition review at `1e0401a`, including `lib/model-routing/calibrate.js:315`, `test/model-routing-calibrate.test.mjs:183`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** Add a dedicated tied-pivot fixture instead of changing a shared fixture whose unique pivot is an existing test premise. Mutation must demonstrate the ordering assertion fails, not merely any failure.
