...
`test/promptobus-e2e.test.mjs` and `test/promptobus-mixed.test.mjs` return the same `EPERM` from `test/scenario.mjs:317` on `/tmp/…/orchestrator.sock`. Each of the three was run twice back to back; the exit code of the `node --test` command was 1 every time, at load averages between 8 and 18 — far below the 115…198 of [PB-159.1](../PB-159.1-pooled-load-reds-beyond-preflight/task.md).
