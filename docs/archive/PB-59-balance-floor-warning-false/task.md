...
- **Priority:** P1.
- **Evidence level:** source/definition review at `1e0401a`, including `lib/model-routing/resolver.js:636`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** Queue the false-warning correction under the current selection policy. Preserve selection until the owner decides whether the quality floor must hold globally across pools. Include that policy decision in the implementation brief; no implicit global-floor change.
