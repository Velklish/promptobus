...
- `README.md` and `README.ru.md`: every command, flag and key of the series present once, with the precedence flag → overlay default → none; the line "The four strategies are …" (141 in both) becomes five; the sample `models` output (138 in both) no longer prints `unknown-remaining: claude exposes no limit source` — the adapters track's finding under PB-26 has the evidence (lands with it); the reference: the reason codes table unchanged unless a task added one; `docs/guides/model-routing.md` — the stale `<workspaceRoot>/model-routing.local.json` path in the layers table and the "Save it as …" line (PB-25's open edge).
- `skills/orchestrate/SKILL.md`: the rubric — `balance` is the strategy for a person who pays for several harnesses and wants them spent evenly; the `near-limit` line; the one-time Cursor plan question; the reviewer no longer pinned to one harness by the package; and the telemetry step — run `promptobus models --refresh` right before `promptobus done` so the record carries the end value of each window (the cache lives 60 s and `done` does not probe). `skills/solo-review/SKILL.md` follows for the reviewer.
- `docs/ROADMAP.md`: the goal of the series marked with evidence.
- `docs/GLOSSARY.md`: the rows for window, binding window, pace, tier, writable layer — from the "Terms this decision adds" table of ADR-004 — and for participant telemetry, telemetry record, window delta (PB-36); `docs/guides/model-routing.md`: the workspace overlay's new home and the fact that `<workspaceRoot>/model-routing.local.json` is no longer read.
