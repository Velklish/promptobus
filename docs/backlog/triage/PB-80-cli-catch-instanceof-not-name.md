...
`ResolveError` names nothing in this package: `grep -rn "class ResolveError" src lib dist` returns no hits. It is a class in the consumer repository ati-agents (its own `cli/lib/resolve.js`), and that repository's own host adapter already converts any `ResolveError` it might throw into a `HostResolveError` before the call reaches this package's `runPromptobus` — so as far as this catch is concerned, the `'ResolveError'` branch matches nothing reachable and is dead.
