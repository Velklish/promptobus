...
Under the only real consumer host, `ati-agents/cli/lib/promptobus/ati-host.js:259-260`, the two host members render differently: `formatNpx: (args) => ['npx', 'ati-agents', ...args].join(' ')` vs `busCommand: (args) => ['ati-agents', 'promptobus', ...args].join(' ')`. Reproduced now: `prune`'s dry-run line would render `npx ati-agents prune --older-than 14 --yes`; running `node cli/bin/agents.js prune --older-than 14 --yes` in the ati-agents repo answers `ati-agents: неизвестная команда «prune»` and prints help with exit 0 — the dispatcher's `case` labels (`cli/bin/agents.js:223-364`) have no top-level `prune` or `models`; both live only behind `case 'promptobus'` (line 322, subcommand allowlist at line 335). `busCommand` on the same host renders the working `ati-agents promptobus prune …`, and `lib/done.js:388` already prints exactly that form for the sibling cleanup hint.
