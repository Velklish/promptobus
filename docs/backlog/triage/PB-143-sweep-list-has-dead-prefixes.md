...
```
Cross-checking several of these against `ati-agents/cli/test/*.mjs` (the sibling package this repository was split from): `doctor.test.mjs:19`, `fresh.test.mjs:18` and `lint.test.mjs:22` now write `ati-agents-doctor-`, `ati-agents-fresh-`, `ati-agents-lint-` — and `ati-agents/cli/test/tmpdir-sweep.mjs` carries its own `SUITE_PREFIXES` with the `ati-agents-` spelling of these same names. This confirms the 32 dead entries in promptobus's list are leftovers from before the split, not live promptobus prefixes.
...
- The `ati-agents` copy of `tmpdir-sweep.mjs` — a separate repository with its own backlog.
