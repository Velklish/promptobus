...
scripts/audit-public.mjs:54 — `const LINK = /\]\(([^)#\s]+?)\)/g;` — is the only pattern the publicity gate uses to find markdown links, consumed by the loop at lines 65-72 that resolves each captured target against the filesystem and fails the gate on a missing or repo-escaping target. Because the character class excludes `#`, a captured group can never contain one — probed directly: ``x`` → no match at all, `[x](#sec)` → no match, ``x`` → matches `docs/A.md`, `[x](docs/A.md "T")` → no match (a titled link is excluded by the same class). Two consequences: (1) `target.startsWith('#')` at line 68 is dead code — `m[1]` can never start with `#` — which is itself evidence the fragment exclusion was accidental rather than intended, since the author clearly meant to special-case a pure-fragment link there; (2) any `path#anchor` link is never existence-checked or checked for leaving the repository — renaming or deleting the target file leaves `npm run audit` green.
