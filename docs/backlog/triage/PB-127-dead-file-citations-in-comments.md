# PB-127 · Comments across lib/, src/ and test/ still cite files and gates that live only in ati-agents' CLI, so a promptobus-only reader who follows them finds nothing
...
- Rewrite each dead citation: where the fact lives in this repo, name the actual file (`guardhook.js` → `src/hooks.ts`; `fresh.js`/`zone.js` → `./util.js`); where the gate lives only in ati-agents, reword to describe it generically without a bare filename (e.g. "the consumer CLI's contract-quote gate reads this value through the package") for `lint.js`/`plugin.js`/`doctor.js`/`headless.js`/`smoke.js` mentions and the eight dead test-file names.
...
- Fix `src/fs/proc.ts:1-3` to name `src/index.ts`/`src/legacy-store.ts`, and `src/v1/messages.ts:135-138` to name the ati-agents reference file and lint key explicitly; drop the unread `INTENT_STALE_MS` re-export from `lib/store.js:51` unless a consumer is expected soon.
...
- Porting the ati-agents contract-quote gate into this repository — a much larger change than the prose fix, and not required to make the comments honest.
...
- `grep -n "pidAlive" src/fs/proc.ts` shows the corrected file names; `grep -n "lint" src/v1/messages.ts` names the ati-agents reference explicitly.
