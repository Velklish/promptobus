...
- Cache at the file the host names (the routing-paths method of ADR-003; standalone answers `~/.promptobus/model-routing/cache.json` — account-scoped knowledge lives under the user home, not under the per-store `promptobusHome()`), mode 0600, written atomically. TTLs: auth and inventory 1 h, limit windows 60 s, confirmed exhaustion until known reset, exhaustion without reset until manual clear, transient failure 5 min. Account separation, if needed, by an opaque local fingerprint.
- `--refresh` ignores matching entries and probes live; `--clear-exhausted <harness>` drops the manual or reset-less exhaustion. `--dry-run` reads the cache only — no probes, no writes; `--refresh --dry-run` probes and still writes nothing (owner's decision 2026-09-05, reversing the plan's wording). A stale or absent snapshot under `--dry-run` is reported (`stale_cache`), not silently accepted. PB-21 wires the flags.
