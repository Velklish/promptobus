...
## Returned to the queue, 2026-09-16

The deferral's return condition is met: the consumer pins `promptobus#v0.9.0` (`cli/package.json`), and the installed copy carries provenance — `provenanceLine` is imported and called in the installed `lib/spawn.js` and `lib/review.js` of `node_modules/promptobus`, not only in the working tree. What the card still asks for is the check itself: one live lift, and the header read, where the field must name the installed copy's `package.json` path rather than the tree's. Path is the only discriminator — both copies report the same `version`.

It also unblocks `PB-185`, which named this card as what stops its firing measurement.
