...
- **Signal**: `models` (text and `--json`) prints a `near-limit` warning per harness whose binding window's `usedPercent` is at or past the threshold (`nearLimit.usedPercent`, default 80, an overlay key) or whose underspend is below `nearLimit.underspend` (default −15 percentage points — the pace layer's one unit, ADR-004); the warning names the window, the reset time and the strategy it would switch to — `economy` when every paced harness is past the threshold, `balance` otherwise (ADR-004); no line when the proposed strategy is already the effective one. Exhausted harnesses are already reported and are not repeated as near-limit.
