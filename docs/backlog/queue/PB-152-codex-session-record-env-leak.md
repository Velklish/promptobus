...
- **Next step:** Keep this focused on reducing durable environment retention; the session record is already written privately. Test with a synthetic canary variable, never real credentials. Preserve the current environment/drop-list semantics for the detached holder.
