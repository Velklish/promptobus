...
- **Priority:** P1.
- **Evidence level:** source/definition review at `1e0401a`, including `lib/codex-session.js:493`, `lib/worktree.js:239`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** The task cites realOrSelf as if it resolves a missing target through its nearest existing parent; verify that behavior before reuse. Tests need both a symlink into an allowed root and a symlink escape out of it. Compare canonical boundaries, including a not-yet-created target.
