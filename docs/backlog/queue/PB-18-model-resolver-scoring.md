...
- Filtering in this order: tuples from the catalog → allow/deny and explicit constraints → drop unrated → drop `unavailable` and `exhausted` → drop PAYG unless allowed → score → reviewer rules → tie-break. No candidates → a typed error the CLI turns into diagnostics before any write.
