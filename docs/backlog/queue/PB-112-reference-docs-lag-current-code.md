...
- Move the store/fan-out/recovery/validation/fault-injection sections from `consumer-cli/docs/reference/14-promptobus-v1.md` into `docs/reference/04-protocol.md`, in English; leave a pointer the other way in the consumer-cli doc; re-point `consumer-cli/cli/lib/lint.js`'s `intent-stale-ms` contract-quote gate at the promptobus copy
...
- `docs/reference/04-protocol.md` documents store layout, fan-out, recovery, and `INTENT_STALE_MS`'s value with a contract marker `consumer-cli`'s lint can check
...
- **Priority:** P2.
- **Evidence level:** source/definition review at `1e0401a`, including `src/v1/messages.ts:139`, `src/host.ts:184`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** This task is confined to promptobus documentation grounded in its source. A consumer pointer or contract-quote gate update is a separate follow-up in that repository and does not license a cross-repository edit here.
