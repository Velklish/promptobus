...
Дополнительная проверка командного пути подтвердила границу замера: `node test/promptobus-mcp.test.mjs`
в унаследованном окружении дал exit 1, 90/106 с красными `foreign-mailbox`, а тот же файл с
`env -u CODEX_THREAD_ID -u CLAUDE_CODE_SESSION_ID -u CURSOR_CONVERSATION_ID -u PROMPTOBUS_CURSOR_SESSION`
дал exit 0, 106/106. Знаменатель не изменился, значит это класс A окружения, а не доказательство
контракта; живая половина здесь неатрибутируема, потому что принадлежность сессии и есть предмет
PB-178.

## Граница с PB-179

**Связь с PB-179.** Незакрытая половина про `claim` не закрывается одним членом драйвера:
при разборе измерено, что store имеет только отрицательную проверку `foreignSession` и не хранит
положительную привязку participant address → session. Поэтому этот контракт остаётся своим
результатом, а положительная проверка участника и её дверь остаются предметом PB-179; в PB-179
реализация не вносилась.
