...
- **Priority:** P1.
- **Evidence level:** source/definition review at `1e0401a`, including `lib/cursor-persist.js:257`, `lib/codex-session.js:114`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** This is the cross-driver registry prerequisite. Preserve the missing-file case; distinguish a missing home declaration from unreadable or malformed records. Complete its changes to both drivers before the separate lifecycle tracks edit them.
