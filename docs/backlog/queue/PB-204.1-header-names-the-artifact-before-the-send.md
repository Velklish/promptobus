...
- Choose the closure and write the reason in this card. Two are known and both are a one-line change to the preambles: send the artifact as its own message first and quote the landed name in the result that follows — which is what the gate record already does and would make the accident deliberate; or give the result artifact a stem unique per attempt, so the passed name and the landed name cannot differ.
