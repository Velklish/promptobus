...
2. The review of a consumer run (11:26 UTC): holder log — `turn/started` 11:26:49, then `approval allow mcpServer/elicitation/request` at 11:26:58, then **nothing for 35 minutes**; both processes (`codex app-server --stdio` and the holder) at 0.0 % CPU; record `state: starting, busy: true` (a 0.4.0 record, the consumer was still on 0.4.0). Killed by the orchestrator at 12:04.

3. The same review lifted again at 12:05 UTC after the first reviewer was killed: `turn/started` 12:05:21, `approval allow mcpServer/elicitation/request` 12:05:30, then silence; killed by the orchestrator at 12:08 — three for three on `review/start`.
