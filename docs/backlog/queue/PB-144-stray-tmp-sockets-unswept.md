...
- Porting `release-gates.mjs` itself into this repository, or standing up CI to hang a release gate on: this repository has neither today, and duplicating a full release-gate script here would need to be kept in sync with consumer-cli' copy by hand for a problem `test/run.mjs` already solves for `$TMPDIR`.
...
- **Priority:** P1.
- **Evidence level:** source/definition review at `1e0401a`, including `test/sock-prefixes.mjs:1`, `test/tmpdir-sweep.test.mjs:97`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** A prefix plus age is not proof that a socket directory is abandoned. The cleanup design must preserve a live neighboring run and use ownership/liveness evidence; test refusal paths and never delete another active session.
