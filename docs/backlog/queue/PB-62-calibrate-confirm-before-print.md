...
- **Priority:** P1.
- **Evidence level:** source/definition review at `1e0401a`, including `lib/models.js:722`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** The proposal must be visible before confirmation. Use the currently documented non-TTY refusal contract for --write without --yes, including an empty proposal; changing that contract is outside the default fix.
