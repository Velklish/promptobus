...
- **Priority:** P1.
- **Evidence level:** source/definition review at `1e0401a`, including `src/hooks.ts:75`, `lib/install.js:317`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** First inspect existing busArgv/layoutBinPath seams; add no public host member unless the existing interface cannot express the command. Implement only this repository and a fake consumer host here; coordinate any real consumer change separately.
