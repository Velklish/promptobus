...
- **Priority:** P1.
- **Evidence level:** source/definition review at `1e0401a`, including `lib/install.js:156`, `src/hooks.ts:75`. Historical live measurements were not repeated; a regression reproducer is still required before a runtime fix is accepted.
- **Next step:** Do not broaden ownership to every command containing the words promptobus guard: that can remove foreign hooks. Require a portable positive ownership signature and regression cases for unrelated guard-like commands. Choose the smallest host-compatible solution after PB-103; no new committed manifest by default.
