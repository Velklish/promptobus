...
The operational tracker for promptobus: **one task is one file**, and **status is the directory** containing it. There is no task list here — `npx github:Velklish/backslop#v0.8.0 status` prints it. For overall project direction, see [ROADMAP.md](../ROADMAP.md); for closed tasks, see the [archive](../archive/README.md).

## Directories

| Directory | Contents | How it gets there |
|---|---|---|
| `triage/` | Unreviewed ideas and findings. The file is the entry | `npx github:Velklish/backslop#v0.8.0 new <slug> --title "…"`; a finding is `npx github:Velklish/backslop#v0.8.0 new <slug> --parent N` |
| `queue/` | The queue; priority is the integer “Order” field, in steps of 10; lower comes first | `npx github:Velklish/backslop#v0.8.0 new <slug> --queue [--top]`, `npx github:Velklish/backslop#v0.8.0 mv N queue [--top \| --after M]` |
| `active/` | Work in progress; “Taken” is the date it was started | `npx github:Velklish/backslop#v0.8.0 mv N active` by the person holding the queue |
| `deferred/` | Deferred work; the “Deferred” section gives the reason and return condition | `npx github:Velklish/backslop#v0.8.0 mv N deferred`, then complete the section |
| [`../archive/`](../archive/README.md) | Closed: `task.md` + `result.md` | `npx github:Velklish/backslop#v0.8.0 archive N`, then complete `result.md` |
...
- **Numbers are sequential** and never reused after closure; `npx github:Velklish/backslop#v0.8.0 new` assigns them across directories. Two parallel creations can take the same number; `npx github:Velklish/backslop#v0.8.0 lint` catches this at merge time, and the loser recreates the file.
...
- **Priority = the “Order” field** in `queue/`. Reorder with `npx github:Velklish/backslop#v0.8.0 mv N queue --top`, `--after M`, or by editing the number manually.
- **Closure** — completed, rejected, or merged — uses `npx github:Velklish/backslop#v0.8.0 archive N`: the file moves to the archive as `task.md`, alongside a dated `result.md`. The approver completes the outcome and result; while `result.md` contains `[TODO]`, `lint` fails.
...
- Project gates are the `gates` field in `backslop.json`; `npm test` runs first and `npx github:Velklish/backslop#v0.8.0 lint` is among them.
