...
**Codex inventory is home- and visibility-specific.** On codex-cli 0.146.0, the measured command was `/opt/homebrew/bin/codex debug models`, run with the participant's exact `CODEX_HOME` and with each row's `visibility` recorded. That participant home returned ten rows: `gpt-5.4-mini` was present but `visibility: hide`, alongside three other hidden rows; two runs in the same home at `2026-09-12T14:12:47.696Z` and `2026-09-12T14:12:49.760Z` matched exactly. The same executable with `CODEX_HOME` unset was unstable across three calls on 0.146.0 — 8, 7 and 8 rows, not one stable seven-row set ([03-cli § Codex availability](../reference/03-cli.md#codex-availability)) — so a bare count from either home is not a participant inventory on that build. The resolver's hidden-row behavior is covered separately by the `model/list` adapter tests: it keeps the raw hidden row for snapshot fidelity, filters it from selectable inventory and prints a matching catalog tuple as `model-not-in-inventory`; catalog membership is therefore not a promise that every home exposes the model. It is not inferred from the debug listing: PB-196 found that on 0.156.1 the row names and hidden marks match — the two rows `debug models` marks `hide` come back from `model/list` only under `includeHidden: true`, and then with `hidden: true` ([03-cli § Codex availability](../reference/03-cli.md#codex-availability)). `models validate` stays offline because it has no participant home to inspect.

**On codex-cli 0.156.1 the owner's home and a participant home answer alike.** On 2026-09-23 the same command ran twice with `CODEX_HOME` unset. It also ran twice in a participant home built by the driver's own `makeParticipantHome`: a copy of `auth.json` and a config that trusts the working directory, with no MCP table. All four payloads were byte-identical, each with nine rows. Seven are `visibility: list`: `gpt-6-astra`, `gpt-6-sol`, `gpt-6-luna`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna` and `gpt-5.5`. Two are `hide`: `gpt-reserve` and `codex-auto-review`. The ladders were `low` … `ultra` for Astra, both Sols and Terra, `low` … `max` for both Lunas, and `low` … `xhigh` for `gpt-5.5`. So the 0.146.0 difference between the two homes did not recur on this build. `gpt-5.4-mini` was listed in neither home, and its catalog rows stay. On 2026-09-25 the four calls were repeated with the same byte-identical answer, and `model/list` was asked in both homes, with no turn: the two payloads were byte-identical, the seven `list` rows and no `hidden` one. The two `hide` rows come back only with `includeHidden: true`, which the adapter does not send ([03-cli § Codex availability](../reference/03-cli.md#codex-availability)). The catalog now rates each listed effort for `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna` and `gpt-5.5`; the five added rungs use the existing base rows' ladder arithmetic.

**The GPT-6 rows (PB-246)** take the full ladders that listing offers, and their bases were rated by the steps below. Every page was opened on 2026-09-23.

- `codex-gpt6-astra-max` cites Terminal-Bench 2.1 under Codex CLI, 87.4 % ±0.9, which bands 9 on the Codex CLI pair. The SWE-bench Verified leaderboard this catalog reads carries no GPT-6 row. Its page does show a site-wide "Current leader · BenchAlign" banner naming GPT-6 Astra at 88.47, 90 % interval 84.0–92.9. That is the site's aggregate over 196 models, not a SWE-bench Verified figure, and it names no harness. The 2.1 page names no effort, so the figure sits on `max`, as described in [Re-rating a row](#re-rating-a-row). The Terminal-Bench 4.0 leaderboard does name the effort, and it puts Astra's `high`, `xhigh` and `max` level within one interval: 57.9 %, 57.9 % and 58.2 %. So the one-band-per-step ladder rates `high` and `xhigh` lower than that leaderboard measures them. Astra has no predecessor, so the successor rule does not apply.
- `codex-sol-max` is re-placed at a `quality` hypothesis of 8. It had been 10 on a Terminal-Bench 2.1 figure of 88.8, which the page no longer carries. Every measure that was opened and covers both neighbours puts GPT-5.6 Sol at max below Astra at max, which is 9, and above Terra at max, which is 7:
  - Terminal-Bench 4.0 under Codex: Astra 58.2 %, Sol 37.3 %, Terra 21.5 %.
  - Terminal-Bench 3.0: Sol 34.6 % against Terra 20.8 %.
  - The Artificial Analysis Intelligence Index: Astra 53, Sol 47, Terra 42.

  The only band strictly between 7 and 9 is 8. None of these figures has a pair for Codex, so the rating stays a hypothesis. The same pass re-banded its `speed` to band 2, at 72.6 tokens/s where it had been 90. Its `quotaCost` takes the vendor promotion of $4 / $20 and lands in band 4; the vendor says the promotion is in force at least through 2026-11-21. The list price of $5 / $30 would give band 6.
- `codex-gpt6-sol-max` and `codex-gpt6-luna-max` have no SWE-bench Verified or Terminal-Bench figure. Each is a hypothesis at its predecessor's band: 8 for Sol and 6 for Luna, and each moves when its predecessor does. The vendor post with the head-to-heads, `https://openai.com/index/introducing-gpt-6-sol-and-luna/`, answered HTTP 403, so none is cited. The one common measure that could be opened, the Artificial Analysis Intelligence Index at max, puts each successor level with its predecessor: 48 against 47 for Sol and 37 against 37 for Luna.

**The Grok 4.7 rows (PB-254)** are Cursor rows whose ids drop the `cursor-` prefix the Grok 4.6 ids carry: `cursor-agent` 2026.09.23 lists `grok-4.7-low` … `grok-4.7-xhigh`, each with a `-fast` twin that the catalog does not rate, as it rates no `-fast` id of any family. The base is `high`, like its predecessor's, and every page was opened on 2026-09-25.
...
- `speed` cites Artificial Analysis at 52.4 tokens/s, band 1, and `quotaCost` the xAI list price of $2 / $6, blended $4, band 1. The Grok 4.6 rows keep their `quotaCost` of 10, a hypothesis from a local usage spike written when no list price was published for that model. On ratings alone the `quality`, `balanced` and `economy` weights put 4.7 above 4.6 at every rung, while the `speed` weights keep 4.6 ahead on its speed band; a local correction of the `quotaCost` band goes through `models calibrate`.
...
**Only Claude Code lifts an approver.** Its floor is 7, and exactly 15
...
- initial numbers go in steps of ten; a rung added later takes an unused gap between two tens, preserving existing priorities and their relative order.
...
The Codex rollout is not a sidecar producer. `event_msg.token_count.info` in the Codex-owned rollout contains token usage (see the normalized records of [0.146.0](../../test/fixtures/codex-app-server/0.146.0/TokenUsage-0.146.0-2026-09-12.json) and [0.156.1](../../test/fixtures/codex-app-server/0.156.1/TokenUsage-0.156.1-2026-09-25.json)). On codex-cli 0.156.1, `token_usage_record` also carries output tokens and `task_complete` carries `duration_ms` and `time_to_first_token_ms`. A fresh one-response turn on 2026-09-25 had 10 output tokens, 4717 ms duration and 4329 ms to first token. A second fresh turn had 39 output tokens, 12142 ms duration and 3850 ms to first token; the 8292 ms difference included a measured 2881 ms wait for a `sleep 3` command and a later model response. Thus neither the full duration nor the difference is model-active generation time. The rollout remains outside the bus in a harness-specific participant home, so it is still refused as a source. Claude's `assistant.message.usage` has the same model-active gap; Cursor supplies output tokens without a generation duration. A rate computed from turn wall time is not a model-active throughput observation. PB-57.1's return condition remains unmet: no stable bus-facing observation supplies model-active time or a direct rate.
