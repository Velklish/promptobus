...
- **Addressed acceptance.** `approver:<slug>` is a fourth participant, lifted with `promptobus review <path> --task <id> --approver` once a reviewer result is on record; its registered address can talk directly to workers in that task only while the calling session holds it, and the canonical exchange stays in the task journal.
